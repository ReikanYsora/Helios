...
return new Date((d+u)/2)}var kt=null;function renderTimelineNightZones(e){const t=function computeNightIntervals(e){const t=e._timeRange;if(!t)return[];const i=getHomeCoords(e.config,e.hass);if(!i)return[];const n=t.start.getTime(),r=t.end.getTime(),s=r-n;if(s<=0)return[];const l=`${n}|${r}|${i.lat.toFixed(4)}|${i.lon.toFixed(4)}`;if(kt&&kt.key===l)return kt.out;const d=[],u=new Date(t.start);u.setHours(0,0,0,0),u.setDate(u.getDate()-1);const c=r+864e5;for(;u.getTime()<=c;){const e=u.getTime(),t=e+864e5,n=findSunCrossing(i.lat,i.lon,e,t,"rising"),r=findSunCrossing(i.lat,i.lon,e,t,"setting");n&&d.push({ms:n.getTime(),kind:"sunrise"}),r&&d.push({ms:r.getTime(),kind:"sunset"}),u.setDate(u.getDate()+1)}d.sort((e,t)=>e.ms-t.ms);const p=[];let g=null,m=!1;for(const v of d)"sunset"===v.kind?g=v.ms:(null!==g?(p.push({startMs:g,endMs:v.ms}),g=null):m||p.push({startMs:-1/0,endMs:v.ms}),m=!0);null!==g&&p.push({startMs:g,endMs:1/0});const f=[];for(const v of p){const e=Math.max(v.startMs,n),t=Math.min(v.endMs,r);t>e&&f.push({startPct:(e-n)/s*100,endPct:(t-n)/s*100})}return kt={key:l,out:f},f}(e);return 0===t.length?K:B`
    ...
    `}async function fetchHaSolarForecast(e){if(e.hass?.callWS&&!(e._haSolarForecastFetching||e._haSolarForecastLoaded&&Date.now()-(e._haSolarForecastFetchedAt??0)<3e5)){e._haSolarForecastFetchedAt=Date.now(),e._haSolarForecastFetching=!0;try{const t=await async function fetchHeliosSeries(e){const t=e._energyDefaults?.solarForecastEntryIds??[];if(0===t.length)return null;const i=new Date(localMidnightMinusDays(e._periodPastDays)).toISOString(),n=new Date(localMidnightMinusDays(-(e._periodFutureDays+1))).toISOString(),r=await Promise.all(t.map(t=>callWS(e.hass,{type:"helios_forecast/series",entry_id:t,start:i,end:n}).then(e=>e?.points??null).catch(()=>null))),s=/* @__PURE__ */new Map;let l=!1;for(const d of r)if(Array.isArray(d)){l=!0;for(const e of d){const t=Date.parse(e.t);Number.isFinite(t)&&"number"==typeof e.pv_w&&Number.isFinite(e.pv_w)&&s.set(t,(s.get(t)??0)+e.pv_w)}}return l?[...s.entries()].map(([e,t])=>({tMs:e,w:t})).sort((e,t)=>e.tMs-t.tMs):null}(e);e._haSolarForecast=null!==t?t:function mergeSolarForecast(e){if(!e||"object"!=typeof e)return[];const t=/* @__PURE__ */new Map;for(const l of Object.keys(e)){const i=e[l]?.wh_hours;if(i&&"object"==typeof i)for(const e of Object.keys(i)){const n=Date.parse(e);if(!Number.isFinite(n))continue;const r=i[e];"number"==typeof r&&Number.isFinite(r)&&t.set(n,(t.get(n)??0)+r)}}const i=[...t.entries()].map(([e,t])=>({tMs:e,wh:t})).sort((e,t)=>e.tMs-t.tMs),n=[];for(let l=1;l<i.length;l++){const e=i[l].tMs-i[l-1].tMs;Number.isFinite(e)&&e>0&&n.push(e)}let r=me;if(n.length>0){n.sort((e,t)=>e-t);const e=n[Math.floor(n.length/2)];Number.isFinite(e)&&e>0&&(r=e)}const s=me/r;return i.map(e=>({tMs:e.tMs,w:e.wh*s}))}(await callWS(e.hass,{type:"energy/solar_forecast"})),e._haSolarForecastLoaded=!0,e.requestUpdate()}catch(L){e._haSolarForecastLoaded=!0}finally{e._haSolarForecastFetching=!1}}}function forecastWattsAt(e,t){if(0===e.length)return null;let i=0,n=e.length-1,r=-1;for(;i<=n;){const s=Math.trunc((i+n)/2);e[s].tMs<=t?(r=s,i=s+1):n=s-1}if(r<0)return null;const s=e[r],l=e[r+1];if(l&&l.tMs-s.tMs<=54e5&&l.tMs>s.tMs){const e=(t-s.tMs)/(l.tMs-s.tMs),i=e<0?0:e>1?1:e;return s.w+(l.w-s.w)*i}return t>=s.tMs+36e5?null:s.w}function forecastAverageWatts(e,t,i){if(0===e.length||i<=t)return null;const n=18e5;let r=0,s=0;for(let l=t+9e5;l<i;l+=n){const t=forecastWattsAt(e,l);r+=null!==t&&Number.isFinite(t)?Math.max(0,t):0,s++}return s>0?r/s:null}function bucketForMs(e,t,i,n){if(t<e)return-1;const r=Math.floor((t-e)/i);return r>=n?-1:r}function interpolateNullGaps(e){const t=e.length;let i=0;for(;i<t;){if(null!==e[i]){i++;continue}let n=i;for(;n<t&&null===e[n];)n++;const r=i>0?e[i-1]:null,s=n<t?e[n]:null;if(null===r&&null===s)return;if(null===r)for(let t=i;t<n;t++)e[t]=s;else{if(null===s){for(let n=i;n<t;n++)e[n]=r;return}{const t=n-i+1;for(let l=i;l<n;l++){const n=(l-i+1)/t;e[l]=r+(s-r)*n}}}i=n}}function buildGridChange(e,t,i,n,r){const s=changeSeriesToWatts(e,t,i,n,r);for(let u=0;u<s.length;u++){const e=s[u];null!==e&&e<0&&(s[u]=0)}const l=bucketForMs(t,r,i,n),d=Math.min(n,l<0?0:l+1);if(d>0){const e=s.slice(0,d);interpolateNullGaps(e);for(let t=0;t<d;t++)s[t]=e[t]}return s}function changeSig(e){const t=e?.length??0;if(0===t)return"0";const i=e[t-1];return`${t}.${i.endMs}.${i.kwh.toFixed(3)}`}function computeDataVersion(e){const t=/* @__PURE__ */(new Date).toDateString(),i=modeBucketsPerHour(e._timelineMode,e.config),n=`${e._timelineMode}.${e._periodPastDays}.${e._periodFutureDays}`,r=e._chartSeries,s=r?.times.length??0;return`d${t}|w${n}|c${i}|s${0===s?"0":`${s}.${r.times[s-1].getTime()}.${r.irradiance[s-1]??0}`}|pv${changeSig(e._pvChangeSeries)}|bc${changeSig(e._batteryChargeChangeSeries)}|bd${changeSig(e._batteryDischargeChangeSeries)}|gi${changeSig(e._gridImportChangeSeries)}|ge${changeSig(e._gridExportChangeSeries)}|f${e._haSolarForecast?.length??0}`}function buildUnifiedStore(e){const t=modeBucketsPerHour(e._timelineMode,e.config),i=24*t,n=e._periodPastDays,r=n+1+e._periodFutureDays,s=r*i,l=me/t,d={bucketsPerHour:t,bucketsPerDay:i,bucketsTotal:s,stepMs:l},u=function storeOriginMs(e){return localMidnightMinusDays(e)}(n),c=u+r*fe,p=Date.now(),g=function buildIrradiance(e,t,i,n){const r=new Array(n.bucketsTotal).fill(null),s=e._chartSeries;if(!s||0===s.times.length)return r;const l=new Array(n.bucketsTotal).fill(0),d=new Array(n.bucketsTotal).fill(0);for(let u=0;u<s.times.length;u++){const e=s.times[u].getTime();if(e<t||e>=i)continue;const r=s.irradiance?.[u];if("number"!=typeof r||!Number.isFinite(r)||r<0)continue;const c=bucketForMs(t,e,n.stepMs,n.bucketsTotal);c<0||(l[c]+=r,d[c]+=1)}for(let u=0;u<n.bucketsTotal;u++)d[u]>0&&(r[u]=l[u]/d[u]);return interpolateNullGaps(r),r}(e,u,c,d),m=function buildProduction(e,t,i,n,r){const s=changeSeriesToWatts(e._pvChangeSeries,t,r.stepMs,r.bucketsTotal,n);for(let u=0;u<s.length;u++){const e=s[u];null!==e&&e<0&&(s[u]=0)}const l=bucketForMs(t,n,r.stepMs,r.bucketsTotal),d=Math.min(r.bucketsTotal,l<0?0:l+1);if(d>0){const e=s.slice(0,d);interpolateNullGaps(e);for(let t=0;t<d;t++)s[t]=e[t]}return s}(e,u,0,p,d),f=function buildForecast(e,t,i,n){const r=new Array(n.bucketsTotal).fill(null),s=e._haSolarForecast;if(!s||0===s.length)return r;const l=n.stepMs>me;for(let d=0;d<n.bucketsTotal;d++){const e=t+d*n.stepMs,u=e+n.stepMs/2;if(u<t||u>=i)continue;const c=l?forecastAverageWatts(s,e,e+n.stepMs):forecastWattsAt(s,u);null!==c&&Number.isFinite(c)&&(r[d]=Math.max(0,c))}return r}(e,u,c,d),v=function buildBattery(e,t,i,n){const r=changeSeriesToWatts(e._batteryChargeChangeSeries,t,n.stepMs,n.bucketsTotal,i),s=changeSeriesToWatts(e._batteryDischargeChangeSeries,t,n.stepMs,n.bucketsTotal,i),l=new Array(n.bucketsTotal).fill(null);for(let c=0;c<n.bucketsTotal;c++){const e=r[c],t=s[c];null===e&&null===t||(l[c]=Math.max(0,e??0)-Math.max(0,t??0))}const d=bucketForMs(t,i,n.stepMs,n.bucketsTotal),u=Math.min(n.bucketsTotal,d<0?0:d+1);if(u>0){const e=l.slice(0,u);interpolateNullGaps(e);for(let t=0;t<u;t++)l[t]=e[t]}return l}(e,u,p,d),b=buildGridChange(e._gridImportChangeSeries,u,d.stepMs,d.bucketsTotal,p),y=buildGridChange(e._gridExportChangeSeries,u,d.stepMs,d.bucketsTotal,p);return{storeStartMs:u,storeEndMs:c,bucketsPerHour:t,bucketsPerDay:i,bucketsTotal:s,stepMs:l,builtAtMs:p,dataVersion:computeDataVersion(e),irradiance:g,production:m,forecast:f,battery:v,gridImport:b,gridExport:y}}function valueAt(e,t,i){if(i<t.storeStartMs||i>=t.storeEndMs)return null;const n=(i-t.storeStartMs)/t.stepMs-.5,r=Math.max(0,Math.min(t.bucketsTotal-1,Math.floor(n))),s=Math.max(0,Math.min(t.bucketsTotal-1,r+1)),l=e[r],d=e[s];if(null===l&&null===d)return null;if(null===l)return d;if(null===d)return l;return l+(d-l)*Math.max(0,Math.min(1,n-r))}function sliceForRange(e,t,i){const n=Math.max(e.storeStartMs,t),r=Math.min(e.storeEndMs,i);if(r<=n)return{times:[],production:[],forecast:[]};const s=e.stepMs,l=Math.floor((n-e.storeStartMs)/s),d=[],u=[],c=[];for(let p=e.storeStartMs+l*s+s/2;p<r;p+=s)p<n||(d.push(new Date(p)),u.push(valueAt(e.production,e,p)),c.push(valueAt(e.forecast,e,p)));return{times:d,production:u,forecast:c}}function groupDevices(e,t,i){const n=monitoringGroups(e),r=hiddenDevices(e);return t.devices.filter(e=>""!==e.statConsumption&&n.get(e.statConsumption)===i&&!r.has(e.statConsumption))}function deviceName(e,t){return t.name||String(e?.states?.[t.statConsumption]?.attributes?.friendly_name??"")||t.statConsumption}function deviceIcon(e,t){const i=e?.states?.[t.statConsumption]?.attributes?.icon;return"string"==typeof i&&i||"mdi:flash"}function deviceWindowKwh(e,t,i){if(!e)return 0;let n=0;for(const r of e){const e=(r.startMs+r.endMs)/2;e>=t&&e<=i&&isFinite(r.kwh)&&(n+=Math.abs(r.kwh))}return n}function refreshDeviceConsumption(e){if(!e.hass)return;const t=function groupedDevices(e,t){const i=monitoringGroups(e),n=hiddenDevices(e);return t.devices.filter(e=>""!==e.statConsumption&&i.has(e.statConsumption)&&!n.has(e.statConsumption))}(e.config,e._energyDefaults).map(e=>e.statConsumption);if(0===t.length)return void(e._deviceChangeSeries.size>0&&(e._deviceChangeSeries=/* @__PURE__ */new Map,e.requestUpdate()));const i=localMidnightMinusDays(e._periodPastDays),n=changeRefreshAnchorMs(),r=[...t].sort(),s=`${r.join(",")}|${i}|${n}`;e._deviceChangeFetch.run(s,()=>fetchChangeById(e.hass,r,i,n,e._storeFetchPeriod).then(t=>{if(null===t)return;const i=/* @__PURE__ */new Map;for(const e of r){const n=mergeChangeSeries(t,[e]);null!==n&&i.set(e,n)}e._deviceChangeSeries=i,e.requestUpdate()}))}function interpAt(e,t,i){const n=Math.min(e.length,t.length);if(0===n)return NaN;if(i<=e[0].getTime())return isFinite(t[0])?t[0]:NaN;if(i>=e[n-1].getTime()){const e=t[n-1];return isFinite(e)?e:NaN}let r=0,s=n-1;for(;s-r>1;){const t=Math.trunc((r+s)/2);e[t].getTime()<=i?r=t:s=t}const l=e[r].getTime(),d=e[s].getTime(),u=t[r],c=t[s];if(!isFinite(u)||!isFinite(c))return NaN;const p=d-l;return p<=0?c:u+(c-u)*(i-l)/p}function pvValueAtTime(e,t,i){const n=wattsAtFromChangeSeries(i?e._pvChangeSeriesPerEntity.get(i)??null:e._pvChangeSeries,t);if(null!==n)return{value:Math.max(0,n),unit:"W",isPredicted:!1};if(i)return{value:NaN,unit:"W",isPredicted:!1};const r=e._unifiedStore;if(r){const i=valueAt(r.forecast,r,t);if(null!==i&&i>0){const n=getHomeCoords(e.config,e.hass);return n&&getSunPosition(new Date(t),n.lat,n.lon).altitude<=0?{value:0,unit:"W",isPredicted:!0}:{value:Math.max(0,i),unit:"W",isPredicted:!0}}}return{value:NaN,unit:"W",isPredicted:!1}}function startOfDay(e){const t=new Date(e);return t.setHours(0,0,0,0),t}function addDays(e,t){const i=new Date(e);return i.setDate(i.getDate()+t),i}function addWeeks(e,t){return addDays(e,7*t)}function addMonths(e,t){const i=new Date(e);return i.setMonth(i.getMonth()+t),i}function startOfISOWeek(e){const t=startOfDay(e);return addDays(t,-(t.getDay()+6)%7)}function startOfMonth(e){const t=new Date(e);return t.setHours(0,0,0,0),t.setDate(1),t}function buildTimelineModel(e,t,i=7){const n=t.getTime()-e.getTime()||1,r=n/fe;let s,l,d,u,c;if(r<=2.05){s="intraday";const t=n/me,r=[1,2,3,4,6,12].find(e=>t/e<=i)??12,p=Math.ceil((e.getHours()+e.getMinutes()/60+.001)/r)*r;l=new Date(startOfDay(e).getTime()+p*me),d=e=>new Date(e.getTime()+r*me),u=null,c="boundary"}else r<=14.05?(s="days",l=addDays(startOfDay(e),1),d=e=>addDays(e,1),u=e=>startOfDay(e),c="centered"):r<=120.05?(s="weeks",l=startOfISOWeek(addWeeks(e,1)),d=e=>addWeeks(e,1),u=e=>startOfISOWeek(e),c="boundary"):(s="months",l=startOfMonth(addMonths(e,1)),d=e=>addMonths(e,1),u=e=>startOfMonth(e),c="centered");const p="days"===s?Math.max(i,16):i,thin=e=>{const t=Math.max(1,Math.ceil(e.length/p));return e.filter((e,i)=>i%t===0)},g=[];for(let b=l,y=0;b.getTime()<t.getTime()&&y<500;y++){const t=(b.getTime()-e.getTime())/n;t>0&&t<1&&g.push({frac:t,date:new Date(b)}),b=d(b)}let m,f=thin(g);if("boundary"===c)m=f;else{const i=[];let r=u(e);for(let s=0;r.getTime()<t.getTime()&&s<500;s++){const s=d(r),l=s.getTime()-r.getTime()||1;if(Math.min(s.getTime(),t.getTime())-Math.max(r.getTime(),e.getTime())>=.99*l){const t=((r.getTime()+s.getTime())/2-e.getTime())/n;i.push({frac:t,date:new Date(r)})}r=s}m=thin(i)}"months"===s&&(f=m.map(t=>({frac:(t.date.getTime()-e.getTime())/n,date:t.date})).filter(e=>e.frac>0&&e.frac<1));const v=[];if(r>1.05&&r<=40){let i=addDays(startOfDay(e),1);for(let r=0;i.getTime()<t.getTime()&&r<64;r++){const t=(i.getTime()-e.getTime())/n;t>0&&t<1&&v.push(t),i=addDays(i,1)}}return{kind:s,start:e,end:t,separators:f,labels:m,dayBoundaries:v}}function renderBottomChart(e){const t=e._chartTarget??"production";return"production"===t?function renderPvChart(e){const t=e,i=e._timeRange,n=1e3,r=100;if(!i)return B`<svg class="hc-chart-svg" viewBox="0 0 ${n} ${r}" preserveAspectRatio="none"></svg>`;const s=i.start.getTime(),l=i.end.getTime()-s;if(l<=0)return B`<svg class="hc-chart-svg" viewBox="0 0 ${n} ${r}" preserveAspectRatio="none"></svg>`;const d=ENERGY_COLOR_pv(t),u=e.hass?.themes?.darkMode?lerpHexToward(d,"#ffffff",.55):lerpHexToward(d,"#000000",.35),c=i.end.getTime(),p=buildTimelineModel(i.start,i.end).dayBoundaries.map(e=>e*n),g=e._unifiedStore,m=g?sliceForRange(g,s,c):null,xOf=e=>(e.getTime()-s)/l*n,f=[];if(m)for(let E=0;E<m.times.length;E++){const e=m.production[E];null!==e&&isFinite(e)&&f.push({t:m.times[E],v:e})}const v=[];if(m)for(let E=0;E<m.times.length;E++){const e=m.forecast[E];null===e||!isFinite(e)||e<=0||v.push({t:m.times[E],v:e})}let b=1;for(const E of f)E.v>b&&(b=E.v);for(const E of v)E.v>b&&(b=E.v);const yOf=e=>r-90*Math.max(0,Math.min(1,e/b)),y=f.map(e=>`${xOf(e.t).toFixed(2)},${yOf(e.v).toFixed(2)}`);let _="",w="";if(y.length>=2){const e=xOf(f[0].t),t=xOf(f[f.length-1].t);_=`M ${e},100 L ${y.join(" L ")} L ${t},100 Z`,w=`M ${y.join(" L ")}`}const H=e._pvChangeSeriesPerEntity.size>1?Array.from(e._pvChangeSeriesPerEntity.keys()):[],j=[];if(H.length>1&&f.length>=2){const t=e,i=chartIsDark(e),n=H.length,r=f.length,s=[];for(let d=0;d<n;d++){const t=H[d],i=new Array(r).fill(0);for(let n=0;n<r;n++){const r=pvValueAtTime(e,f[n].t.getTime(),t).value;i[n]=isFinite(r)&&r>0?r:0}s.push(i)}const l=new Array(r).fill(0);for(let e=0;e<n;e++){const d=[],u=[];for(let t=0;t<r;t++){let i=0;for(let e=0;e<n;e++)i+=s[e][t];const r=i>0?s[e][t]/i:0,c=l[t],p=c+r*f[t].v;l[t]=p,d.push(`${xOf(f[t].t).toFixed(2)},${yOf(p).toFixed(2)}`),u.push(`${xOf(f[t].t).toFixed(2)},${yOf(c).toFixed(2)}`)}j.push({color:energySolarColor(t,i,e),path:`M ${d.join(" L ")} L ${u.reverse().join(" L ")} Z`})}}let z="";v.length>=2&&(z=`M ${v.map(e=>`${xOf(e.t).toFixed(2)},${yOf(e.v).toFixed(2)}`).join(" L ")}`);const M=e._chartHoverPct;let C=0,$=NaN,D=NaN,R=!1;if(null!==M&&M>=0&&M<=100){C=M/100*n;const e=s+M/100*l,t=f.length>0?f[f.length-1].t.getTime():-1/0;if(f.length>=1&&e<=t){const t=interpAt(f.map(e=>e.t),f.map(e=>e.v),e);isFinite(t)&&($=yOf(Math.max(0,t)))}if(v.length>=1){const t=interpAt(v.map(e=>e.t),v.map(e=>e.v),e);isFinite(t)&&(D=yOf(Math.max(0,t)))}R=isFinite($)||isFinite(D)}const A=[];if(R&&null!==M&&j.length>0){const t=s+M/100*l,i=interpAt(f.map(e=>e.t),f.map(e=>e.v),t);if(isFinite(i)&&i>0){const n=H.map(i=>{const n=pvValueAtTime(e,t,i).value;return isFinite(n)&&n>0?n:0}),r=n.reduce((e,t)=>e+t,0);if(r>0){let t=0;for(let s=0;s<H.length;s++)t+=n[s]/r,A.push({y:yOf(t*i),color:energySolarColor(e,chartIsDark(e),s)})}}}return B`
        ...
        `:K}
        ${R&&isFinite(D)?B`
            <div class="hc-hover-dot-html" style="left: ${(C/n*100).toFixed(2)}%; top: ${(D/r*100).toFixed(2)}%; background: ${u};"></div>
        `:K}
        ${A.map(e=>B`
            <div class="hc-hover-dot-html" style="left: ${(C/n*100).toFixed(2)}%; top: ${(e.y/r*100).toFixed(2)}%; background: ${e.color};"></div>
        `)}
    `}(e):function renderTargetChart(e,t){const i=e,n=e._unifiedStore,r=e._timeRange,s=1e3,l=100;if(!n||!r)return B`<svg class="hc-chart-svg" viewBox="0 0 ${s} ${l}" preserveAspectRatio="none"></svg>`;const d=r.start.getTime(),u=r.end.getTime(),c=u-d;if(c<=0)return B`<svg class="hc-chart-svg" viewBox="0 0 ${s} ${l}" preserveAspectRatio="none"></svg>`;const xOf=e=>(e-d)/c*s,toPts=(e,t)=>{const i=[];for(let r=0;r<e.length;r++){const s=e[r];if(null===s||!isFinite(s))continue;const l=n.storeStartMs+(r+.5)*n.stepMs;l<d||l>u||i.push({t:l,v:t?t(s):s})}return i},sum=e=>e.reduce((e,t)=>e+t.v,0);let p,g=0,m=null;if("consumption"===t){const e=[];for(let t=0;t<n.production.length;t++){const i=n.production[t],r=n.gridImport[t],s=n.gridExport[t],l=n.battery[t];if(null===i&&null===r&&null===s&&null===l)continue;const c=n.storeStartMs+(t+.5)*n.stepMs;if(c<d||c>u)continue;const p=consumptionLoad(i??0,r??0,s??0,l??0);e.push({t:c,v:p})}p=[{pts:e,color:ENERGY_COLOR_consumption(i)}]}else if("grid"===t){const e=toPts(n.gridImport),t=toPts(n.gridExport);p=[{pts:e,color:ENERGY_COLOR_gridImport(i)},{pts:t,color:ENERGY_COLOR_gridExport(i)}]}else if("battery"===t){const t=toPts(n.battery,e=>Math.max(0,e)),r=toPts(n.battery,e=>Math.max(0,-e));p=[{pts:t,color:ENERGY_COLOR_batteryIn(i)},{pts:r,color:ENERGY_COLOR_batteryOut(i)}];let s=0;for(const e of p)for(const t of e.pts)t.v>s&&(s=t.v);const l=e._batterySocPerBankHistory.length>0?e._batterySocPerBankHistory:e._batterySocHistory?[e._batterySocHistory]:[];if(l.length>0){const e=s>0?s/100:1,t=ENERGY_COLOR_batteryIn(i),r=ENERGY_COLOR_batteryOut(i),c=cssHex(i,"--secondary-text-color","#9e9e9e"),flowColorAt=(e,i)=>{const s=Math.floor(((e+i)/2-n.storeStartMs)/n.stepMs),l=s>=0&&s<n.battery.length?n.battery[s]:null;return null===l||Math.abs(l)<5?c:l>0?t:r},f=[];for(const i of l){const t=[];for(let l=0;l<i.times.length;l++){const n=i.times[l].getTime();if(n<d||n>u)continue;const r=i.values[l];void 0!==r&&isFinite(r)&&t.push({t:n,v:Math.max(0,Math.min(100,r))*e})}if(t.length<2)continue;f.push(t);const n=t.length-1,r=[];for(let e=0;e<n;e++)r.push(flowColorAt(t[e].t,t[e+1].t));let s=0;for(let e=1;e<=n;e++)e!==n&&r[e]===r[s]||(p.push({pts:t.slice(s,e+1),color:r[s],lineOnly:!0,dashed:!0,noHoverDot:!0}),s=e)}f.length>0&&(m={banks:f,flowColorAt:flowColorAt}),g=s>0?s:100}}else if("battery-soc"===t){const t=e._batterySocHistory,n=[];if(t)for(let e=0;e<t.times.length;e++){const i=t.times[e].getTime();if(i<d||i>u)continue;const r=t.values[e];void 0!==r&&isFinite(r)&&n.push({t:i,v:r})}p=[{pts:n,color:ENERGY_COLOR_batteryOut(i)}],g=100}else isGroupTarget(t)?p=groupDevices(e.config,e._energyDefaults,groupOfTarget(t)).map(t=>({pts:toPts(changeSeriesToWatts(e._deviceChangeSeries.get(t.statConsumption)??null,n.storeStartMs,n.stepMs,n.bucketsTotal,Date.now()),e=>Math.abs(e)),color:deviceColorByIndex(i,t.index)})):(p=[{pts:toPts(n.irradiance),color:ENERGY_COLOR_sun(i)}],g=1e3);let f=g;if(f<=0){f=1;for(const e of p)for(const t of e.pts)t.v>f&&(f=t.v)}const v=10,yOf=e=>l-Math.max(0,Math.min(1,e/f))*(l-v),b=p.map(e=>{if(e.pts.length<2)return{area:"",line:"",color:e.color,dashed:!!e.dashed,total:sum(e.pts)};const t=e.pts.map(e=>`${xOf(e.t).toFixed(2)},${yOf(e.v).toFixed(2)}`),i=xOf(e.pts[0].t),n=xOf(e.pts[e.pts.length-1].t);return{area:e.lineOnly?"":`M ${i},${l} L ${t.join(" L ")} L ${n},${l} Z`,line:`M ${t.join(" L ")}`,color:e.color,dashed:!!e.dashed,total:sum(e.pts)}}),y=[];let _=null,w=null;if("irradiance"===t&&e._chartSeries){const t=e._chartSeries,n=[];for(let e=0;e<t.times.length;e++){const i=t.times[e].getTime();if(i<d||i>u)continue;const r=t.cloudLow[e],s=t.cloudMid[e],l=t.cloudHigh[e];(isFinite(r)||isFinite(s)||isFinite(l))&&n.push({t:i,lo:isFinite(r)?Math.max(0,r):0,mi:isFinite(s)?Math.max(0,s):0,hi:isFinite(l)?Math.max(0,l):0})}if(n.length>=2){const yOfPct=e=>l-Math.max(0,Math.min(1,e/100))*(l-v),e=[{pick:e=>e.lo,color:lerpHexToward(ENERGY_COLOR_cloud(i),"#ffffff",.55)},{pick:e=>e.mi,color:ENERGY_COLOR_cloud(i)},{pick:e=>e.hi,color:lerpHexToward(ENERGY_COLOR_cloud(i),"#000000",.5)}],t=new Array(n.length).fill(0);for(const i of e){const e=[],r=[];for(let s=0;s<n.length;s++){const l=t[s],d=l+i.pick(n[s]);t[s]=d,e.push(`${xOf(n[s].t).toFixed(2)},${yOfPct(d).toFixed(2)}`),r.push(`${xOf(n[s].t).toFixed(2)},${yOfPct(l).toFixed(2)}`)}y.push({area:`M ${e.join(" L ")} L ${r.reverse().join(" L ")} Z`,color:i.color})}_={bands:n,yOfPct:yOfPct,colors:e.map(e=>e.color)}}}let H="";if("irradiance"===t&&n){const e=sliceForRange(n,d,u),t=[];let i=0;for(let n=0;n<e.times.length;n++){const r=e.forecast[n];null===r||!isFinite(r)||r<=0||(t.push({t:e.times[n],v:r}),r>i&&(i=r))}if(t.length>=2&&i>0){const yOfF=e=>l-Math.max(0,Math.min(1,e/i))*(l-v);H=`M ${t.map(e=>`${xOf(e.t.getTime()).toFixed(2)},${yOfF(e.v).toFixed(2)}`).join(" L ")}`,w={pts:t.map(e=>({t:e.t.getTime(),v:e.v})),yOf:yOfF}}}const j=e.hass?.themes?.darkMode?lerpHexToward(ENERGY_COLOR_pv(i),"#ffffff",.75):lerpHexToward(ENERGY_COLOR_pv(i),"#000000",.55),z=buildTimelineModel(r.start,r.end).dayBoundaries.map(e=>e*s),M=e._chartHoverPct;let C=0,$=!1;const D=[];if(null!==M&&M>=0&&M<=100){C=M/100*s;const e=d+M/100*c;for(const t of p){if(t.pts.length<1||t.noHoverDot)continue;const i=interpAt(t.pts.map(e=>new Date(e.t)),t.pts.map(e=>e.v),e);isFinite(i)&&(D.push({y:yOf(Math.max(0,i)),color:t.color}),$=!0)}if(m)for(const t of m.banks){if(t.length<1)continue;const i=interpAt(t.map(e=>new Date(e.t)),t.map(e=>e.v),e);isFinite(i)&&(D.push({y:yOf(Math.max(0,i)),color:m.flowColorAt(e,e)}),$=!0)}if(w&&w.pts.length>=1){const t=interpAt(w.pts.map(e=>new Date(e.t)),w.pts.map(e=>e.v),e);isFinite(t)&&t>0&&(D.push({y:w.yOf(t),color:j}),$=!0)}if(_&&_.bands.length>=1){const t=_.bands.map(e=>new Date(e.t)),i=interpAt(t,_.bands.map(e=>e.lo),e),n=interpAt(t,_.bands.map(e=>e.mi),e),r=interpAt(t,_.bands.map(e=>e.hi),e),s=[isFinite(i)?i:0,(isFinite(i)?i:0)+(isFinite(n)?n:0),(isFinite(i)?i:0)+(isFinite(n)?n:0)+(isFinite(r)?r:0)],l=[isFinite(i),isFinite(n),isFinite(r)];for(let e=0;e<3;e++)l[e]&&(D.push({y:_.yOfPct(s[e]),color:_.colors[e]}),$=!0)}}return B`
    ...
    `}function renderTimelineHoverTooltip(e){const t=e._timeRange,i=e._chartSeries;if(!t)return K;const n=t.start.getTime(),r=t.end.getTime()-n;if(r<=0)return K;const s=e._chartHoverPct;if(null===s||s<0||s>100)return K;const l=s,d=n+l/100*r,u=i?interpAt(i.times,i.irradiance,d):NaN,c=i?interpAt(i.times,i.cloudLow,d):NaN,p=i?interpAt(i.times,i.cloudMid,d):NaN,g=i?interpAt(i.times,i.cloudHigh,d):NaN,m=pvValueAtTime(e,d),f=e._chartTarget??"production",v=e._unifiedStore,b=v?valueAt(v.gridImport,v,d)??NaN:NaN,y=v?valueAt(v.gridExport,v,d)??NaN:NaN,_=v?valueAt(v.battery,v,d)??NaN:NaN,w=v?valueAt(v.production,v,d)??NaN:NaN,H=isFinite(w)||isFinite(b)||isFinite(y)||isFinite(_),j=consumptionLoad(isFinite(w)?w:0,isFinite(b)?b:0,isFinite(y)?y:0,isFinite(_)?_:0),z=e._batterySocHistory?interpAt(e._batterySocHistory.times,e._batterySocHistory.values,d):NaN,M=valueDecimals(e.config),C=powerUnit(e.config),$=irradianceUnit(e.config),kw=t=>formatPower(e.hass,t,M,C),D=e._pvChangeSeriesPerEntity,R=D.size>1?Array.from(D.keys()):[],A=[];for(let L=0;L<R.length;L++){const t=R[L],i=pvValueAtTime(e,d,t);if(!isFinite(i.value))continue;const n=formatPower(e.hass,pvNormalizeToWatts(i.value,i.unit),M,C);A.push({id:t,label:solarSourceName(e,L),valueText:n,colorIdx:L})}const E=isFinite(m.value),T=targetLabel(e,f),O=pickTranslations(e.hass?.language).cloudCover,P=function gridImportName(e){return e._energyDefaults.gridName||statFriendly(e,e._energyDefaults.gridStatEnergyFroms)}(e),F=function gridExportName(e){return e._energyDefaults.gridName||statFriendly(e,e._energyDefaults.gridStatEnergyTos)}(e),I=function batteryChargeName(e){return e._energyDefaults.batteryName||statFriendly(e,e._energyDefaults.batteryStatEnergyTos)}(e),U=function batteryDischargeName(e){return e._energyDefaults.batteryName||statFriendly(e,e._energyDefaults.batteryStatEnergyFroms)}(e),W=e,G=isGroupTarget(f)?groupDevices(e.config,e._energyDefaults,groupOfTarget(f)).map(t=>{const i=wattsAtFromChangeSeries(e._deviceChangeSeries.get(t.statConsumption)??null,d);return{w:null===i?NaN:Math.abs(i),color:deviceColorByIndex(W,t.index),name:deviceName(e.hass,t),icon:deviceIcon(e.hass,t)}}):[],q=ENERGY_COLOR_cloud(W),Z=lerpHexToward(q,"#ffffff",.55),Y=lerpHexToward(q,"#000000",.5),J=(e._batterySocPerBankHistory.length>0?e._batterySocPerBankHistory:e._batterySocHistory?[e._batterySocHistory]:[]).map(e=>interpAt(e.times,e.values,d)),X=targetLabel(e,"battery-soc"),Q=!isFinite(_)||Math.abs(_)<5?cssHex(W,"--secondary-text-color","#9e9e9e"):_>0?ENERGY_COLOR_batteryIn(W):ENERGY_COLOR_batteryOut(W),ee=new Date(d),te=e.hass?.language||void 0,ie=r/fe,ae=ie<=2.05?{hour:"2-digit",minute:"2-digit"}:ie<=14.05?{weekday:"short",hour:"2-digit",minute:"2-digit"}:{weekday:"short",day:"numeric",month:"short"},ne=new Intl.DateTimeFormat(te,ae).format(ee),oe=new Date(ee);oe.setHours(0,0,0,0);const re=/* @__PURE__ */new Date;re.setHours(0,0,0,0);const se=oe.getTime()===re.getTime(),le=d>Date.now();let de=function computeDailyKwhTotals(e){const t=/* @__PURE__ */new Map;if(!e._timeRange)return t;const{start:i,end:n}=e._timeRange,r=i.getTime(),s=n.getTime(),dayKey=e=>{const t=new Date(e);return t.setHours(0,0,0,0),t.getTime()},l=e._pvChangeSeries;if(l&&l.length>0){const e=new Date(r);for(e.setHours(0,0,0,0);e.getTime()<s;){const i=e.getTime(),n=new Date(e);n.setDate(n.getDate()+1);const r=sumChangeForDay(l,i,n.getTime());null!==r&&t.set(i,Math.max(0,r)),e.setTime(n.getTime())}}const d=e._unifiedStore;if(d){const e=Date.now(),i=d.stepMs/me;for(let n=0;n<d.bucketsTotal;n++){const l=d.storeStartMs+(n+.5)*d.stepMs;if(l<r||l>s)continue;if(l<e)continue;const u=d.forecast[n];if(null===u||!isFinite(u)||u<=0)continue;const c=dayKey(l);t.set(c,(t.get(c)??0)+u*i/1e3)}}return t}(e).get(oe.getTime());se&&!le&&"number"==typeof e._haSolarTodayKwh&&isFinite(e._haSolarTodayKwh)&&(de=e._haSolarTodayKwh);const ue=le&&void 0!==de&&isFinite(de)&&de>=.05,ce=void 0!==de&&isFinite(de)&&de>=.05?formatEnergyKwh(e.hass,de,M,C):"",he=Date.now(),pe=he>=n&&he<=n+r&&Math.abs(l-(he-n)/r*100)<=1.2,ge=(e.hass?.language||"").toLowerCase().startsWith("fr")?"Retour au live":"Back to live";return B`
        <div
        ...
        >
            <div class="tb-hover-tooltip">
                ...
                ${"production"===f?B`
                    ${ue&&ce?B`
                        <div class="tb-hover-tooltip-row">
                            ...
                            <span class="tb-hover-tooltip-value">${ce}</span>
                        </div>
                    `:K}
                    ${E?B`
                        <div class="tb-hover-tooltip-row">
                ...
                ${"irradiance"===f?B`
                    ...
                    ${isFinite(p)?B`
                        <div class="tb-hover-tooltip-row">
                            <ha-icon class="tb-hover-tooltip-icon" style="color:${q}" icon="mdi:format-vertical-align-center"></ha-icon>
        ...
        `}render(){const e=null!==getHomeCoords(this.host.config,this.host.hass),t=!1!==this.host._interactive,i=this.host._labelLayout,n=resolvePvLiveEntity(this.host._energyDefaults),r=ENERGY_COLOR_pv(this.host),s=!this.host._isLiveMode&&null!==this.host._selectedTime,l=s&&this.host._selectedTime.getTime()>Date.now()+6e4,d=""!==n&&null!==i?s?function pvRateAtTime(e,t){const i=wattsAtFromChangeSeries(e._pvChangeSeries,t.getTime());return null===i?null:{value:Math.max(0,i),unit:"W"}}(this.host,this.host._selectedTime):null!==this.host._pvCurrent?currentPvRate(this.host):null:null;let u=null;if(l&&""!==n&&null!==i&&this.host._unifiedStore){const e=valueAt(this.host._unifiedStore.forecast,this.host._unifiedStore,this.host._selectedTime.getTime());null!==e&&e>0&&(u={value:e,unit:"W"})}const c=l&&null!==u,p=c?u:d,g=e&&null!==i&&""!==n&&null!==p&&(!l||c)&&(!s||p.value>0),m=valueDecimals(this.host.config),f=powerUnit(this.host.config),v=irradianceUnit(this.host.config),b=g?(c?"~ ":"")+formatPvValue(this.host.hass,p.value,p.unit,m,f):"",y=null!==d?pvNormalizeToWatts(d.value,d.unit):0,_=flowDuration(y,5e3,.5),w=!(y>0),H=resolveBatteryEntities(this.host._energyDefaults),j=null!==H.socEntity,z=null!==H.powerEntity,M=!this.host._isLiveMode&&null!==this.host._selectedTime,C=M&&this.host._selectedTime.getTime()>Date.now()+6e4,$=M&&!C?this.host._selectedTime.getTime():null,D=null!==$?wattsAtFromChangeSeries(this.host._gridImportChangeSeries,$):this.host._gridImportValue,R=null!==$?wattsAtFromChangeSeries(this.host._gridExportChangeSeries,$):this.host._gridExportValue,A=null===D?null:Math.max(0,D),E=null===R?null:Math.max(0,R),T=null!==$?"W":this.host._gridImportUnit,O=null!==$?"W":this.host._gridExportUnit,L=M?function batterySampleAtTime(e,t){if(!e||0===e.times.length)return null;const i=t.getTime(),n=e.times[0].getTime(),r=e.times[e.times.length-1].getTime();if(i<n||i>r+6e4)return null;let s=e.times.length-1;for(let l=0;l<e.times.length;l++)if(e.times[l].getTime()>i){s=l-1;break}return s<0&&(s=0),e.values[s]}(this.host._batterySocHistory,this.host._selectedTime):this.host._batterySoc;let P;if(M){const e=this.host._selectedTime.getTime(),t=wattsAtFromChangeSeries(this.host._batteryChargeChangeSeries,e),i=wattsAtFromChangeSeries(this.host._batteryDischargeChangeSeries,e);P=null===t&&null===i?null:Math.max(0,t??0)-Math.max(0,i??0)}else P=this.host._batteryPower;const F=M?"W":this.host._batteryPowerUnit,I=e&&null!==i&&!C&&j&&null!==L,U=e&&null!==i&&!C&&z&&null!==P,W=I||U,q=I?`${Math.round(L)} %`:"",Z=U?function formatBatteryPower(e,t,i,n,r="kW",s="default"){const l=pvNormalizeToWatts(t,i);return"hidden"===s?formatPowerKw(e,Math.abs(l),n,!1,r):formatPowerKw(e,"inverted"===s?-l:l,n,!0,r)}(this.host.hass,-P,F,m,f,function batterySign(e){const t=e?.["battery-sign"];return"inverted"===t||"hidden"===t?t:"default"}(this.host.config)):"",Y=l||null===p?null:pvNormalizeToWatts(p.value,p.unit),J=null!==A||null!==E?(A??0)-(E??0):null,X=U?P:null,Q=this.host._energyDefaults,ee=M||(0===Q.solarStatEnergyFroms.length||null!==Y)&&(0===Q.gridStatEnergyFroms.length&&0===Q.gridStatEnergyTos.length||null!==J)&&(0===Q.batteryStatEnergyFroms.length&&0===Q.batteryStatEnergyTos.length||null!==X)?null===Y&&null===J&&null===X?null:Math.max(0,(Y??0)+(J??0)-(X??0)):null,te=e&&null!==i&&!C&&null!==ee,ie=te?formatGridValue(this.host.hass,ee,"W",m,f):"",ae=U&&P>0,ne=U&&P<0,oe=ae?"var(--energy-battery-in-color, #f06292)":"var(--energy-battery-out-color, #4db6ac)";this._batteryLeaderColor=oe;const re=U?Math.abs(pvNormalizeToWatts(P,F)):0,se=U&&re<5,le=flowDuration(re,5e3),de=i?.batteryLabel.x??0,ue=i?.batteryLabel.y??0,ce=function batteryLevelIcon(e,t){if(null===e||!isFinite(e))return"mdi:battery";const i=Math.max(0,Math.min(100,10*Math.round(e/10)));return t?i>=100?"mdi:battery-charging-100":i<=0?"mdi:battery-charging-outline":`mdi:battery-charging-${i}`:i>=100?"mdi:battery":i<=0?"mdi:battery-outline":`mdi:battery-${i}`}(I?L:null,ae),he=U?Z:q,pe=ue-4.2,ge=!!(i&&ae&&g)?ue+4.2:ue,me=i&&W&&ne?this._buildLPathToHome(i,de,ge,22):"",fe=i&&W&&!ne?this._buildLPathToHome(i,de,ge,22):"",ve=i&&ae&&g?this._buildLPath(i.pvLabel.x+14,i.pvLabel.y+11,de-30,pe,!0):"",be=this._buildLPathToHome(i,i?.gridLabel.x??0,i?.gridLabel.y??0,22),ye=[.32,.44,.6799999999999999,.56],ke=this.host._isLiveMode||null===this.host._selectedTime?null:this.host._selectedTime.getTime(),_e=i?function activeGroups(e,t){const i=[];for(let n=1;n<=4;n++)groupDevices(e,t,n).length>0&&i.push(n);return i}(this.host.config,this.host._energyDefaults).map(e=>{const t=i.groupLabels[e-1],n=e<=2?t.x+48:t.x-48,r=i.home.x+96*(ye[e-1]-.5),s=i.home.y+14,l=this._buildLPath(r,s,n,t.y,!0),d=null!==ke?function groupPowerWAt(e,t,i){let n=0,r=!1;for(const s of groupDevices(e.config,e._energyDefaults,t)){const t=wattsAtFromChangeSeries(e._deviceChangeSeries.get(s.statConsumption)??null,i);null!==t&&(n+=t,r=!0)}return r?n:null}(this.host,e,ke):function groupLivePowerW(e,t){const i=groupDevices(e.config,e._energyDefaults,t).map(e=>e.statRate).filter(e=>""!==e);if(0===i.length)return null;const{watts:n,any:r}=sumLiveWatts(e.hass,i);return r?n:null}(this.host,e),u=monitoringGroupColor(this.host.config,e),c=monitoringGroupIcon(this.host.config,e),p=null===d?0:Math.abs(d);return{g:e,anchor:t,leadPath:l,watts:d,color:u,icon:c,beadDur:p<5?null:flowDuration(p,5e3)}}):[],we=null!==A?Math.abs(pvNormalizeToWatts(A,T)):0,Se=null!==E?Math.abs(pvNormalizeToWatts(E,O)):0,proportionalBeadDur=(e,t)=>{const i=Math.max(e,1);return Math.min(8,Math.max(1.2,1.2*t/i))},He=we<5?null:proportionalBeadDur(we,5e3),je=Se<5?null:proportionalBeadDur(Se,1e3),ze=(A??0)>=(E??0),Me=ze?"var(--energy-grid-consumption-color, #488fc2)":"var(--energy-grid-return-color, #8353d1)";this._gridLeaderColor=Me;const xe=ze?He:je,Ce=this.host._sunScene,$e=e&&null!==Ce&&Ce.arc.length>=2,De=ENERGY_COLOR_sun(this.host),Re=function darkenHex(e,t){const i=1-Math.max(0,Math.min(1,t)),n=Math.round(parseInt(e.slice(1,3),16)*i),r=Math.round(parseInt(e.slice(3,5),16)*i),s=Math.round(parseInt(e.slice(5,7),16)*i),h=e=>e.toString(16).padStart(2,"0");return`#${h(n)}${h(r)}${h(s)}`}(De,.2),Ae=$e?function buildArcSegments(e,t){const i=[];for(let n=0;n<e.length-1;n++){const r=e[n],s=e[n+1];i.push({x1:r.x,y1:r.y,x2:s.x,y2:s.y,color:arcColor(.5*(r.altitude+s.altitude),t),nearness:.5*(r.nearness+s.nearness),belowHorizon:r.belowHorizon||s.belowHorizon})}return i}(Ce.arc,De):[],Ee=this.host._arcBackBuf,Te=this.host._arcFrontBuf,Oe=this.host._arcFrontNearBuf;Ee.length=0,Te.length=0,Oe.length=0;for(const B of Ae)B.belowHorizon?Ee.push(B):B.nearness>=.5?Oe.push(B):Te.push(B);const Le=$e&&Ce.sun.altitude>0,Pe=Ce?.sun.irradiance??0,Fe=formatIrradiance(this.host.hass,Pe,m,v),Ie=Math.sqrt(Math.max(0,Math.min(1,Pe/1e3))),Ue=$e&&Ce.sun.altitude>0&&this.host._weatherAvailable,We=flowDuration(Pe,1e3,.8);let Ne=Ce?.home.x??0,Be=Ce?.home.y??0;if(i&&Ce&&g){const e=i.pvLabel.x,t=i.pvLabel.y,n=28,r=11,s=Ce.sun.x-e,l=Ce.sun.y-t,d=Math.max(0,n-r);if(Math.abs(s)<=d)Ne=Ce.sun.x,Be=t+(l>=0?1:-1)*r;else{const i=e+(s>=0?1:-1)*d,n=t,l=Ce.sun.x-i,u=Ce.sun.y-n,c=Math.sqrt(l*l+u*u)||1;Ne=i+r*l/c,Be=n+r*u/c}}return B`
                ...
                ${$e&&Ee.length>0?B`
                    <svg
                        ...
                        style="--solar-daylight:${Ce.daylight}"
                    >
                        ${Ee.map(e=>G`
                            <line
                                ...
                                x1="${e.x1}" y1="${e.y1}"
                                x2="${e.x2}" y2="${e.y2}"
                                stroke-width="${.5*(Mt+3.5*e.nearness)}"
                            ></line>
                        `)}
                        ${Ee.map(e=>G`
                            <line
                ...
                ${g?B`
                    <div
                        class="pv-pct-label ${c?"is-predicted":""} ${t&&"production"===this.host._chartTarget?"is-chart-active":""}"
                ...
                ${W?B`
                    ...
                    <div
                        ...
                        style="left:${de}px; top:${ue}px; --battery-leader-color:${oe}"
                        role=${t?"button":K}
                        tabindex=${t?"0":K}
                        data-target="battery"
                        @click=${t?this.host.onChartTargetClick:void 0}
                ...
                ${!e||null===i||null===A&&null===E||C?K:B`
                    <svg class="grid-leader-svg">
                        <path class="grid-leader-line" style="stroke:${Me}" d=${be} />
                    ...
                    <div
                        ...
                        style="left:${i.gridLabel.x}px; top:${i.gridLabel.y}px; --grid-leader-color:${Me}"
                        role=${t?"button":K}
                        tabindex=${t?"0":K}
                        data-target="grid"
                        @click=${t?this.host.onChartTargetClick:void 0}
                    >
                        <ha-icon icon=${ze?"mdi:transmission-tower-export":"mdi:transmission-tower-import"}></ha-icon>
                        <span>${formatGridValue(this.host.hass,ze?A??0:E??0,ze?T:O,m,f)}</span>
                    </div>
                `}
                    ...
                    >
                        ${Te.map(e=>G`
                            <line
                                ...
                                x1="${e.x1}" y1="${e.y1}"
                                x2="${e.x2}" y2="${e.y2}"
                                stroke-width="${Mt+3.5*e.nearness}"
                            ></line>
                        `)}
                        ${Te.map(e=>G`
                            <line
                    ...
                    <svg class="solar-svg solar-ray-svg"
                        ...
                        >
                            <animateMotion
                                dur="${We}s"
                                repeatCount="indefinite"
                                path="M ${Ce.sun.x},${Ce.sun.y} L ${Ne},${Be}"
                            ></animateMotion>
                        </circle>
                    </svg>
                `:K}
                ...
                ${$e?B`
                    <svg
                        class="solar-svg solar-svg-sun ${Ce.sun.nearness>=.5?"solar-svg-sun-near":"solar-svg-sun-far"}"
                        style="--solar-daylight:${Ce.daylight}"
                    >
                        ${(()=>{const e=this.host._engine?.getSunArcScale()??1,t=Math.min((10+10*Ce.sun.nearness)*e,22),i=t*Ie,n=3*t,r=.55*Ie;return G`
                                <defs>
                ...
                ${Ue?B`
                    <div
                        ...
                        style="left:${Ce.sun.x}px; top:${Ce.sun.y-22}px"
                        role=${t?"button":K}
                        tabindex=${t?"0":K}
                ...
                ${$e&&Ce?B`
                    ${this._renderSunCrossing(Ce.sunrise,Ce.home,"mdi:weather-sunset-up",De)}
                    ${this._renderSunCrossing(Ce.sunset,Ce.home,"mdi:weather-sunset-down",De)}
                `:K}
...
`,At=($t=class HeliosCardEditor extends le{constructor(...e){super(...e),this._cfg={},this._pickerReady=!1,this._openSection=null,this._sliderDebounce=/* @__PURE__ */new Map,this._energyDefaults=Xe,this._energyDefaultsLoaded=!1,this._gridGuard={status:"unknown",cleanEvals:0,fetchKey:"",fetching:!1,entityKey:""},this._onSectionToggleEvt=e=>{const t=e.currentTarget.dataset.section;t&&this._onSectionToggle(t,e)},this._onNumFieldChange=e=>{const t=e.currentTarget.dataset.key;t&&this._numField(t,e)},this._onNumSliderInput=e=>{const t=e.currentTarget.dataset.key;t&&this._numSlider(t,e)},this._onEntityValueChanged=e=>{const t=e.currentTarget.dataset.key;if(!t)return;const i=e.detail.value,n=null==i||""===i?void 0:i;(this._cfg[t]??void 0)!==(n??void 0)&&this._update(t,n)},this._onBoolToggleClick=e=>{const t=e.currentTarget,i=t.dataset.key;i&&this._update(i,"true"===t.dataset.value)},this._onDeviceToggleClick=e=>{const t=e.currentTarget.dataset.stat;if(!t)return;const i=this._cfg["hidden-devices"],n=Array.isArray(i)?i.filter(e=>"string"==typeof e):[],r=n.includes(t)?n.filter(e=>e!==t):[...n,t];this._update("hidden-devices",r.length?r:void 0)},this._onDeviceGroupClick=e=>{const t=e.currentTarget.dataset.stat;if(!t)return;const i=monitoringGroups(this._cfg),n=(i.get(t)??0)+1,r=n>4?0:n,s={};for(const[l,d]of i)l!==t&&(s[l]=d);r>=1&&(s[t]=r),this._update("monitoring-groups",Object.keys(s).length?s:void 0)},this._solarIrradianceEntityFilter=e=>{if(!e||!e.attributes)return!1;if("irradiance"===e.attributes.device_class)return!0;const t=String(e.attributes.unit_of_measurement??"").trim();return"W/m²"===t||"W/m2"===t},this._onGroupNameChanged=e=>{const t=e.currentTarget;t.dataset.group&&this._updateGroupMap("monitoring-group-names",t.dataset.group,t.value.trim())},this._onGroupColorChanged=e=>{e.stopPropagation();const t=e.currentTarget.dataset.group;t&&this._updateGroupMap("monitoring-group-colors",t,"string"==typeof e.detail.value?e.detail.value:"")},this._onGroupIconChanged=e=>{e.stopPropagation();const t=e.currentTarget.dataset.group;t&&this._updateGroupMap("monitoring-group-icons",t,"string"==typeof e.detail.value?e.detail.value:"")},this._resetFeedback=null}disconnectedCallback(){super.disconnectedCallback(),unsubscribeEnergyPrefs(this);for(const e of this._sliderDebounce.values())window.clearTimeout(e);this._sliderDebounce.clear(),void 0!==this._resetFeedbackTimer&&(window.clearTimeout(this._resetFeedbackTimer),this._resetFeedbackTimer=void 0)}setConfig(e){if(this._cfg={...e},Dt.LEGACY_KEYS.some(e=>e in this._cfg)&&setTimeout(()=>{if(!this.isConnected)return;const e={...this._cfg};let t=!1;for(const i of Dt.LEGACY_KEYS)i in e&&(delete e[i],t=!0);t&&(this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:e}})),this._cfg=e)},0),!this._cfg["cache-id"]){const e=`c${Date.now().toString(36)}${Math.floor(1e9*Math.random()).toString(36)}`;setTimeout(()=>{this.isConnected&&(this._cfg["cache-id"]||this._update("cache-id",e))},0)}}connectedCallback(){super.connectedCallback(),this._ensureEntityPicker(),subscribeEnergyPrefs(this)}async _ensureEntityPicker(){if(!this._pickerReady)if("undefined"!=typeof customElements&&customElements.get("ha-entity-picker"))this._pickerReady=!0;else try{const e=window;if("function"==typeof e.loadCardHelpers){const t=await e.loadCardHelpers();if(t?.createCardElement){const e=(await t.createCardElement({type:"entities",entities:[]}))?.constructor;"function"==typeof e?.getConfigElement&&await e.getConfigElement()}}"undefined"!=typeof customElements&&await Promise.race([customElements.whenDefined("ha-entity-picker"),new Promise(e=>{setTimeout(e,Dt.PICKER_LOAD_TIMEOUT_MS)})])}catch(e){}finally{this._pickerReady=!0}}_t(){return pickTranslations(this.hass?.language)}updated(){this.hass&&refreshGridGuard(this),this._pruneStaleDeviceIds()}_pruneStaleDeviceIds(){if(!this._energyDefaultsLoaded)return;const e=this._energyDefaults.devices;if(0===e.length)return;const t=new Set(e.map(e=>e.statConsumption)),i=["hidden-devices"],n={...this._cfg};let r=!1;for(const l of i){const e=this._cfg[l];if(!Array.isArray(e))continue;const i=e.filter(e=>"string"==typeof e&&t.has(e));i.length!==e.length&&(r=!0,i.length?n[l]=i:delete n[l])}const s=this._cfg["monitoring-groups"];if(s&&"object"==typeof s&&!Array.isArray(s)){const e={};let i=!1;for(const[n,r]of Object.entries(s))t.has(n)&&"number"==typeof r?e[n]=r:i=!0;i&&(r=!0,Object.keys(e).length?n["monitoring-groups"]=e:delete n["monitoring-groups"])}r&&(this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:n}})),this._cfg=n)}_liveStatusLine(e,t,i){return B`
        ...
        `}_renderLiveDataStatus(e){if(!this._energyDefaultsLoaded)return K;const t=this._energyDefaults,i=t.solarStatEnergyFroms.length>0,n=t.gridStatEnergyFroms.length>0||t.gridStatEnergyTos.length>0,r=t.batteryStatEnergyFroms.length>0||t.batteryStatEnergyTos.length>0,s="flagged"===this._gridGuard.status,l=t.solarStatRates.length>0,d=t.gridStatRates.length>0&&!s,u=!batteryLiveIsBucketSourced(t),c=(i||n||r)&&(!i||l)&&(!n||d)&&(!r||u);return B`
        ...
        `}_update(e,t){const i={...this._cfg};void 0===t?delete i[e]:i[e]=t,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:i}})),this._cfg=i}_numField(e,t){const i=t.target.value.trim();if(""===i)return void this._update(e,void 0);const n=parseFloat(i);isFinite(n)&&this._update(e,n)}_numSlider(e,t){const i=parseFloat(t.target.value);if(!isFinite(i))return;this._cfg={...this._cfg,[e]:i};const n=String(e),r=this._sliderDebounce.get(n);void 0!==r&&window.clearTimeout(r);const s=window.setTimeout(()=>{this._sliderDebounce.delete(n),this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:this._cfg}}))},Dt.SLIDER_COMMIT_DELAY_MS);this._sliderDebounce.set(n,s)}_onSectionToggle(e,t){const i=t.currentTarget;i.open?(this._openSection=e,requestAnimationFrame(()=>{i.scrollIntoView({behavior:"smooth",block:"start"})})):this._openSection===e&&(this._openSection=null)}_fmtNum(e,t){return t>=1?String(Math.round(e)):e.toFixed(2)}_renderToggle(e,t,i,n,r,s=!1){const l=this._cfg,d=s?!1!==l[e]:!0===l[e],u=this._t();return B`
                <div class="field">
                    ...
                    <div class="segmented-toggle">
                        ...
                        >${n??u.editor.autoRotateOn}</button>
                        <button
                            type="button"
                            class="seg-option ${d?"":"active"}"
                            data-key=${e} data-value="false"
                            @click=${this._onBoolToggleClick}
                        >${r??u.editor.autoRotateOff}</button>
                    </div>
                </div>
        ...
        `}_renderGroupsSection(e){const t=Array.from({length:4},(e,t)=>t+1);return B`
            ...
            `)}
        `}_updateGroupMap(e,t,i){const n=this._cfg[e],r=n&&"object"==typeof n&&!Array.isArray(n)?{...n}:{};i?r[t]=i:delete r[t],this._update(e,Object.keys(r).length?r:void 0)}_renderDeviceRow(e,t,i,n){const r=e.statConsumption,s=this._deviceName(e),l=deviceColorByIndex(this,e.index),d=!t.has(r),u=i.get(r)??0;return B`
            <div class="device-row ${d?"":"is-hidden"}">
                ...
                <button
                    type="button"
                    class="device-group ${u?"active":""}"
                    style=${u?`--group-pill-color:${monitoringGroupColor(this._cfg,u)}`:""}
                    data-stat=${r}
                    aria-label=${n.editor.deviceGroupLabel??"Monitoring group"}
                    title=${u?`${n.editor.group??"Group"} ${u}`:n.editor.noGroup??"No group"}
                    @click=${this._onDeviceGroupClick}
                >
                    ${u?(()=>{const e=monitoringGroupIcon(this._cfg,u);return e?B`<ha-icon icon=${e}></ha-icon>`:B`<span class="device-group-num">${u}</span>`})():B`<ha-icon icon="mdi:close"></ha-icon>`}
                </button>
                <button
                    type="button"
    ...
    `:K}(this):K}
        ...
        `}_maybeRebuildUnifiedStore(){(function isStoreFresh(e,t){return!!t&&t.dataVersion===computeDataVersion(e)})(this,this._unifiedStore)||(this._unifiedStore=buildUnifiedStore(this))}_isCameraLocked(){return!!this._engine&&this._engine.isCameraLocked()}_registerCacheId(){const e=cacheId(this.config);if(!e)return;const t=Ot.get(e)??[];t.includes(this)||(t.push(this),Ot.set(e,t))}_unregisterCacheId(){const e=cacheId(this.config),t=e?Ot.get(e):void 0;if(!t)return;const i=t.indexOf(this);i>=0&&t.splice(i,1),0===t.length&&Ot.delete(e)}effectiveCacheId(){const e=cacheId(this.config);if(!e)return"";const t=Ot.get(e),i=t?t.indexOf(this):-1;return i>0?`${e}#${i+1}`:e}_uiStateStorageKey(){const e=this.effectiveCacheId();if(e)return`helios:ui-state:${e}`;const t=getHomeCoords(this.config,this.hass);return t?`helios:ui-state:${Math.round(1e3*t.lat)/1e3}:${Math.round(1e3*t.lon)/1e3}`:null}_restoreUiState(){if(this._uiStateRestored)return;const e=this._uiStateStorageKey();if(e){this._uiStateRestored=!0;try{const t=window.localStorage.getItem(e);if(!t)return;const i=JSON.parse(t);if(i&&"object"==typeof i){const e=["production","consumption","grid","battery","battery-soc","irradiance",..._t];"string"==typeof i.chartTarget&&e.includes(i.chartTarget)&&(this._chartTarget=i.chartTarget),"string"==typeof i.timelineMode&&i.timelineMode in Be&&(this._timelineMode=i.timelineMode,this._periodPastDays=modePastDays(this._timelineMode),this._periodFutureDays=modeFutureDays(this._timelineMode))}}catch(L){}}}persistUiState(){const e=this._uiStateStorageKey();if(e)try{window.localStorage.setItem(e,JSON.stringify({chartTarget:this._chartTarget,timelineMode:this._timelineMode}))}catch(L){}}},Et.styles=[Ze,Ye],Et);__decorate([n$1({attribute:!1}),__decorateMetadata("design:type",Object)],Lt.prototype,"hass",void 0),__decorate([n$1({attribute:!1}),__decorateMetadata("design:type",Object)],Lt.prototype,"config",void 0),__decorate([n$1({attribute:!1}),__decorateMetadata("design:type",Object)],Lt.prototype,"preview",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Lt.prototype,"_engine",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Lt.prototype,"_now",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Lt.prototype,"_cloudCover",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Lt.prototype,"_labelLayout",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Lt.prototype,"_pvCurrent",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Lt.prototype,"_pvUnit",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Lt.prototype,"_pvChangeSeries",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Lt.prototype,"_pvChangeSeriesPerEntity",void 0),__decorate([r$1(),__decorateMetadata("design:type",Array)],Lt.prototype,"_haSolarForecast",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Lt.prototype,"_batterySoc",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Lt.prototype,"_batteryPower",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Lt.prototype,"_batteryPowerUnit",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Lt.prototype,"_gridImportValue",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Lt.prototype,"_gridImportUnit",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Lt.prototype,"_gridExportValue",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Lt.prototype,"_gridExportUnit",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Lt.prototype,"_gridImportChangeSeries",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Lt.prototype,"_gridExportChangeSeries",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Lt.prototype,"_batterySocHistory",void 0),__decorate([r$1(),__decorateMetadata("design:type",Array)],Lt.prototype,"_batterySocPerBankHistory",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Lt.prototype,"_periodHourly",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Lt.prototype,"_batteryChargeChangeSeries",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Lt.prototype,"_batteryDischargeChangeSeries",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Lt.prototype,"_deviceChangeSeries",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Lt.prototype,"_sunScene",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Lt.prototype,"_energyDefaults",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Lt.prototype,"_haSolarTodayKwh",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Lt.prototype,"_homeHover",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Lt.prototype,"_chartHoverPct",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Lt.prototype,"_chartTarget",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Lt.prototype,"_infoPanelOpen",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Lt.prototype,"_uiHidden",void 0),__decorate([function e$2(e,t){return(i,n,r)=>{const o=t=>t.renderRoot?.querySelector(e)??null;if(t){const{get:e,set:t}="object"==typeof n?i:r??(()=>{const e=Symbol();return{get(){return this[e]},set(t){this[e]=t}}})();return e$3(i,n,{get(){let i=e.call(this);return void 0===i&&(i=o(this),(null!==i||this.hasUpdated)&&t.call(this,i)),i}})}return e$3(i,n,{get(){return o(this)}})}}("ha-card"),__decorateMetadata("design:type","undefined"==typeof HTMLElement?Object:HTMLElement)],Lt.prototype,"_haCard",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Lt.prototype,"_chartSeries",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Lt.prototype,"_timeRange",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Lt.prototype,"_selectedTime",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Lt.prototype,"_isLiveMode",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Lt.prototype,"_timelineMode",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Lt.prototype,"_unifiedStore",void 0),Lt=__decorate([t$2("helios-card")],Lt);export{Lt as HeliosCard};