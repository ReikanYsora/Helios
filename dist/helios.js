                ...
                <details class="advanced-section about-section" ?open="${"about"===this._openSection}" @toggle="${e=>this._onSectionToggle("about",e)}">
                    ...
                    <div class="about-row">
                        ...
                        <a class="about-row-link about-version-link"
                           href="https://github.com/ReikanYsora/Helios/releases/tag/v${"2026.7.1-a8"}"
                    ...
                    >
                        ...
                        >
                            ...
                            >
                                ...
                                ${function renderTimelineFutureMask(e){const t=e._timeRange;if(!t)return U``;const i=t.start.getTime(),r=t.end.getTime(),s=r-i;if(s<=0)return U``;const n=Date.now();return n<=i||n>=r?U``:U`
        <div
            class="hc-future-mask"
            style="left:${((n-i)/s*100).toFixed(2)}%"
        ></div>
    `}(this)}
                                ${function renderTimelineTicks(e){if(!e._timeRange)return U``;const{start:t,end:i}=e._timeRange,r=i.getTime()-t.getTime(),toPct=e=>Math.max(0,Math.min(100,(e.getTime()-t.getTime())/r*100)),s=toPct(/* @__PURE__ */new Date),n=!e._isLiveMode&&null!==e._selectedTime,l=n?toPct(e._selectedTime):0;return U`
...
<!--  Camera lock chip (top-left). Tapping flips the
                    ...
                    >
                        ${Ie.map(e=>q`
                            <line
                                ...
                                x1="${e.x1}" y1="${e.y1}"
                                x2="${e.x2}" y2="${e.y2}"
                                stroke-width="${(St.OUTLINE_FAR+(St.OUTLINE_NEAR-St.OUTLINE_FAR)*e.nearness)*St.NIGHT_STROKE_FACTOR}"
                            ></line>
                        `)}
                        ${Ie.map(e=>q`
                            <line
                                ...
                                x2="${e.x2}" y2="${e.y2}"
                                stroke="${e.color}"
                                stroke-width="${(St.SEGMENT_FAR+(St.SEGMENT_NEAR-St.SEGMENT_FAR)*e.nearness)*St.NIGHT_STROKE_FACTOR}"
                            ></line>
                        `)}
                    </svg>
                `:G}
                    ...
                    >
                        ${Ne.map(e=>q`
                            <line
                                ...
                                x1="${e.x1}" y1="${e.y1}"
                                x2="${e.x2}" y2="${e.y2}"
                                stroke-width="${St.OUTLINE_FAR+(St.OUTLINE_NEAR-St.OUTLINE_FAR)*e.nearness}"
                            ></line>
                        `)}
                        ${Ne.map(e=>q`
                            <line
                                ...
                                x2="${e.x2}" y2="${e.y2}"
                                stroke="${e.color}"
                                stroke-width="${St.SEGMENT_FAR+(St.SEGMENT_NEAR-St.SEGMENT_FAR)*e.nearness}"
                            ></line>
                        `)}
                    </svg>
                `:G}
                ...
                ${De?U`
                    <svg
                        ...
                        style="--solar-daylight:${Ee.daylight}"
                    >
                        ${(()=>{const e=this._engine?.getSunArcScale()??1,t=Math.min((St.SUN_R_FAR+(St.SUN_R_NEAR-St.SUN_R_FAR)*Ee.sun.nearness)*e,22),i=t*Be,r=3*t;return q`
                                <defs>
                                    <radialGradient id="solar-halo-grad">
                                        <stop offset="0%"   stop-color="${Le}" stop-opacity="${.55*Be}"></stop>
                                ...
                                <circle
                                    ...
                                    r="${t}"
                                    fill="${Le}"
                                    fill-opacity="${St.SUN_FILL_OPACITY_BG}"
                                ></circle>
                                <circle
                ...
                ${Ue&&this._cloudCover>=0?(()=>{const e=Ee.sun.x,t=Ee.sun.y-22,i=t-St.CHIP_HALF_H_PX,r=this._engine?.getViewportWidth()??0,s=St.CHIP_HALF_W_PX,n=s+16+76,l=r<=0||e+n<=r-8?1:e-n>=8?-1:e<r/2?1:-1,c=l>0?e+s+16:e-s-16,d=l>0?"translate(0, -100%)":"translate(-100%, -100%)";return U`
                        <div
        ...
        `}_renderClockTooltip(e){if(0===this._clockData.length)return U``;const t=this._clockSubMode,fmtSlot=e=>`${String(Math.floor(e/4)%24).padStart(2,"0")}:${String(e%4*15).padStart(2,"0")}`,i=Math.floor(e/4);return U`
            <div class="clock-tip">
                <div class="clock-tip-head">${"histogram"===t?`${String(i).padStart(2,"0")}:00 – ${String((i+1)%24).padStart(2,"0")}:00`:`${fmtSlot(e)} – ${fmtSlot((e+1)%96)}`}</div>
                ${this._clockData.map(i=>{const r=clockTargetMeta(this,i.target);if(i.layers.length>1){const r=i.layers.map(i=>({l:i,v:clockLayerValue(i,t,e)})).filter(e=>e.v>0);if(r.length>0)return U`${r.map(({l:e,v:t})=>U`
