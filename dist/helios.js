...
return new Date((c+d)/2)}var nt=null;function renderTimelineNightZones(e){const t=function computeNightIntervals(e){const t=e._timeRange;if(!t)return[];const i=getHomeCoords(e.config,e.hass);if(!i)return[];const r=t.start.getTime(),s=t.end.getTime(),n=s-r;if(n<=0)return[];const l=`${r}|${s}|${i.lat.toFixed(4)}|${i.lon.toFixed(4)}`;if(nt&&nt.key===l)return nt.out;const c=[],d=new Date(t.start);d.setHours(0,0,0,0),d.setDate(d.getDate()-1);const u=s+864e5;for(;d.getTime()<=u;){const e=d.getTime(),t=e+864e5,r=findSunCrossing(i.lat,i.lon,e,t,"rising"),s=findSunCrossing(i.lat,i.lon,e,t,"setting");r&&c.push({ms:r.getTime(),kind:"sunrise"}),s&&c.push({ms:s.getTime(),kind:"sunset"}),d.setDate(d.getDate()+1)}c.sort((e,t)=>e.ms-t.ms);const p=[];let m=null,g=!1;for(const y of c)"sunset"===y.kind?m=y.ms:(null!==m?(p.push({startMs:m,endMs:y.ms}),m=null):g||p.push({startMs:-1/0,endMs:y.ms}),g=!0);null!==m&&p.push({startMs:m,endMs:1/0});const f=[];for(const y of p){const e=Math.max(y.startMs,r),t=Math.min(y.endMs,s);t>e&&f.push({startPct:(e-r)/n*100,endPct:(t-r)/n*100})}return nt={key:l,out:f},f}(e);return 0===t.length?G:U`
    ...
    `}function renderTimelineHoverTooltip(e){const t=e._timeRange,i=e._chartSeries;if(!t)return G;const r=t.start.getTime(),s=t.end.getTime()-r;if(s<=0)return G;const n=e._chartHoverPct;if(null===n||n<0||n>100)return G;const l=n,c=r+l/100*s,d=i?interpAt(i.times,i.irradiance,c):NaN,u=i?interpAt(i.times,i.cloudLow,c):NaN,p=i?interpAt(i.times,i.cloudMid,c):NaN,m=i?interpAt(i.times,i.cloudHigh,c):NaN,g=e._customEntityHistory?interpAt(e._customEntityHistory.times,e._customEntityHistory.values,c):NaN,f=pvValueAtTime(e,c),y=e._chartTarget??"production",b=e._unifiedStore,_=b?valueAt(b.gridImport,b,c)??NaN:NaN,v=b?valueAt(b.gridExport,b,c)??NaN:NaN,w=b?valueAt(b.battery,b,c)??NaN:NaN,$=b?valueAt(b.production,b,c)??NaN:NaN,M=isFinite($)||isFinite(_)||isFinite(v)||isFinite(w),T=Math.max(0,(isFinite($)?$:0)+(isFinite(_)?_:0)-(isFinite(v)?v:0)-(isFinite(w)?w:0)),C=e._batterySocHistory?interpAt(e._batterySocHistory.times,e._batterySocHistory.values,c):NaN,F=valueDecimals(e.config),kw=t=>`${formatLocalisedNumber(e.hass,t/1e3,F)} kW`,H=e._pvHistoryPerEntity,E=H.size>1?Array.from(H.keys()):[],A=[];for(let I=0;I<E.length;I++){const t=E[I],i=H.get(t);if(!i)continue;const r=pvValueAtTime(e,c,i);if(!isFinite(r.value))continue;const s="W"===r.unit?0:F,n=`${formatLocalisedNumber(e.hass,r.value,s)} ${r.unit}`;A.push({id:t,label:solarSourceName(e,I),valueText:n,colorIdx:I})}const D=isFinite(f.value),R=clockTargetLabel(e,y),P=gridImportName(e),L=gridExportName(e),O=batteryChargeName(e),z=batteryDischargeName(e),W=e,j=ENERGY_COLOR_cloud(W),B=lerpHexToward(j,"#ffffff",.55),q=lerpHexToward(j,"#000000",.5),K=new Date(c),X=e.hass?.language||void 0,Y=s/864e5,Z=Y<=2.05?{hour:"2-digit",minute:"2-digit"}:Y<=14.05?{weekday:"short",hour:"2-digit",minute:"2-digit"}:{weekday:"short",day:"numeric",month:"short"},J=new Intl.DateTimeFormat(X,Z).format(K),Q=new Date(K);Q.setHours(0,0,0,0);const ee=/* @__PURE__ */new Date;ee.setHours(0,0,0,0);const te=Q.getTime()===ee.getTime(),ie=c>Date.now();let oe=function computeDailyKwhTotals(e){const t=/* @__PURE__ */new Map;if(!e._timeRange)return t;const{start:i,end:r}=e._timeRange,s=i.getTime(),n=r.getTime(),dayKey=e=>{const t=new Date(e);return t.setHours(0,0,0,0),t.getTime()},l=e._pvChangeSeries;if(l&&l.length>0){const e=new Date(s);for(e.setHours(0,0,0,0);e.getTime()<n;){const i=e.getTime(),r=new Date(e);r.setDate(r.getDate()+1);const s=sumChangeForDay(l,i,r.getTime());null!==s&&t.set(i,Math.max(0,s)),e.setTime(r.getTime())}}const c=e._unifiedStore;if(c){const e=Date.now(),i=c.stepMs/36e5;for(let r=0;r<c.bucketsTotal;r++){const l=c.storeStartMs+(r+.5)*c.stepMs;if(l<s||l>n)continue;if(l<e)continue;const d=c.forecast[r];if(null===d||!isFinite(d)||d<=0)continue;const u=dayKey(l);t.set(u,(t.get(u)??0)+d*i/1e3)}}return t}(e).get(Q.getTime());te&&!ie&&"number"==typeof e._haSolarTodayKwh&&isFinite(e._haSolarTodayKwh)&&(oe=e._haSolarTodayKwh);const re=ie&&void 0!==oe&&isFinite(oe)&&oe>=.05,ae=void 0!==oe&&isFinite(oe)&&oe>=.05?formatLocalisedNumber(e.hass,oe,F)+" kWh":"",se=Date.now(),ne=se>=r&&se<=r+s&&Math.abs(l-(se-r)/s*100)<=1.2,le=D&&"W"!==f.unit?F:0,ce=(e.hass?.language||"").toLowerCase().startsWith("fr")?"Retour au live":"Back to live";return U`
        <div
            class="tb-hover-tooltip-tail ${ne?"is-magnet-snap":""}"
        ...
        >
            <div class="tb-hover-tooltip">
                <div class="tb-hover-tooltip-time">
                    ...
                    <span
                        class="tb-hover-tooltip-live-chip ${ne?"is-visible":""}"
                        title=${ce}
                        aria-label=${ce}
                ...
                <details class="advanced-section about-section" data-section="about" ?open=${"about"===this._openSection} @toggle=${this._onSectionToggleEvt}>
                    ...
                    <div class="about-row">
                        ...
                        <a class="about-row-link about-version-link"
                           href="https://github.com/ReikanYsora/Helios/releases/tag/v${"2026.7.1-a22"}"
...
<!--  Camera lock chip (top-left). Tapping flips the
                ...
                ${Ve&&this._cloudCover>=0?(()=>{const e=Ee.sun.x,t=Ee.sun.y-22,i=t-St.CHIP_HALF_H_PX,r=this._engine?.getViewportWidth()??0,s=St.CHIP_HALF_W_PX,n=s+16+76,l=r<=0||e+n<=r-8?1:e-n>=8?-1:e<r/2?1:-1,c=l>0?e+s+16:e-s-16,d=l>0?"translate(0, -100%)":"translate(-100%, -100%)";return U`
                        <div
        ...
        `}_renderClockTooltip(e){if(0===this._clockData.length)return G;this._clockSubMode;const t=Math.floor(e/function clockSlotsPerHour(){return ut}());return U`
            <div class="clock-tip">
                <div class="clock-tip-head">${`${String(t).padStart(2,"0")}:00 – ${String((t+1)%24).padStart(2,"0")}:00`}</div>
                ${this._clockData.map(t=>{const i=clockTargetMeta(this,t.target);if(t.layers.length>1){const i=t.layers.map(i=>({l:i,v:clockLayerValue(i,t,0,e)})).filter(e=>e.v>0);if(i.length>0)return U`${i.map(({l:e,v:i})=>U`
