...
return new Date((c+d)/2)}var lt=null;function renderTimelineNightZones(e){const t=function computeNightIntervals(e){const t=e._timeRange;if(!t)return[];const i=getHomeCoords(e.config,e.hass);if(!i)return[];const r=t.start.getTime(),s=t.end.getTime(),n=s-r;if(n<=0)return[];const l=`${r}|${s}|${i.lat.toFixed(4)}|${i.lon.toFixed(4)}`;if(lt&&lt.key===l)return lt.out;const c=[],d=new Date(t.start);d.setHours(0,0,0,0),d.setDate(d.getDate()-1);const u=s+864e5;for(;d.getTime()<=u;){const e=d.getTime(),t=e+864e5,r=findSunCrossing(i.lat,i.lon,e,t,"rising"),s=findSunCrossing(i.lat,i.lon,e,t,"setting");r&&c.push({ms:r.getTime(),kind:"sunrise"}),s&&c.push({ms:s.getTime(),kind:"sunset"}),d.setDate(d.getDate()+1)}c.sort((e,t)=>e.ms-t.ms);const p=[];let m=null,g=!1;for(const y of c)"sunset"===y.kind?m=y.ms:(null!==m?(p.push({startMs:m,endMs:y.ms}),m=null):g||p.push({startMs:-1/0,endMs:y.ms}),g=!0);null!==m&&p.push({startMs:m,endMs:1/0});const f=[];for(const y of p){const e=Math.max(y.startMs,r),t=Math.min(y.endMs,s);t>e&&f.push({startPct:(e-r)/n*100,endPct:(t-r)/n*100})}return lt={key:l,out:f},f}(e);return 0===t.length?U``:U`
    ...
    `}function interpAt(e,t,i){const r=Math.min(e.length,t.length);if(0===r)return NaN;if(i<=e[0].getTime())return isFinite(t[0])?t[0]:NaN;if(i>=e[r-1].getTime()){const e=t[r-1];return isFinite(e)?e:NaN}let s=0,n=r-1;for(;n-s>1;){const t=s+n>>1;e[t].getTime()<=i?s=t:n=t}const l=e[s].getTime(),c=e[n].getTime(),d=t[s],u=t[n];if(!isFinite(d)||!isFinite(u))return NaN;const p=c-l;return p<=0?u:d+(u-d)*(i-l)/p}function renderPvChart(e){const t=e,i=e._timeRange;e._pvHistory;const r=1e3,s=100;if(!i)return U`<svg class="hc-chart-svg" viewBox="0 0 ${r} ${s}" preserveAspectRatio="none"></svg>`;const n=i.start.getTime(),l=i.end.getTime()-n;if(l<=0)return U`<svg class="hc-chart-svg" viewBox="0 0 ${r} ${s}" preserveAspectRatio="none"></svg>`;const c=ENERGY_COLOR_pv(t),d=e.hass?.themes?.darkMode?lerpHexToward(c,"#ffffff",.55):lerpHexToward(c,"#000000",.35),u=i.end.getTime(),p=buildTimelineModel(i.start,i.end).dayBoundaries.map(e=>e*r),m=(e._pvUnit||"").toLowerCase(),g="wh"===m||"kwh"===m||"mwh"===m,f=e._unifiedStore,y=f?function sliceForRange(e,t,i){const r=Math.max(e.storeStartMs,t),s=Math.min(e.storeEndMs,i);if(s<=r)return{times:[],production:[],forecast:[],cloud:[],irradiance:[]};const n=e.stepMs,l=Math.floor((r-e.storeStartMs)/n),c=[],d=[],u=[],p=[],m=[];for(let g=e.storeStartMs+l*n+n/2;g<s;g+=n)g<r||(c.push(new Date(g)),d.push(valueAt(e.production,e,g)),u.push(valueAt(e.forecast,e,g)),p.push(valueAt(e.cloud,e,g)),m.push(valueAt(e.irradiance,e,g)));return{times:c,production:d,forecast:u,cloud:p,irradiance:m}}(f,n,u):null,xOf=e=>(e.getTime()-n)/l*r,b=(()=>{const e=g?"kwh"===m?"kw":"mwh"===m?"mw":"wh"===m?"w":"":m;return"kw"===e?.001:"mw"===e?1e-6:1})(),_=[];if(y)for(let I=0;I<y.times.length;I++){const e=y.production[I];null!==e&&isFinite(e)&&_.push({t:y.times[I],v:e*b})}const v=[];if(y)for(let I=0;I<y.times.length;I++){const e=y.forecast[I];null===e||!isFinite(e)||e<=0||v.push({t:y.times[I],v:e*b})}let w=1;for(const I of _)I.v>w&&(w=I.v);for(const I of v)I.v>w&&(w=I.v);const yOf=e=>s-90*Math.max(0,Math.min(1,e/w)),$=_.map(e=>`${xOf(e.t).toFixed(2)},${yOf(e.v).toFixed(2)}`);let M="",T="";if($.length>=2){const e=xOf(_[0].t),t=xOf(_[_.length-1].t);M=`M ${e},100 L ${$.join(" L ")} L ${t},100 Z`,T=`M ${$.join(" L ")}`}const C=e._pvHistoryPerEntity.size>1?Array.from(e._pvHistoryPerEntity.keys()).sort():[],F=[];if(C.length>1&&_.length>=2){const t=e,i=chartIsDark(e),r=C.length,s=_.length,n=[];for(let c=0;c<r;c++){const t=e._pvHistoryPerEntity.get(C[c]),i=new Array(s).fill(0);if(t)for(let r=0;r<s;r++){const s=pvValueAtTime(e,_[r].t.getTime(),t).value;i[r]=isFinite(s)&&s>0?s:0}n.push(i)}const l=new Array(s).fill(0);for(let e=0;e<r;e++){const c=[],d=[];for(let t=0;t<s;t++){let i=0;for(let e=0;e<r;e++)i+=n[e][t];const s=i>0?n[e][t]/i:0,u=l[t],p=u+s*_[t].v;l[t]=p,c.push(`${xOf(_[t].t).toFixed(2)},${yOf(p).toFixed(2)}`),d.push(`${xOf(_[t].t).toFixed(2)},${yOf(u).toFixed(2)}`)}F.push({color:energySolarColor(t,i,e),path:`M ${c.join(" L ")} L ${d.reverse().join(" L ")} Z`})}}let H="";v.length>=2&&(H=`M ${v.map(e=>`${xOf(e.t).toFixed(2)},${yOf(e.v).toFixed(2)}`).join(" L ")}`);const A=e._chartHoverPct;let E=0,D=NaN,R=NaN,L=!1;if(null!==A&&A>=0&&A<=100){E=A/100*r;const e=n+A/100*l,t=_.length>0?_[_.length-1].t.getTime():-1/0;if(_.length>=1&&e<=t){const t=interpAt(_.map(e=>e.t),_.map(e=>e.v),e);isFinite(t)&&(D=yOf(Math.max(0,t)))}if(v.length>=1){const t=interpAt(v.map(e=>e.t),v.map(e=>e.v),e);isFinite(t)&&(R=yOf(Math.max(0,t)))}L=isFinite(D)||isFinite(R)}const P=[];if(L&&null!==A&&F.length>0){const t=n+A/100*l,i=interpAt(_.map(e=>e.t),_.map(e=>e.v),t);if(isFinite(i)&&i>0){const r=C.map(i=>{const r=e._pvHistoryPerEntity.get(i);if(!r)return 0;const s=pvValueAtTime(e,t,r).value;return isFinite(s)&&s>0?s:0}),s=r.reduce((e,t)=>e+t,0);if(s>0){let t=0;for(let n=0;n<C.length;n++)t+=r[n]/s,P.push({y:yOf(t*i),color:energySolarColor(e,chartIsDark(e),n)})}}}return U`
        ...
        `:G}
        ${L&&isFinite(R)?U`
            <div class="hc-hover-dot-html" style="left: ${(E/r*100).toFixed(2)}%; top: ${(R/s*100).toFixed(2)}%; background: ${d};"></div>
        `:G}
        ${P.map(e=>U`
                ...
                <details class="advanced-section about-section" ?open="${"about"===this._openSection}" @toggle="${e=>this._onSectionToggle("about",e)}">
                    ...
                    <div class="about-row">
                        ...
                        <a class="about-row-link about-version-link"
                           href="https://github.com/ReikanYsora/Helios/releases/tag/v${"2026.7.1-a9"}"
...
<!--  Camera lock chip (top-left). Tapping flips the
                        ...
                        ${(()=>{const e=this._engine?.getSunArcScale()??1,t=Math.min((St.SUN_R_FAR+(St.SUN_R_NEAR-St.SUN_R_FAR)*Ee.sun.nearness)*e,22),i=t*Be,r=3*t;return q`
                                <defs>
                                    <radialGradient id="solar-halo-grad">
                                        <stop offset="0%"   stop-color="${Re}" stop-opacity="${.55*Be}"></stop>
                                        <stop offset="100%" stop-color="${Re}" stop-opacity="0"></stop>
                                    </radialGradient>
                                </defs>
                                <circle
                ...
                ${De&&Ee?U`
                    ${this._renderSunCrossing(Ee.sunrise,Ee.home,"mdi:weather-sunset-up",Re)}
                    ${this._renderSunCrossing(Ee.sunset,Ee.home,"mdi:weather-sunset-down",Re)}
                `:G}
