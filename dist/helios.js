                ...
                <details class="advanced-section about-section" ?open="${"about"===this._openSection}" @toggle="${e=>this._onSectionToggle("about",e)}">
                    ...
                    <div class="about-row">
                        ...
                        <a class="about-row-link about-version-link"
                           href="https://github.com/ReikanYsora/Helios/releases/tag/v${"2026.7.1-a4"}"
...
<!--  Camera lock chip (top-left). Tapping flips the
                ...
                ${e&&"clock"===this._viewMode?(()=>{const e=function availableClockTargets(e){const t=e._unifiedStore,i=e._pvHistoryPerEntity.size>0||hasSignal(t?.production)||hasSignal(t?.forecast),r=hasSignal(t?.gridImport)||hasSignal(t?.gridExport),s=hasSignal(t?.battery),n=!!e._batterySocHistory&&e._batterySocHistory.values.length>0,l=[];return i&&l.push("production"),(i||r||s)&&l.push("consumption"),n&&l.push("battery-soc"),s&&l.push("battery"),r&&l.push("grid"),hasSignal(t?.irradiance)&&l.push("irradiance"),hasSignal(t?.cloud)&&l.push("cloud"),customEntityId(e.config)&&l.push("custom"),l}(this);return e.length?B`
                        <div class="overlay-top-right">
                            ${e.map(e=>{const t=clockTargetMeta(this,e),i=this._clockTargets.includes(e),r=function clockTargetLabel(e,t){return"custom"===t?resolveCustomEntityLive(e.hass,customEntityId(e.config))?.name||customEntityId(e.config)||"Custom":(String(e.hass?.language??"").toLowerCase().startsWith("fr")?ht:ct)[t]}(this,e);return B`
                                    <button
                                        type="button"
                                        class="overlay-btn ${i?"is-on":""}"
                    ...
                    >
                        ${Ie.map(e=>q`
                            <line
                                ...
                                x1="${e.x1}" y1="${e.y1}"
                                x2="${e.x2}" y2="${e.y2}"
                                stroke-width="${(_t.OUTLINE_FAR+(_t.OUTLINE_NEAR-_t.OUTLINE_FAR)*e.nearness)*_t.NIGHT_STROKE_FACTOR}"
                            ></line>
                        `)}
                        ${Ie.map(e=>q`
                            <line
                                ...
                                x2="${e.x2}" y2="${e.y2}"
                                stroke="${e.color}"
                                stroke-width="${(_t.SEGMENT_FAR+(_t.SEGMENT_NEAR-_t.SEGMENT_FAR)*e.nearness)*_t.NIGHT_STROKE_FACTOR}"
                            ></line>
                        `)}
                    </svg>
                `:G}
                    ...
                    >
                        ${Ne.map(e=>q`
                            <line
                                ...
                                x1="${e.x1}" y1="${e.y1}"
                                x2="${e.x2}" y2="${e.y2}"
                                stroke-width="${_t.OUTLINE_FAR+(_t.OUTLINE_NEAR-_t.OUTLINE_FAR)*e.nearness}"
                            ></line>
                        `)}
                        ${Ne.map(e=>q`
                            <line
                                ...
                                x2="${e.x2}" y2="${e.y2}"
                                stroke="${e.color}"
                                stroke-width="${_t.SEGMENT_FAR+(_t.SEGMENT_NEAR-_t.SEGMENT_FAR)*e.nearness}"
                            ></line>
                        `)}
                    </svg>
                `:G}
                ...
                ${De?B`
                    <svg
                        ...
                        style="--solar-daylight:${Ee.daylight}"
                    >
                        ${(()=>{const e=this._engine?.getSunArcScale()??1,t=Math.min((_t.SUN_R_FAR+(_t.SUN_R_NEAR-_t.SUN_R_FAR)*Ee.sun.nearness)*e,22),i=t*Ue,r=3*t;return q`
                                <defs>
                                ...
                                <circle
                                    ...
                                    r="${t}"
                                    fill="${Le}"
                                    fill-opacity="${_t.SUN_FILL_OPACITY_BG}"
                                ></circle>
                                <circle
                ...
                ${Ve&&this._cloudCover>=0?(()=>{const e=Ee.sun.x,t=Ee.sun.y-22,i=t-_t.CHIP_HALF_H_PX,r=this._engine?.getViewportWidth()??0,s=_t.CHIP_HALF_W_PX,n=s+16+76,l=r<=0||e+n<=r-8?1:e-n>=8?-1:e<r/2?1:-1,c=l>0?e+s+16:e-s-16,d=l>0?"translate(0, -100%)":"translate(-100%, -100%)";return B`
                        <div
