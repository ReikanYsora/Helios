...
return new Date((c+d)/2)}var st=null;function renderTimelineNightZones(e){const t=function computeNightIntervals(e){const t=e._timeRange;if(!t)return[];const i=getHomeCoords(e.config,e.hass);if(!i)return[];const r=t.start.getTime(),s=t.end.getTime(),n=s-r;if(n<=0)return[];const l=`${r}|${s}|${i.lat.toFixed(4)}|${i.lon.toFixed(4)}`;if(st&&st.key===l)return st.out;const c=[],d=new Date(t.start);d.setHours(0,0,0,0),d.setDate(d.getDate()-1);const u=s+864e5;for(;d.getTime()<=u;){const e=d.getTime(),t=e+864e5,r=findSunCrossing(i.lat,i.lon,e,t,"rising"),s=findSunCrossing(i.lat,i.lon,e,t,"setting");r&&c.push({ms:r.getTime(),kind:"sunrise"}),s&&c.push({ms:s.getTime(),kind:"sunset"}),d.setDate(d.getDate()+1)}c.sort((e,t)=>e.ms-t.ms);const p=[];let m=null,g=!1;for(const y of c)"sunset"===y.kind?m=y.ms:(null!==m?(p.push({startMs:m,endMs:y.ms}),m=null):g||p.push({startMs:-1/0,endMs:y.ms}),g=!0);null!==m&&p.push({startMs:m,endMs:1/0});const f=[];for(const y of p){const e=Math.max(y.startMs,r),t=Math.min(y.endMs,s);t>e&&f.push({startPct:(e-r)/n*100,endPct:(t-r)/n*100})}return st={key:l,out:f},f}(e);return 0===t.length?B``:B`
        ${t.map(e=>B`
            ...
            ></div>
        `)}
    `}var chartIsDark=e=>!!e.hass?.themes?.darkMode;function pvValueAtTime(e,t,i){const r=(e._pvUnit||"").trim();if(!r)return{value:NaN,unit:"",isPredicted:!1};const s=r.toLowerCase(),n="wh"===s||"kwh"===s||"mwh"===s,l=n?"kwh"===s?"kW":"mwh"===s?"MW":"W":r,c=l.toLowerCase(),d="kw"===c?.001:"mw"===c?1e-6:1,u=getHomeCoords(e.config,e.hass);if(u&&getSunPosition(new Date(t),u.lat,u.lon).altitude<=0)return{value:0,unit:l,isPredicted:!1};const p=i??e._pvHistory,m=p&&p.times.length>=1?p.times[0].getTime():1/0,g=p&&p.times.length>=1?p.times[p.times.length-1].getTime():-1/0;if(p&&p.times.length>=2&&t>=m&&t<=g)if(n)for(let y=1;y<p.times.length;y++){const e=p.times[y].getTime();if(t>e)continue;const i=p.times[y-1].getTime();if(t<i)break;const r=(e-i)/36e5;if(r<=0||r>6)break;const s=p.values[y]-p.values[y-1];if(!isFinite(s)||s<0)break;return{value:Math.max(0,s/r),unit:l,isPredicted:!1}}else{const e=interpAt(p.times,p.values,t);if(isFinite(e))return{value:Math.max(0,e),unit:l,isPredicted:!1}}if(!i){const i=e._pvCalibStats;if(i&&i.times.length>=2&&t<=g)if(n)for(let e=1;e<i.times.length;e++){const r=i.times[e].getTime();if(t>r)continue;const s=i.times[e-1].getTime();if(t<s)break;const n=(r-s)/36e5;if(n<=0||n>6)break;const c=i.values[e]-i.values[e-1];if(!isFinite(c)||c<0)break;return{value:Math.max(0,c/n),unit:l,isPredicted:!1}}else{const e=interpAt(i.times,i.values,t);if(isFinite(e))return{value:Math.max(0,e),unit:l,isPredicted:!1}}}if(i)return{value:NaN,unit:l,isPredicted:!1};const f=e._unifiedStore;if(f){const e=valueAt(f.forecast,f,t);if(null!==e&&e>0)return{value:Math.max(0,e)*d,unit:l,isPredicted:!0}}return{value:NaN,unit:l,isPredicted:!1}}function renderTimelineHoverTooltip(e){const t=e._timeRange,i=e._chartSeries;if(!t)return B``;const r=t.start.getTime(),s=t.end.getTime()-r;if(s<=0)return B``;const n=e._chartHoverPct;if(null===n||n<0||n>100)return B``;const l=n,c=r+l/100*s,d=i?interpAt(i.times,i.irradiance,c):NaN,u=i?interpAt(i.times,i.cloudLow,c):NaN,p=i?interpAt(i.times,i.cloudMid,c):NaN,m=i?interpAt(i.times,i.cloudHigh,c):NaN,g=e._customEntityHistory?interpAt(e._customEntityHistory.times,e._customEntityHistory.values,c):NaN,f=pvValueAtTime(e,c),y=e._chartTarget??"production",b=e._unifiedStore,v=b?valueAt(b.gridImport,b,c)??NaN:NaN,_=b?valueAt(b.gridExport,b,c)??NaN:NaN,w=b?valueAt(b.battery,b,c)??NaN:NaN,$=b?valueAt(b.production,b,c)??NaN:NaN,M=isFinite($)||isFinite(v)||isFinite(_)||isFinite(w),T=Math.max(0,(isFinite($)?$:0)+(isFinite(v)?v:0)-(isFinite(_)?_:0)-(isFinite(w)?w:0)),C=e._batterySocHistory?interpAt(e._batterySocHistory.times,e._batterySocHistory.values,c):NaN,kw=t=>`${formatLocalisedNumber(e.hass,t/1e3,1)} kW`,F=e._pvHistoryPerEntity,H=F.size>1?Array.from(F.keys()).sort():[],A=[];for(let B=0;B<H.length;B++){const t=H[B],i=F.get(t);if(!i)continue;const r=pvValueAtTime(e,c,i);if(!isFinite(r.value))continue;const s=e.hass?.states?.[t],n=String(s?.attributes?.friendly_name??t),l="W"===r.unit?0:Math.abs(r.value)<100?1:0,d=`${formatLocalisedNumber(e.hass,r.value,l)} ${r.unit}`;A.push({id:t,label:n,valueText:d,colorIdx:B})}const E=isFinite(f.value),D=e,L=ENERGY_COLOR_cloud(D),R=lerpHexToward(L,"#ffffff",.55),P=lerpHexToward(L,"#000000",.5),I=new Date(c),O=e.hass?.language||void 0,z=new Intl.DateTimeFormat(O,{hour:"2-digit",minute:"2-digit"}).format(I),W=new Date(I);W.setHours(0,0,0,0);const j=/* @__PURE__ */new Date;j.setHours(0,0,0,0);const U=W.getTime()===j.getTime(),q=c>Date.now();let K=function computeDailyKwhTotals(e){const t=/* @__PURE__ */new Map;if(!e._timeRange)return t;const{start:i,end:r}=e._timeRange,s=i.getTime(),n=r.getTime(),dayKey=e=>{const t=new Date(e);return t.setHours(0,0,0,0),t.getTime()},l=e._pvChangeSeries;if(l&&l.length>0){const e=new Date(s);for(e.setHours(0,0,0,0);e.getTime()<n;){const i=e.getTime(),r=new Date(e);r.setDate(r.getDate()+1);const s=sumChangeForDay(l,i,r.getTime());null!==s&&t.set(i,Math.max(0,s)),e.setTime(r.getTime())}}const c=e._unifiedStore;if(c){const e=Date.now(),i=c.stepMs/36e5;for(let r=0;r<c.bucketsTotal;r++){const l=c.storeStartMs+(r+.5)*c.stepMs;if(l<s||l>n)continue;if(l<e)continue;const d=c.forecast[r];if(null===d||!isFinite(d)||d<=0)continue;const u=dayKey(l);t.set(u,(t.get(u)??0)+d*i/1e3)}}return t}(e).get(W.getTime());U&&!q&&"number"==typeof e._haSolarTodayKwh&&isFinite(e._haSolarTodayKwh)&&(K=e._haSolarTodayKwh);const X=!q&&void 0!==K&&isFinite(K)&&K>=.05,Y=q&&void 0!==K&&isFinite(K)&&K>=.05,Z=void 0!==K&&isFinite(K)&&K>=.05?formatLocalisedNumber(e.hass,K,1)+" kWh":"",J=Date.now(),Q=J>=r&&J<=r+s&&Math.abs(l-(J-r)/s*100)<=1.2,ee=E?"W"===f.unit?0:Math.abs(f.value)<100?1:0:0,te=(e.hass?.language||"").toLowerCase().startsWith("fr")?"Retour au live":"Back to live";return B`
        <div
        ...
        >
            <div class="tb-hover-tooltip">
                ...
                ${"cloud"===y?B`
                    ...
                    ${isFinite(p)?B`
                        <div class="tb-hover-tooltip-row">
                            <ha-icon class="tb-hover-tooltip-icon" style="color:${L}" icon="mdi:format-vertical-align-center"></ha-icon>
                            <span class="tb-hover-tooltip-value">${Math.round(Math.max(0,Math.min(100,p)))} %</span>
                        </div>
                    `:G}
                    ${isFinite(u)?B`
                        <div class="tb-hover-tooltip-row">
                            <ha-icon class="tb-hover-tooltip-icon" style="color:${R}" icon="mdi:format-vertical-align-bottom"></ha-icon>
    ...
    `}function interpAt(e,t,i){const r=Math.min(e.length,t.length);if(0===r)return NaN;if(i<=e[0].getTime())return isFinite(t[0])?t[0]:NaN;if(i>=e[r-1].getTime()){const e=t[r-1];return isFinite(e)?e:NaN}let s=0,n=r-1;for(;n-s>1;){const t=s+n>>1;e[t].getTime()<=i?s=t:n=t}const l=e[s].getTime(),c=e[n].getTime(),d=t[s],u=t[n];if(!isFinite(d)||!isFinite(u))return NaN;const p=c-l;return p<=0?u:d+(u-d)*(i-l)/p}function renderPvChart(e){const t=e,i=e._timeRange;e._pvHistory;const r=1e3,s=100;if(!i)return B`<svg class="hc-chart-svg" viewBox="0 0 ${r} ${s}" preserveAspectRatio="none"></svg>`;const n=i.start.getTime(),l=i.end.getTime()-n;if(l<=0)return B`<svg class="hc-chart-svg" viewBox="0 0 ${r} ${s}" preserveAspectRatio="none"></svg>`;const c=ENERGY_COLOR_pv(t),d=e.hass?.themes?.darkMode?lerpHexToward(c,"#ffffff",.55):lerpHexToward(c,"#000000",.35),u=i.end.getTime(),p=buildTimelineModel(i.start,i.end).dayBoundaries.map(e=>e*r),m=(e._pvUnit||"").toLowerCase(),g="wh"===m||"kwh"===m||"mwh"===m,f=e._unifiedStore,y=f?function sliceForRange(e,t,i){const r=Math.max(e.storeStartMs,t),s=Math.min(e.storeEndMs,i);if(s<=r)return{times:[],production:[],forecast:[],cloud:[],irradiance:[]};const n=e.stepMs,l=Math.floor((r-e.storeStartMs)/n),c=[],d=[],u=[],p=[],m=[];for(let g=e.storeStartMs+l*n+n/2;g<s;g+=n)g<r||(c.push(new Date(g)),d.push(valueAt(e.production,e,g)),u.push(valueAt(e.forecast,e,g)),p.push(valueAt(e.cloud,e,g)),m.push(valueAt(e.irradiance,e,g)));return{times:c,production:d,forecast:u,cloud:p,irradiance:m}}(f,n,u):null,xOf=e=>(e.getTime()-n)/l*r,b=(()=>{const e=g?"kwh"===m?"kw":"mwh"===m?"mw":"wh"===m?"w":"":m;return"kw"===e?.001:"mw"===e?1e-6:1})(),v=[];if(y)for(let I=0;I<y.times.length;I++){const e=y.production[I];null!==e&&isFinite(e)&&v.push({t:y.times[I],v:e*b})}const _=[];if(y)for(let I=0;I<y.times.length;I++){const e=y.forecast[I];null===e||!isFinite(e)||e<=0||_.push({t:y.times[I],v:e*b})}let w=1;for(const I of v)I.v>w&&(w=I.v);for(const I of _)I.v>w&&(w=I.v);const yOf=e=>s-90*Math.max(0,Math.min(1,e/w)),$=v.map(e=>`${xOf(e.t).toFixed(2)},${yOf(e.v).toFixed(2)}`);let M="",T="";if($.length>=2){const e=xOf(v[0].t),t=xOf(v[v.length-1].t);M=`M ${e},100 L ${$.join(" L ")} L ${t},100 Z`,T=`M ${$.join(" L ")}`}const C=e._pvHistoryPerEntity.size>1?Array.from(e._pvHistoryPerEntity.keys()).sort():[],F=[];if(C.length>1&&v.length>=2){const t=e,i=chartIsDark(e),r=C.length,s=v.length,n=[];for(let c=0;c<r;c++){const t=e._pvHistoryPerEntity.get(C[c]),i=new Array(s).fill(0);if(t)for(let r=0;r<s;r++){const s=pvValueAtTime(e,v[r].t.getTime(),t).value;i[r]=isFinite(s)&&s>0?s:0}n.push(i)}const l=new Array(s).fill(0);for(let e=0;e<r;e++){const c=[],d=[];for(let t=0;t<s;t++){let i=0;for(let e=0;e<r;e++)i+=n[e][t];const s=i>0?n[e][t]/i:0,u=l[t],p=u+s*v[t].v;l[t]=p,c.push(`${xOf(v[t].t).toFixed(2)},${yOf(p).toFixed(2)}`),d.push(`${xOf(v[t].t).toFixed(2)},${yOf(u).toFixed(2)}`)}F.push({color:energySolarColor(t,i,e),path:`M ${c.join(" L ")} L ${d.reverse().join(" L ")} Z`})}}let H="";_.length>=2&&(H=`M ${_.map(e=>`${xOf(e.t).toFixed(2)},${yOf(e.v).toFixed(2)}`).join(" L ")}`);const A=e._chartHoverPct;let E=0,D=NaN,L=NaN,R=!1;if(null!==A&&A>=0&&A<=100){E=A/100*r;const e=n+A/100*l,t=v.length>0?v[v.length-1].t.getTime():-1/0;if(v.length>=1&&e<=t){const t=interpAt(v.map(e=>e.t),v.map(e=>e.v),e);isFinite(t)&&(D=yOf(Math.max(0,t)))}if(_.length>=1){const t=interpAt(_.map(e=>e.t),_.map(e=>e.v),e);isFinite(t)&&(L=yOf(Math.max(0,t)))}R=isFinite(D)||isFinite(L)}const P=[];if(R&&null!==A&&F.length>0){const t=n+A/100*l,i=interpAt(v.map(e=>e.t),v.map(e=>e.v),t);if(isFinite(i)&&i>0){const r=C.map(i=>{const r=e._pvHistoryPerEntity.get(i);if(!r)return 0;const s=pvValueAtTime(e,t,r).value;return isFinite(s)&&s>0?s:0}),s=r.reduce((e,t)=>e+t,0);if(s>0){let t=0;for(let n=0;n<C.length;n++)t+=r[n]/s,P.push({y:yOf(t*i),color:energySolarColor(e,chartIsDark(e),n)})}}}return B`
        <svg
            class="hc-chart-svg"
            viewBox="0 0 ${r} ${s}"
        ...
        ${R&&isFinite(D)?B`
            <div class="hc-hover-dot-html" style="left: ${(E/r*100).toFixed(2)}%; top: ${(D/s*100).toFixed(2)}%; background: ${c};"></div>
        `:G}
        ${R&&isFinite(L)?B`
            <div class="hc-hover-dot-html" style="left: ${(E/r*100).toFixed(2)}%; top: ${(L/s*100).toFixed(2)}%; background: ${d};"></div>
        `:G}
        ${P.map(e=>B`
            <div class="hc-hover-dot-html" style="left: ${(E/r*100).toFixed(2)}%; top: ${(e.y/s*100).toFixed(2)}%; background: ${e.color};"></div>
        `)}
    `}function renderBottomChart(e){const t=e._chartTarget??"production";return"production"===t?renderPvChart(e):function renderTargetChart(e,t){const i=e,r=e._unifiedStore,s=e._timeRange,n=1e3,l=100;if(!r||!s)return B`<svg class="hc-chart-svg" viewBox="0 0 ${n} ${l}" preserveAspectRatio="none"></svg>`;const c=s.start.getTime(),d=s.end.getTime(),u=d-c;if(u<=0)return B`<svg class="hc-chart-svg" viewBox="0 0 ${n} ${l}" preserveAspectRatio="none"></svg>`;const xOf=e=>(e-c)/u*n,toPts=(e,t)=>{const i=[];for(let s=0;s<e.length;s++){const n=e[s];if(null===n||!isFinite(n))continue;const l=r.storeStartMs+(s+.5)*r.stepMs;l<c||l>d||i.push({t:l,v:t?t(n):n})}return i},sum=e=>e.reduce((e,t)=>e+t.v,0);let p,m=0;if("consumption"===t){const e=[];for(let t=0;t<r.production.length;t++){const i=r.production[t],s=r.gridImport[t],n=r.gridExport[t],l=r.battery[t];if(null===i&&null===s&&null===n&&null===l)continue;const u=r.storeStartMs+(t+.5)*r.stepMs;if(u<c||u>d)continue;const p=Math.max(0,(i??0)+(s??0)-(n??0)-(l??0));e.push({t:u,v:p})}p=[{pts:e,color:ENERGY_COLOR_consumption(i)}]}else if("grid"===t){const e=toPts(r.gridImport),t=toPts(r.gridExport);p=[{pts:e,color:ENERGY_COLOR_gridImport(i)},{pts:t,color:ENERGY_COLOR_gridExport(i)}]}else if("battery"===t){const e=toPts(r.battery,e=>Math.max(0,e)),t=toPts(r.battery,e=>Math.max(0,-e));p=[{pts:e,color:ENERGY_COLOR_batteryIn(i)},{pts:t,color:ENERGY_COLOR_batteryOut(i)}]}else if("battery-soc"===t){const t=e._batterySocHistory,r=[];if(t)for(let e=0;e<t.times.length;e++){const i=t.times[e].getTime();if(i<c||i>d)continue;const s=t.values[e];void 0!==s&&isFinite(s)&&r.push({t:i,v:s})}p=[{pts:r,color:ENERGY_COLOR_batteryOut(i)}],m=100}else if("custom"===t){const t=e._customEntityHistory,r=[];if(t)for(let e=0;e<t.times.length;e++){const i=t.times[e].getTime();if(i<c||i>d)continue;const s=t.values[e];isFinite(s)&&r.push({t:i,v:Math.abs(s)})}p=[{pts:r,color:cssHex(i,"--red-color","#f44336")}]}else if("cloud"===t){const t=e._chartSeries,r=[],s=[],n=[];if(t)for(let e=0;e<t.times.length;e++){const i=t.times[e].getTime();if(i<c||i>d)continue;const l=t.cloudLow[e],u=t.cloudMid[e],p=t.cloudHigh[e];(isFinite(l)||isFinite(u)||isFinite(p))&&(r.push({t:i,v:isFinite(l)?Math.max(0,l):0}),s.push({t:i,v:isFinite(u)?Math.max(0,u):0}),n.push({t:i,v:isFinite(p)?Math.max(0,p):0}))}p=[{pts:r,color:lerpHexToward(ENERGY_COLOR_cloud(i),"#ffffff",.55)},{pts:s,color:ENERGY_COLOR_cloud(i)},{pts:n,color:lerpHexToward(ENERGY_COLOR_cloud(i),"#000000",.5)}],m=0}else p=[{pts:toPts(r.irradiance),color:ENERGY_COLOR_sun(i)}],m=1e3;const g="cloud"===t&&p.length>1&&p.every(e=>e.pts.length===p[0].pts.length&&e.pts.length>=2);let f=m;if(f<=0)if(f=1,g){const e=p[0].pts.length;for(let t=0;t<e;t++){let e=0;for(const i of p)e+=i.pts[t].v;e>f&&(f=e)}}else for(const T of p)for(const e of T.pts)e.v>f&&(f=e.v);const y=10,yOf=e=>l-Math.max(0,Math.min(1,e/f))*(l-y);let b;if(g){const e=p[0].pts.length,t=new Array(e).fill(0);b=p.map(i=>{const r=[],s=[];for(let l=0;l<e;l++){const e=t[l],n=e+i.pts[l].v;t[l]=n,r.push(`${xOf(i.pts[l].t).toFixed(2)},${yOf(n).toFixed(2)}`),s.push(`${xOf(i.pts[l].t).toFixed(2)},${yOf(e).toFixed(2)}`)}const n=`M ${r.join(" L ")}`;return{area:`M ${r.join(" L ")} L ${s.reverse().join(" L ")} Z`,line:n,color:i.color,total:sum(i.pts)}})}else b=p.map(e=>{if(e.pts.length<2)return{area:"",line:"",color:e.color,total:sum(e.pts)};const t=e.pts.map(e=>`${xOf(e.t).toFixed(2)},${yOf(e.v).toFixed(2)}`),i=xOf(e.pts[0].t),r=xOf(e.pts[e.pts.length-1].t);return{area:`M ${i},${l} L ${t.join(" L ")} L ${r},${l} Z`,line:`M ${t.join(" L ")}`,color:e.color,total:sum(e.pts)}});const v=buildTimelineModel(s.start,s.end).dayBoundaries.map(e=>e*n),_=e._chartHoverPct;let w=0,$=!1;const M=[];if(null!==_&&_>=0&&_<=100){w=_/100*n;const e=c+_/100*u;let t=0;for(const i of p){if(i.pts.length<1)continue;const r=interpAt(i.pts.map(e=>new Date(e.t)),i.pts.map(e=>e.v),e);isFinite(r)&&(g?(t+=Math.max(0,r),M.push({y:yOf(t),color:i.color})):M.push({y:yOf(Math.max(0,r)),color:i.color}),$=!0)}}return B`
...
`;function __decorateMetadata(e,t){if("object"==typeof Reflect&&"function"==typeof Reflect.metadata)return Reflect.metadata(e,t)}function __decorate(e,t,i,r){var s,n=arguments.length,l=n<3?t:null===r?r=Object.getOwnPropertyDescriptor(t,i):r;if("object"==typeof Reflect&&"function"==typeof Reflect.decorate)l=Reflect.decorate(e,t,i,r);else for(var c=e.length-1;c>=0;c--)(s=e[c])&&(l=(n<3?s(l):n>3?s(t,i,l):s(t,i))||l);return n>3&&l&&Object.defineProperty(t,i,l),l}var _t,wt,xt=(yt=class HeliosCardEditor extends le{constructor(...e){super(...e),this._cfg={},this._pickerReady=!1,this._openSection="location",this._sliderDebounce=/* @__PURE__ */new Map,this._solarIrradianceEntityFilter=e=>{if(!e||!e.attributes)return!1;if("irradiance"===e.attributes.device_class)return!0;const t=String(e.attributes.unit_of_measurement??"").trim();return"W/m²"===t||"W/m2"===t},this._customEntityFilter=e=>{if(!e||!e.attributes)return!1;const t=String(e.attributes.device_class??"");if("power"===t||"energy"===t)return!0;const i=String(e.attributes.unit_of_measurement??"").trim().toLowerCase();return"w"===i||"kw"===i||"mw"===i||"wh"===i||"kwh"===i||"mwh"===i},this._resetFeedback=null}disconnectedCallback(){super.disconnectedCallback();for(const e of this._sliderDebounce.values())window.clearTimeout(e);this._sliderDebounce.clear(),void 0!==this._resetFeedbackTimer&&(window.clearTimeout(this._resetFeedbackTimer),this._resetFeedbackTimer=void 0)}setConfig(e){if(this._cfg={...e},!this._cfg["cache-id"]){const e=`c${Date.now().toString(36)}${Math.floor(1e9*Math.random()).toString(36)}`;setTimeout(()=>{this._cfg["cache-id"]||this._update("cache-id",e)},0)}}connectedCallback(){super.connectedCallback(),this._ensureEntityPicker()}async _ensureEntityPicker(){if(!this._pickerReady)if("undefined"!=typeof customElements&&customElements.get("ha-entity-picker"))this._pickerReady=!0;else try{const e=window;if("function"==typeof e.loadCardHelpers){const t=await e.loadCardHelpers();if(t?.createCardElement){const e=(await t.createCardElement({type:"entities",entities:[]}))?.constructor;"function"==typeof e?.getConfigElement&&await e.getConfigElement()}}"undefined"!=typeof customElements&&await Promise.race([customElements.whenDefined("ha-entity-picker"),new Promise(e=>setTimeout(e,8e3))])}catch(e){console.warn("[HELIOS] Failed to lazy-load ha-entity-picker:",e)}finally{this._pickerReady=!0}}_t(){return pickTranslations(this.hass?.language)}_update(e,t){const i={...this._cfg,[e]:t};this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:i}})),this._cfg=i}_numField(e,t){const i=t.target.value.trim();if(""===i)return void this._update(e,void 0);const r=parseFloat(i);isFinite(r)&&this._update(e,r)}_numSlider(e,t){const i=parseFloat(t.target.value);if(!isFinite(i))return;this._cfg={...this._cfg,[e]:i};const r=String(e),s=this._sliderDebounce.get(r);void 0!==s&&window.clearTimeout(s);const n=window.setTimeout(()=>{this._sliderDebounce.delete(r),this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:this._cfg}}))},bt.SLIDER_COMMIT_DELAY_MS);this._sliderDebounce.set(r,n)}_onSectionToggle(e,t){const i=t.currentTarget;i.open?(this._openSection=e,requestAnimationFrame(()=>{i.scrollIntoView({behavior:"smooth",block:"start"})})):this._openSection===e&&(this._openSection=null)}_fmtNum(e,t){return t>=1?String(Math.round(e)):e.toFixed(2)}render(){const e=this._cfg,t=this._t(),i=this.hass?.config?.latitude,r=this.hass?.config?.longitude,s="number"==typeof i&&isFinite(i)?String(i):"52.379",n="number"==typeof r&&isFinite(r)?String(r):"4.900";return B`
                ...
                <details class="advanced-section about-section" ?open="${"about"===this._openSection}" @toggle="${e=>this._onSectionToggle("about",e)}">
                    ...
                    <div class="about-row">
                        ...
                        <a class="about-row-link about-version-link"
                           href="https://github.com/ReikanYsora/Helios/releases/tag/v${"2026.7.1-a7"}"
        ...
        `}render(){const e=null!==getHomeCoords(this.config,this.hass),t=this._labelLayout,i=resolvePvLiveEntity(this._energyDefaults),r=ENERGY_COLOR_pv(this),s=!this._isLiveMode&&null!==this._selectedTime,n=s&&this._selectedTime.getTime()>Date.now()+6e4,l=""!==i&&null!==t?s?function pvRateAtTime(e,t){const i=wattsAtFromChangeSeries(e._pvChangeSeries,t.getTime());return null===i?null:{value:Math.max(0,i),unit:"W"}}(this,this._selectedTime):null!==this._pvCurrent?function currentPvRate(e){const t=e._energyDefaults.solarStatRates;if(t.length>0){let i=0,r=!1;for(const s of t){const t=e.hass?.states?.[s];if(!t)continue;const n=parseFloat(t.state);isFinite(n)&&(i+=pvNormalizeToWatts(n,String(t.attributes?.unit_of_measurement??"")),r=!0)}if(r)return{value:Math.max(0,i),unit:"W"}}const i=latestWattsFromChangeSeries(e._pvChangeSeries,Date.now());return null===i?null:{value:Math.max(0,i),unit:"W"}}(this):null:null;let c=null;if(n&&""!==i&&null!==t&&this._unifiedStore){const e=valueAt(this._unifiedStore.forecast,this._unifiedStore,this._selectedTime.getTime());null!==e&&e>0&&(c={value:e,unit:"W"})}const d=n&&null!==c,u=d?c:l,p=e&&null!==t&&""!==i&&null!==u&&(!n||d),m=function valueDecimals(e){const t=e?.["value-decimals"],i="number"==typeof t?t:"string"==typeof t?parseFloat(t):NaN;if(!Number.isFinite(i))return 1;const r=Math.round(i);return r<0?0:r>3?3:r}(this.config),g=p?(d?"≈ ":"")+formatPvValue(this.hass,u.value,u.unit,m):"",f=null!==l?pvNormalizeToWatts(l.value,l.unit):0,y=flowDuration(f,5e3,.5),b=!(f>0),v=resolveBatteryEntities(this._energyDefaults),_=null!==v.socEntity,w=null!==v.powerEntity,$=!this._isLiveMode&&null!==this._selectedTime,M=$&&this._selectedTime.getTime()>Date.now()+6e4,T=$&&!M?this._selectedTime.getTime():null,C=null!==T?wattsAtFromChangeSeries(this._gridImportChangeSeries,T):this._gridImportValue,F=null!==T?wattsAtFromChangeSeries(this._gridExportChangeSeries,T):this._gridExportValue,H=null===C?null:Math.max(0,C),A=null===F?null:Math.max(0,F),E=null!==T?"W":this._gridImportUnit,D=null!==T?"W":this._gridExportUnit,L=$?function batterySampleAtTime(e,t){if(!e||0===e.times.length)return null;const i=t.getTime(),r=e.times[0].getTime(),s=e.times[e.times.length-1].getTime();if(i<r||i>s+6e4)return null;let n=e.times.length-1;for(let l=0;l<e.times.length;l++)if(e.times[l].getTime()>i){n=l-1;break}return n<0&&(n=0),e.values[n]}(this._batterySocHistory,this._selectedTime):this._batterySoc;let R;if($){const e=this._selectedTime.getTime(),t=wattsAtFromChangeSeries(this._batteryChargeChangeSeries,e),i=wattsAtFromChangeSeries(this._batteryDischargeChangeSeries,e);R=null===t&&null===i?null:Math.max(0,t??0)-Math.max(0,i??0)}else R=this._batteryPower;const P=$?"W":this._batteryPowerUnit,I=e&&null!==t&&!M&&_&&null!==L,O=e&&null!==t&&!M&&w&&null!==R,z=I?`${Math.round(L)} %`:"",W=O?function formatBatteryPower(e,t,i,r){return formatPowerKw(e,pvNormalizeToWatts(t,i),r,!0)}(this.hass,-R,P,m):"",j=n||null===u?null:pvNormalizeToWatts(u.value,u.unit),U=null!==H||null!==A?(H??0)-(A??0):null,K=O?R:null,X=null===j&&null===U&&null===K?null:Math.max(0,(j??0)+(U??0)-(K??0)),Y=e&&null!==t&&!M&&null!==X,Z=Y?formatGridValue(this.hass,X,"W",m):"",J=O&&R>0,Q=O&&R<0,ee=J?"var(--energy-battery-in-color, #f06292)":"var(--energy-battery-out-color, #4db6ac)",te=O?Math.abs(pvNormalizeToWatts(R,P)):0,ie=O&&te<5,oe=flowDuration(te,5e3),buildLPathToHome=(e,i,r)=>{if(!t)return"";const s=t.home.x,n=t.home.y,l=s>e?1:-1,c=n>i?1:-1,d=e+l*r,u=i,p=s-13*l,m=n-c*wt.HOME_PILL_HALF_HEIGHT_PX,g=Math.min(12,Math.abs(p-d)/2,Math.abs(m-u)/2),f=p-l*g,y=u+c*g;return`M ${d.toFixed(1)},${u.toFixed(1)} L ${f.toFixed(1)},${u.toFixed(1)} Q ${p.toFixed(1)},${u.toFixed(1)} ${p.toFixed(1)},${y.toFixed(1)} L ${p.toFixed(1)},${m.toFixed(1)}`},re=t?.batterySocLabel.x??0,ae=t?.batterySocLabel.y??0,se=t?.batteryPowerLabel.x??0,ne=t?.batteryPowerLabel.y??0,le=t&&O?`M ${re.toFixed(1)},${(ae-14).toFixed(1)} L ${se.toFixed(1)},${(ne+14).toFixed(1)}`:"",ce=t&&Q?buildLPathToHome(re,ae,22):"",he=t&&I&&!O&&!Q?buildLPathToHome(re,ae,22):"",de=t&&J&&p?((e,t,i,r,s)=>{const n=i>e?1:-1,l=r>t?1:-1,c=Math.min(12,Math.abs(i-e)/2,Math.abs(r-t)/2);if(s){const s=r-l*c,d=e+n*c;return`M ${e.toFixed(1)},${t.toFixed(1)} L ${e.toFixed(1)},${s.toFixed(1)} Q ${e.toFixed(1)},${r.toFixed(1)} ${d.toFixed(1)},${r.toFixed(1)} L ${i.toFixed(1)},${r.toFixed(1)}`}const d=i-n*c,u=t+l*c;return`M ${e.toFixed(1)},${t.toFixed(1)} L ${d.toFixed(1)},${t.toFixed(1)} Q ${i.toFixed(1)},${t.toFixed(1)} ${i.toFixed(1)},${u.toFixed(1)} L ${i.toFixed(1)},${r.toFixed(1)}`})(t.pvLabel.x+14,t.pvLabel.y+11,se-30,ne,!0):"",ue=buildLPathToHome(t?.gridLabel.x??0,t?.gridLabel.y??0,22),pe=null!==this._gridImportValue?Math.abs(pvNormalizeToWatts(this._gridImportValue,this._gridImportUnit)):0,me=null!==this._gridExportValue?Math.abs(pvNormalizeToWatts(this._gridExportValue,this._gridExportUnit)):0,proportionalBeadDur=(e,t)=>{const i=Math.max(e,1);return Math.min(8,Math.max(1.2,1.2*t/i))},fe=pe<5?null:proportionalBeadDur(pe,5e3),ye=me<5?null:proportionalBeadDur(me,1e3),be=(H??0)>=(A??0),ve=be?"var(--energy-grid-consumption-color, #488fc2)":"var(--energy-grid-return-color, #8353d1)",we=be?fe:ye,xe=resolveCustomEntityLive(this.hass,customEntityId(this.config)),Se=resolveCustomEntityIcon(this.hass,this.config),$e=_e,ke=buildLPathToHome(t?.customLabel.x??0,t?.customLabel.y??0,22),Me=this._isLiveMode||null===this._selectedTime?null:this._selectedTime.getTime(),Te=function customChipWatts(e,t,i,r){if(!t)return null;if(null!==r)return function customSampleAtTime(e,t){if(!e||0===e.times.length)return null;if(t<e.times[0].getTime()||t>e.times[e.times.length-1].getTime()+36e5)return null;let i=e.times.length-1;for(let r=0;r<e.times.length;r++)if(e.times[r].getTime()>t){i=r-1;break}return e.values[i<0?0:i]}(i,r);const s=e?.states?.[t];if(!s)return null;const n=String(s.attributes?.unit_of_measurement??"");if("energy"===String(s.attributes?.device_class??"")||qe.has(n.trim().toLowerCase()))return i&&i.values.length>0?i.values[i.values.length-1]:null;const l=parseFloat(s.state);return isFinite(l)?pvNormalizeToWatts(l,n):null}(this.hass,customEntityId(this.config),this._customEntityHistory,Me),Ce=null===Te?"":formatPvValue(this.hass,Te,"W",m),Fe=null===Te?0:Math.abs(Te),He=null===Te||Fe<5?null:Math.min(8,Math.max(1.2,6e3/Math.max(Fe,1))),Ae=null===Te||Te>=0,Ee=this._sunScene,De=e&&null!==Ee&&Ee.arc.length>=2,Le=ENERGY_COLOR_sun(this),Re=function darkenHex(e,t){const i=1-Math.max(0,Math.min(1,t)),r=Math.round(parseInt(e.slice(1,3),16)*i),s=Math.round(parseInt(e.slice(3,5),16)*i),n=Math.round(parseInt(e.slice(5,7),16)*i),h=e=>e.toString(16).padStart(2,"0");return`#${h(r)}${h(s)}${h(n)}`}(Le,.2),Pe=De?function buildArcSegments(e,t){const i=[];for(let r=0;r<e.length-1;r++){const s=e[r],n=e[r+1];i.push({x1:s.x,y1:s.y,x2:n.x,y2:n.y,color:arcColor(.5*(s.altitude+n.altitude),t),nearness:.5*(s.nearness+n.nearness),belowHorizon:s.belowHorizon||n.belowHorizon})}return i}(Ee.arc,Le):[],Ie=this._arcBackBuf,Ne=this._arcFrontBuf,Oe=this._arcFrontNearBuf;Ie.length=0,Ne.length=0,Oe.length=0;for(let B=0;B<Pe.length;B++){const e=Pe[B];e.belowHorizon?Ie.push(e):e.nearness>=.5?Oe.push(e):Ne.push(e)}const ze=De&&Ee.sun.altitude>0,We=Ee?.sun.irradiance??0,je=Math.round(We),Ue=Math.sqrt(Math.max(0,Math.min(1,We/1e3))),Ve=De&&Ee.sun.altitude>0,Be=flowDuration(We,1e3,.8);let Ke=Ee?.home.x??0,Ge=Ee?.home.y??0;if(t&&Ee&&i){const e=t.pvLabel.x,i=t.pvLabel.y,r=28,s=11,n=Ee.sun.x-e,l=Ee.sun.y-i,c=Math.max(0,r-s);if(Math.abs(n)<=c)Ke=Ee.sun.x,Ge=i+(l>=0?1:-1)*s;else{const t=e+(n>=0?1:-1)*c,r=i,l=Ee.sun.x-t,d=Ee.sun.y-r,u=Math.sqrt(l*l+d*d)||1;Ke=t+s*l/u,Ge=r+s*d/u}}const Xe=this.hass?.themes;return B`
            <ha-card class="${[this._computeIsDark(Xe)?"theme-dark":"theme-light",this._isCameraLocked()?"camera-locked":"",this.preview?"helios-edit":"","clock"===this._viewMode?"mode-clock":"","lidar"===this._viewMode?"mode-lidar":""].filter(Boolean).join(" ")}">
                ...
                ${e&&"clock"===this._viewMode?B`
                    ...
                    <div
                        ...
                        @keydown=${e=>{"Enter"!==e.key&&" "!==e.key||(e.preventDefault(),this._toggleClockSubMode())}}
                    >
                    ...
                    >
                        ...
                        >
                            ...
                            >
                                ...
                                ${function renderTimelineFutureMask(e){const t=e._timeRange;if(!t)return B``;const i=t.start.getTime(),r=t.end.getTime(),s=r-i;if(s<=0)return B``;const n=Date.now();return n<=i||n>=r?B``:B`
        <div
            class="hc-future-mask"
            style="left:${((n-i)/s*100).toFixed(2)}%"
        ></div>
    `}(this)}
                                ${function renderTimelineTicks(e){if(!e._timeRange)return B``;const{start:t,end:i}=e._timeRange,r=i.getTime()-t.getTime(),toPct=e=>Math.max(0,Math.min(100,(e.getTime()-t.getTime())/r*100)),s=toPct(/* @__PURE__ */new Date),n=!e._isLiveMode&&null!==e._selectedTime,l=n?toPct(e._selectedTime):0;return B`
...
<!--  Camera lock chip (top-left). Tapping flips the
                      ...
                      useless on touch.                              -->
                ${e?(()=>{const e=this._isCameraLocked(),t=e?"mdi:lock":"mdi:lock-open-variant",i="scene"===this._viewMode,r="clock"===this._viewMode,s="lidar"===this._viewMode,n=hasLocalLidar(this.config);return B`
                ...
                ${e&&"clock"===this._viewMode?(()=>{const e=function availableClockTargets(e){const t=e._unifiedStore,i=e._pvHistoryPerEntity.size>0||hasSignal(t?.production)||hasSignal(t?.forecast),r=hasSignal(t?.gridImport)||hasSignal(t?.gridExport),s=hasSignal(t?.battery),n=!!e._batterySocHistory&&e._batterySocHistory.values.length>0,l=[];return i&&l.push("production"),(i||r||s)&&l.push("consumption"),n&&l.push("battery-soc"),s&&l.push("battery"),r&&l.push("grid"),hasSignal(t?.irradiance)&&l.push("irradiance"),hasSignal(t?.cloud)&&l.push("cloud"),customEntityId(e.config)&&l.push("custom"),l}(this);return e.length?B`
                        <div class="overlay-top-right">
                            ${e.map(e=>{const t=clockTargetMeta(this,e),i=this._clockTargets.includes(e),r=function clockTargetLabel(e,t){return"custom"===t?resolveCustomEntityLive(e.hass,customEntityId(e.config))?.name||customEntityId(e.config)||"Custom":(String(e.hass?.language??"").toLowerCase().startsWith("fr")?dt:ht)[t]}(this,e);return B`
                                    <button
                                        type="button"
                                        class="overlay-btn ${i?"is-on":""}"
                    ...
                    >
                        ${Ie.map(e=>q`
                            <line
                                ...
                                x1="${e.x1}" y1="${e.y1}"
                                x2="${e.x2}" y2="${e.y2}"
                                stroke-width="${(wt.OUTLINE_FAR+(wt.OUTLINE_NEAR-wt.OUTLINE_FAR)*e.nearness)*wt.NIGHT_STROKE_FACTOR}"
                            ></line>
                        `)}
                        ${Ie.map(e=>q`
                            <line
                                ...
                                x2="${e.x2}" y2="${e.y2}"
                                stroke="${e.color}"
                                stroke-width="${(wt.SEGMENT_FAR+(wt.SEGMENT_NEAR-wt.SEGMENT_FAR)*e.nearness)*wt.NIGHT_STROKE_FACTOR}"
                            ></line>
                        `)}
                    </svg>
                `:G}
                ...
                ${p?(()=>{const e=t.pvLabel.x,i=t.pvLabel.y+11,s=this._nudgeToHomePill(e,i,t.home.x,t.home.y);return B`
                ...
                ${!e||null===t||null===H&&null===A||M?G:B`
                    ...
                    >
                        ...
                        <span>${formatGridValue(this.hass,be?H??0:A??0,be?E:D,m)}</span>
                    </div>
                `}
                    ...
                    >
                        ${Ne.map(e=>q`
                            <line
                                ...
                                x1="${e.x1}" y1="${e.y1}"
                                x2="${e.x2}" y2="${e.y2}"
                                stroke-width="${wt.OUTLINE_FAR+(wt.OUTLINE_NEAR-wt.OUTLINE_FAR)*e.nearness}"
                            ></line>
                        `)}
                        ${Ne.map(e=>q`
                            <line
                                ...
                                x2="${e.x2}" y2="${e.y2}"
                                stroke="${e.color}"
                                stroke-width="${wt.SEGMENT_FAR+(wt.SEGMENT_NEAR-wt.SEGMENT_FAR)*e.nearness}"
                            ></line>
                        `)}
                    </svg>
                `:G}
                    ...
                    <svg class="solar-svg solar-ray-svg"
                        ...
                        >
                            <animateMotion
                                dur="${Be}s"
                                repeatCount="indefinite"
                                path="M ${Ee.sun.x},${Ee.sun.y} L ${Ke},${Ge}"
                            ></animateMotion>
                        </circle>
                    </svg>
                `:G}
                ...
                ${De?B`
                    <svg
                        ...
                        style="--solar-daylight:${Ee.daylight}"
                    >
                        ${(()=>{const e=this._engine?.getSunArcScale()??1,t=Math.min((wt.SUN_R_FAR+(wt.SUN_R_NEAR-wt.SUN_R_FAR)*Ee.sun.nearness)*e,22),i=t*Ue,r=3*t;return q`
                                <defs>
                                    <radialGradient id="solar-halo-grad">
                                        <stop offset="0%"   stop-color="${Le}" stop-opacity="${.55*Ue}"></stop>
                                        <stop offset="100%" stop-color="${Le}" stop-opacity="0"></stop>
                                    </radialGradient>
                                </defs>
                                <circle
                                ...
                                <circle
                                    ...
                                    r="${t}"
                                    fill="${Le}"
                                    fill-opacity="${wt.SUN_FILL_OPACITY_BG}"
                                ></circle>
                                <circle
                ...
                ${Ve&&this._cloudCover>=0?(()=>{const e=Ee.sun.x,t=Ee.sun.y-22,i=t-wt.CHIP_HALF_H_PX,r=this._engine?.getViewportWidth()??0,s=wt.CHIP_HALF_W_PX,n=s+16+76,l=r<=0||e+n<=r-8?1:e-n>=8?-1:e<r/2?1:-1,c=l>0?e+s+16:e-s-16,d=l>0?"translate(0, -100%)":"translate(-100%, -100%)";return B`
                        <div
                ...
                ${De&&Ee?B`
                    ${this._renderSunCrossing(Ee.sunrise,Ee.home,"mdi:weather-sunset-up",Le)}
                    ${this._renderSunCrossing(Ee.sunset,Ee.home,"mdi:weather-sunset-down",Le)}
                `:G}
        ...
        `}_maybeRebuildUnifiedStore(){(function isStoreFresh(e,t){return!!t&&t.dataVersion===computeDataVersion(e)})(this,this._unifiedStore)||(this._unifiedStore=buildUnifiedStore(this))}_isCameraLocked(){return!!this._engine&&this._engine.isCameraLocked()}_setViewMode(e){if(this._viewMode!==e){if(this._clockAnimSeq++,this._clockExiting=[],this._clockSlotFrom.clear(),this._clockSlideStart=0,"clock"===e){0===this._clockTargets.length&&(this._clockTargets=[this._chartTarget]),this._rebuildClockData();const t=Date.now();return this._clockGrowStart.clear(),this._clockTargets.forEach(e=>this._clockGrowStart.set(e,t)),this._engine?.setHomeOnly(!0),this._updateClockHomeAppearance(),this._viewMode=e,this._persistUiState(),void this._clockAnimate()}this._engine?.setHomeOnly(!1),this._updateHomeAppearance(!1),this._clockTargets.length>0&&this._setChartTarget(this._clockTargets[0]),this._viewMode=e,this._persistUiState()}}_toggleClockSubMode(){this._clockSubMode="area"===this._clockSubMode?"histogram":"area",this._persistUiState(),this._scheduleClockPaint()}_captureClockSlots(){const e=Date.now(),t=1-(1-(this._clockSlideStart?Math.min(1,(e-this._clockSlideStart)/320):1))**3,i=/* @__PURE__ */new Map;this._clockData.forEach((e,r)=>{const s=this._clockSlotFrom.get(e.target)??r;i.set(e.target,s+(r-s)*t)}),this._clockSlotFrom=i,this._clockSlideStart=e}_startClockDim(){null!==this._clockHoverSlot&&(this._clockDimSlot=this._clockHoverSlot);const e=null!==this._clockHoverSlot?1:0;if(this.preview||window.matchMedia?.("(prefers-reduced-motion: reduce)").matches)return this._clockDim=e,0===e&&(this._clockDimSlot=null),void this._scheduleClockPaint();const t=++this._clockDimSeq,i=this._clockDim,r=performance.now(),tick=s=>{if(t!==this._clockDimSeq||"clock"!==this._viewMode)return;const n=Math.min(1,(s-r)/150);this._clockDim=i+(e-i)*n,this._paintClock(),n<1?requestAnimationFrame(tick):(this._clockDim=e,0===e&&(this._clockDimSlot=null),this._paintClock())};requestAnimationFrame(tick)}_registerCacheId(){const e=cacheId(this.config);if(!e)return;const t=kt.get(e)??[];t.includes(this)||(t.push(this),kt.set(e,t))}_unregisterCacheId(){const e=cacheId(this.config),t=e?kt.get(e):void 0;if(!t)return;const i=t.indexOf(this);i>=0&&t.splice(i,1),0===t.length&&kt.delete(e)}_effectiveCacheId(){const e=cacheId(this.config);if(!e)return"";const t=kt.get(e),i=t?t.indexOf(this):-1;return i>0?`${e}#${i+1}`:e}_uiStateStorageKey(){const e=this._effectiveCacheId();if(e)return`helios:ui-state:${e}`;const t=getHomeCoords(this.config,this.hass);return t?`helios:ui-state:${Math.round(1e3*t.lat)/1e3}:${Math.round(1e3*t.lon)/1e3}`:null}_restoreUiState(){if(this._uiStateRestored)return;const e=this._uiStateStorageKey();if(e){this._uiStateRestored=!0;try{const t=window.localStorage.getItem(e);if(!t)return;const i=JSON.parse(t);if(i&&"object"==typeof i){("scene"===i.viewMode||"clock"===i.viewMode||"lidar"===i.viewMode&&hasLocalLidar(this.config))&&(this._viewMode=i.viewMode);const e=["production","consumption","grid","battery","battery-soc","irradiance","cloud","custom"];if("string"==typeof i.chartTarget&&e.includes(i.chartTarget)&&(this._chartTarget=i.chartTarget),"area"!==i.clockSubMode&&"histogram"!==i.clockSubMode||(this._clockSubMode=i.clockSubMode),Array.isArray(i.clockTargets)){const t=/* @__PURE__ */new Set,r=[];for(const s of i.clockTargets)"string"==typeof s&&e.includes(s)&&!t.has(s)&&(t.add(s),r.push(s));this._clockTargets=r,r.length>0&&(this._chartTarget=r[0])}}}catch(I){}}}_persistUiState(){const e=this._uiStateStorageKey();if(e)try{window.localStorage.setItem(e,JSON.stringify({viewMode:this._viewMode,chartTarget:this._chartTarget,clockTargets:this._clockTargets,clockSubMode:this._clockSubMode}))}catch(I){}}_rebuildClockData(){this._clockData=this._clockTargets.map(e=>function buildClockData(e,t){const i=e._unifiedStore,r=e,s=e.themeIsDark(),n=clockTargetMeta(e,t),data=(e,i)=>({target:t,color:n.color,unit:e,layers:i});if("production"===t){if(!i)return data("power",[]);const t=Array.from(e._pvHistoryPerEntity.keys()).sort(),n=Date.now(),l=t.map(()=>new Array(ct).fill(0)),c=t.map(()=>new Array(ct).fill(0));for(let r=0;r<i.bucketsTotal;r++){const s=i.storeStartMs+(r+.5)*i.stepMs;if(s>n)break;const d=slotOf(new Date(s));t.forEach((t,i)=>{const r=e._pvHistoryPerEntity.get(t);if(!r)return;const n=pvValueAtTime(e,s,r).value;isFinite(n)&&n>0&&(l[i][d]+=n,c[i][d]+=1)})}const d=l.map((e,t)=>e.map((e,i)=>c[t][i]?e/c[t][i]:0)),u=t.map((t,i)=>({color:energySolarColor(r,s,i),icon:"mdi:solar-power",label:String(e.hass?.states?.[t]?.attributes?.friendly_name??t),values:d[i]})),p=binSlotAvg(i,i.forecast).map((e,t)=>d.reduce((e,i)=>e+i[t],0)<=0&&e>0?e:0);return p.some(e=>e>0)&&u.push({color:ENERGY_COLOR_pv(r),icon:"mdi:solar-power",label:"",values:p,predicted:!0}),data("power",u)}const avgOf=(e,t)=>fillGaps(e.map((e,i)=>t[i]?e/t[i]:NaN));if("battery-soc"===t){const t=e._batterySocHistory,i=new Array(ct).fill(0),s=new Array(ct).fill(0);if(t)for(let e=0;e<t.times.length;e++){const r=t.values[e];if(!isFinite(r))continue;const n=slotOf(t.times[e]);i[n]+=r,s[n]+=1}return data("percent",[{color:ENERGY_COLOR_batteryOut(r),icon:"mdi:battery",label:"",values:avgOf(i,s)}])}if("cloud"===t){const t=e._chartSeries,i=[new Array(ct).fill(0),new Array(ct).fill(0),new Array(ct).fill(0)],s=[new Array(ct).fill(0),new Array(ct).fill(0),new Array(ct).fill(0)];if(t)for(let e=0;e<t.times.length;e++){const r=slotOf(t.times[e]);[t.cloudLow[e],t.cloudMid[e],t.cloudHigh[e]].forEach((e,t)=>{isFinite(e)&&(i[t][r]+=Math.max(0,e),s[t][r]+=1)})}const n=ENERGY_COLOR_cloud(r),l=[lerpHexToward(n,"#ffffff",.55),n,lerpHexToward(n,"#000000",.5)],c=["mdi:format-vertical-align-bottom","mdi:format-vertical-align-center","mdi:format-vertical-align-top"];return data("percent",[0,1,2].map(e=>({color:l[e],icon:c[e],label:"",values:avgOf(i[e],s[e])})))}if("custom"===t){const t=e._customEntityHistory,i=new Array(ct).fill(0),r=new Array(ct).fill(0);if(t)for(let e=0;e<t.times.length;e++){const s=t.values[e];if(!isFinite(s))continue;const n=slotOf(t.times[e]);i[n]+=Math.abs(s),r[n]+=1}return data("power",[{color:n.color,icon:n.icon,label:"",values:avgOf(i,r)}])}if(!i)return data("irradiance"===t?"irradiance":"power",[]);let l,c="power";if("grid"===t)l=[{series:i.gridImport,color:ENERGY_COLOR_gridImport(r),icon:"mdi:transmission-tower-import"},{series:i.gridExport,color:ENERGY_COLOR_gridExport(r),icon:"mdi:transmission-tower-export"}];else if("battery"===t){const e=i.battery.map(e=>null===e?null:Math.max(0,e));l=[{series:i.battery.map(e=>null===e?null:Math.max(0,-e)),color:ENERGY_COLOR_batteryOut(r),icon:"mdi:battery-arrow-up"},{series:e,color:ENERGY_COLOR_batteryIn(r),icon:"mdi:battery-arrow-down"}]}else if("irradiance"===t)c="irradiance",l=[{series:i.irradiance,color:ENERGY_COLOR_sun(r),icon:"mdi:white-balance-sunny"}];else{const e=new Array(i.bucketsTotal).fill(null);for(let t=0;t<i.bucketsTotal;t++){const r=i.production[t],s=i.gridImport[t],n=i.gridExport[t],l=i.battery[t];null===r&&null===s&&null===n&&null===l||(e[t]=Math.max(0,(r??0)+(s??0)-(n??0)-(l??0)))}l=[{series:e,color:ENERGY_COLOR_consumption(r),icon:"mdi:home-lightning-bolt"}]}return data(c,l.map(e=>({color:e.color,icon:e.icon,label:"",values:binSlotAvg(i,e.series)})))}(this,e))}_clockSlotNow(e,t){const i=this._clockSlotFrom.get(t)??e;return i+(e-i)*(1-(1-(this._clockSlideStart?Math.min(1,(Date.now()-this._clockSlideStart)/320):1))**3)}_clockAnimActive(){const e=Date.now();if(this._clockExiting.length>0)return!0;if(this._clockSlideStart&&e-this._clockSlideStart<320)return!0;for(const t of this._clockGrowStart.values())if(e-t<320)return!0;return!1}_clockAnimate(){const settle=()=>{const e=Date.now();this._clockExiting=this._clockExiting.filter(t=>e-t.start<320);for(const[t,i]of this._clockGrowStart)e-i>=320&&this._clockGrowStart.delete(t);this._clockSlideStart&&e-this._clockSlideStart>=320&&(this._clockSlideStart=0,this._clockSlotFrom.clear())};if(this.preview||window.matchMedia?.("(prefers-reduced-motion: reduce)").matches)return this._clockExiting=[],this._clockGrowStart.clear(),this._clockSlideStart=0,this._clockSlotFrom.clear(),void this._paintClock();const e=++this._clockAnimSeq,tick=()=>{e===this._clockAnimSeq&&"clock"===this._viewMode&&(settle(),this._paintClock(),this._clockAnimActive()&&requestAnimationFrame(tick))};requestAnimationFrame(tick)}_scheduleClockPaint(){requestAnimationFrame(()=>this._paintClock())}_paintClock(){if("clock"!==this._viewMode)return;const e=this._clockSvg,t=this._engine?._renderer?.camera;if(!e||!t||!t.hasViewport)return;const i=Date.now(),r=this._clockData.map((e,t)=>{const r=this._clockGrowStart.get(e.target),s=r?1-(1-Math.min(1,(i-r)/320))**3:1;return{data:e,slot:this._clockSlotNow(t,e.target),heightScale:s,opacity:1}});for(const n of this._clockExiting){const e=1-(1-Math.min(1,(i-n.start)/320))**3;r.push({data:n.data,slot:n.slot,heightScale:n.h0*(1-e),opacity:1-e})}const s=projectClockFrame(t,r,this._clockSubMode,this._clockDimSlot,this._clockDim);e.innerHTML=s.svg,this._clockHits=s.hits,this._clockLabels?.forEach((e,t)=>{const i=s.labels[t];i&&(e.style.left=`${i.x.toFixed(1)}px`,e.style.top=`${i.y.toFixed(1)}px`,e.style.opacity=i.opacity.toFixed(3),e.style.transform=i.transform)}),this._clockCompassLabels?.forEach((e,t)=>{const i=s.compass[t];i&&(e.style.left=`${i.x.toFixed(1)}px`,e.style.top=`${i.y.toFixed(1)}px`,e.style.transform=i.transform)}),this._clampClockTip()}_clampClockTip(){const e=this._clockTip,t=this._haCard;if(!e||!t)return;const i=t.clientWidth,r=t.clientHeight,s=e.offsetWidth,n=e.offsetHeight;let l=this._clockHoverX+14;l+s>i-8&&(l=this._clockHoverX-14-s),l=Math.max(8,Math.min(l,i-s-8));const c=Math.max(8,Math.min(this._clockHoverY-n/2,r-n-8));e.style.left=`${l}px`,e.style.top=`${c}px`}_formatClockHour(e){return function formatHaTime(e,t){const i=e?.locale,r={hour:"numeric",minute:"2-digit",hour12:haUseAmPm(i)};try{return new Intl.DateTimeFormat(i?.language,r).format(t)}catch(I){return new Intl.DateTimeFormat(void 0,r).format(t)}}(this.hass,new Date(2e3,0,1,e))}_renderClockTooltip(e){if(0===this._clockData.length)return B``;const t=this._clockSubMode,fmtSlot=e=>`${String(Math.floor(e/4)%24).padStart(2,"0")}:${String(e%4*15).padStart(2,"0")}`,i=Math.floor(e/4),r="histogram"===t?`${String(i).padStart(2,"0")}:00 – ${String((i+1)%24).padStart(2,"0")}:00`:`${fmtSlot(e)} – ${fmtSlot((e+1)%96)}`;return B`
            <div
                class="clock-tip"
                style="left:${this._clockHoverX}px;top:${this._clockHoverY}px"
            >
                <div class="clock-tip-head">${r}</div>
                ${this._clockData.map(i=>{const r=clockTargetMeta(this,i.target);if(i.layers.length>1){const r=i.layers.map(i=>({l:i,v:clockLayerValue(i,t,e)})).filter(e=>e.v>0);if(r.length>0)return B`${r.map(({l:e,v:t})=>B`
