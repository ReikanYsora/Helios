...
return new Date((d+c)/2)}var kt=null;function renderTimelineNightZones(e){const t=function computeNightIntervals(e){const t=e._timeRange;if(!t)return[];const i=getHomeCoords(e.config,e.hass);if(!i)return[];const n=t.start.getTime(),r=t.end.getTime(),s=r-n;if(s<=0)return[];const l=`${n}|${r}|${i.lat.toFixed(4)}|${i.lon.toFixed(4)}`;if(kt&&kt.key===l)return kt.out;const d=[],c=new Date(t.start);c.setHours(0,0,0,0),c.setDate(c.getDate()-1);const u=r+864e5;for(;c.getTime()<=u;){const e=c.getTime(),t=e+864e5,n=findSunCrossing(i.lat,i.lon,e,t,"rising"),r=findSunCrossing(i.lat,i.lon,e,t,"setting");n&&d.push({ms:n.getTime(),kind:"sunrise"}),r&&d.push({ms:r.getTime(),kind:"sunset"}),c.setDate(c.getDate()+1)}d.sort((e,t)=>e.ms-t.ms);const p=[];let g=null,m=!1;for(const v of d)"sunset"===v.kind?g=v.ms:(null!==g?(p.push({startMs:g,endMs:v.ms}),g=null):m||p.push({startMs:-1/0,endMs:v.ms}),m=!0);null!==g&&p.push({startMs:g,endMs:1/0});const f=[];for(const v of p){const e=Math.max(v.startMs,n),t=Math.min(v.endMs,r);t>e&&f.push({startPct:(e-n)/s*100,endPct:(t-n)/s*100})}return kt={key:l,out:f},f}(e);return 0===t.length?K:B`
    ...
    `}var _t=["group-1","group-2","group-3","group-4"];function groupTarget(e){return`group-${e}`}function isGroupTarget(e){return"string"==typeof e&&e.startsWith("group-")}function groupOfTarget(e){return isGroupTarget(e)?Number(e.slice(6)):0}function statFriendly(e,t){const i=t[0];return i?String(e.hass?.states?.[i]?.attributes?.friendly_name??i):""}function solarSourceName(e,t){const i=e._energyDefaults.solarStatEnergyFroms[t];return i?String(e.hass?.states?.[i]?.attributes?.friendly_name??i):`PV ${t+1}`}var wt={production:"Production",consumption:"Consumption",grid:"Grid",battery:"Battery","battery-soc":"Battery charge",irradiance:"Irradiance"},St={production:"Production",consumption:"Consommation",grid:"Réseau",battery:"Batterie","battery-soc":"Charge batterie",irradiance:"Irradiance"};function targetLabel(e,t){const i=String(e.hass?.language??"").toLowerCase();if(isGroupTarget(t)){const n=groupOfTarget(t);return monitoringGroupName(e.config,n)||`${i.startsWith("fr")?"Groupe":"Group"} ${n}`}return(i.startsWith("fr")?St:wt)[t]}var chartIsDark=e=>!!e.hass?.themes?.darkMode,xt=96;function slotOf(e){return Math.min(95,Math.floor(4*serverHourFrac(e)))}function fillGaps(e){const t=e.length;if(!e.some(e=>Number.isFinite(e)))return new Array(t).fill(0);const i=e.slice();for(let n=0;n<t;n++){if(Number.isFinite(i[n]))continue;let r=1;for(;!Number.isFinite(e[((n-r)%t+t)%t]);)r++;let s=1;for(;!Number.isFinite(e[(n+s)%t]);)s++;const l=e[((n-r)%t+t)%t];i[n]=l+(e[(n+s)%t]-l)*(r/(r+s))}return i}function expandHourly(e,t){const i=new Array(96);for(let n=0;n<96;n++){const r=Math.max(0,e[Math.floor(n/4)]??0);i[n]=t?r/4:r}return i}function binSlotSum(e,t){const i=new Array(xt).fill(0),n=e.stepMs/me,r=9e5;for(let s=0;s<e.bucketsTotal;s++){const l=t[s];if(null===l||!isFinite(l))continue;const d=Math.max(0,l)*n/1e3,c=e.storeStartMs+s*e.stepMs,u=c+e.stepMs;for(let t=c;t<u;){const n=Math.floor(t/r)*r+r,s=Math.min(u,n);i[slotOf(t)]+=d*((s-t)/e.stepMs),t=s}}return i}function buildPeriodData(e,t){if(e._periodHourly)return function buildPeriodDataHourly(e,t){const data=(e,t)=>({unit:e,layers:t}),oneE=e=>({values:expandHourly(e,!0)}),oneA=e=>({values:expandHourly(e,!1)});switch(e){case"production":return data("energy",t.pv.map(e=>oneE(e)));case"consumption":return data("energy",[oneE(t.consumption)]);case"grid":return data("energy",[oneE(t.gridImport),oneE(t.gridExport)]);case"battery":return data("energy",[oneE(t.batteryDischarge),oneE(t.batteryCharge)]);case"battery-soc":return data("percent",[oneA(t.soc)]);default:return data("energy",[])}}(t,e._periodHourly);const i=e._unifiedStore,data=(e,t)=>({unit:e,layers:t});if(modeBucketsPerHour(e._timelineMode,e.config)<1)return data("energy",[]);if("production"===t){if(!i)return data("energy",[]);const t=Date.now(),n=i.stepMs/me,r=9e5,s=e._energyDefaults.solarStatEnergyFroms,l=s.length>=2&&s.every(t=>e._pvChangeSeriesPerEntity.has(t)),d=l?s:s.slice(0,1),c=l?s.map(n=>changeSeriesToWatts(e._pvChangeSeriesPerEntity.get(n)??null,i.storeStartMs,i.stepMs,i.bucketsTotal,t)):1===s.length?[changeSeriesToWatts(e._pvChangeSeries,i.storeStartMs,i.stepMs,i.bucketsTotal,t)]:[],u=d.map(()=>new Array(xt).fill(0));for(let e=0;e<i.bucketsTotal&&!(i.storeStartMs+(e+.5)*i.stepMs>t);e++){const t=i.storeStartMs+e*i.stepMs,s=t+i.stepMs;for(let l=0;l<d.length;l++){const d=c[l]?.[e];if(null==d||!(d>0))continue;const p=d*n/1e3;for(let e=t;e<s;){const t=Math.floor(e/r)*r+r,n=Math.min(s,t);u[l][slotOf(e)]+=p*((n-e)/i.stepMs),e=n}}}return data("energy",d.map((e,t)=>({values:u[t]})))}if("battery-soc"===t){const t=e._batterySocHistory,i=new Array(xt).fill(0),n=new Array(xt).fill(0);if(t)for(let e=0;e<t.times.length;e++){const r=t.values[e];if(!isFinite(r))continue;const s=slotOf(t.times[e].getTime());i[s]+=r,n[s]+=1}return data("percent",[{values:fillGaps(i.map((e,t)=>n[t]?e/n[t]:NaN))}])}if(!i)return data("energy",[]);let n;if("grid"===t)n=[{series:i.gridImport},{series:i.gridExport}];else if("battery"===t){const e=i.battery.map(e=>null===e?null:Math.max(0,e));n=[{series:i.battery.map(e=>null===e?null:Math.max(0,-e))},{series:e}]}else{const e=new Array(i.bucketsTotal).fill(null);for(let t=0;t<i.bucketsTotal;t++){const n=i.production[t],r=i.gridImport[t],s=i.gridExport[t],l=i.battery[t];null===n&&null===r&&null===s&&null===l||(e[t]=consumptionLoad(n??0,r??0,s??0,l??0))}n=[{series:e}]}return data("energy",n.map(e=>({values:binSlotSum(i,e.series)})))}function hourlyOf(e,t){const i=new Array(24).fill(0);for(let n=0;n<24;n++){let r=0;for(let t=0;t<4;t++)r+=Math.max(0,e[4*n+t]??0);i[n]=t?r:r/4}return i}function layerPeriodTotal(e,t){const i="energy"===t.unit,n=hourlyOf(e.values,i);let r=0;for(let s=0;s<24;s++)r+=Math.max(0,n[s]);return i?r:r/24}function formatClock(e,t){const i=e?.locale?.language??"en";return t.toLocaleTimeString(i,{hour:"2-digit",minute:"2-digit"})}function formatDayLength(e){const t=Math.max(0,Math.round(e/6e4));return`${Math.floor(t/60)}h${String(t%60).padStart(2,"0")}`}function buildMetrics(e,t){const i=e._timeRange;if(!i)return[];const n=i.start.getTime(),r=i.end.getTime();if(r<=n)return[];const s=e.hass,l=valueDecimals(e.config),d=powerUnit(e.config),c=irradianceUnit(e.config),u=function windowDays(e,t){return Math.max(1,Math.round((t-e)/fe))}(n,r),energy=e=>formatEnergyKwh(s,e,l,d);if("irradiance"===t){const t=e._unifiedStore,irr=e=>formatIrradiance(s,e,l,c),i=[];if(t){const e=function aggWatts(e,t,i,n){let r=0,s=0,l=0;for(let d=0;d<t.length;d++){const c=t[d];if(null===c||!isFinite(c))continue;const u=e.storeStartMs+(d+.5)*e.stepMs;u<i||u>n||(c>r&&(r=c),s+=c,l++)}return{peak:r,avg:l?s/l:0,count:l}}(t,t.irradiance,n,r);i.push({icon:"mdi:trending-up",value:irr(e.peak)},{icon:"mdi:approximately-equal",value:irr(e.avg)})}const d=e._sunScene;if(d&&d.sunrise&&d.sunset){const e=d.sunrise.time,t=d.sunset.time,n=/* @__PURE__ */new Date((e.getTime()+t.getTime())/2);let r=0;for(const i of d.arc)i.altitude>r&&(r=i.altitude);i.push({icon:"mdi:weather-sunset-up",value:formatClock(s,e)},{icon:"mdi:weather-sunny",value:formatClock(s,n)},{icon:"mdi:weather-sunset-down",value:formatClock(s,t)},{icon:"mdi:angle-acute",value:`${Math.round(r)}°`},{icon:"mdi:timelapse",value:formatDayLength(t.getTime()-e.getTime())})}return i}if(isGroupTarget(t))return groupDevices(e.config,e._energyDefaults,groupOfTarget(t)).map(t=>({label:deviceName(e.hass,t),value:energy(deviceWindowKwh(e._deviceChangeSeries.get(t.statConsumption),n,r))}));const p=buildPeriodData(e,t);if("battery-soc"===t){const e=function socStats(e){const t=e.layers[0];if(!t)return null;const i=hourlyOf(t.values,!1);let n=1/0,r=0,s=0,l=0;for(const d of i)isFinite(d)&&(d<n&&(n=d),d>r&&(r=d),s+=d,l++);return l?{min:n,avg:s/l,max:r}:null}(p);if(!e)return[];const pct=e=>`${Math.round(e)} %`;return[{icon:"mdi:arrow-down",value:pct(e.min)},{icon:"mdi:approximately-equal",value:pct(e.avg)},{icon:"mdi:arrow-up",value:pct(e.max)}]}if(!p.layers.length)return[];if("grid"===t){const e=layerPeriodTotal(p.layers[0],p),t=p.layers[1]?layerPeriodTotal(p.layers[1],p):0,i=e-t,n=`${i<0?"-":""}${energy(Math.abs(i))}`;return[{icon:"mdi:transmission-tower-import",value:energy(e)},{icon:"mdi:transmission-tower-export",value:energy(t)},{icon:"mdi:scale-balance",value:n},{icon:"mdi:calendar-today",value:energy(e/u)}]}if("battery"===t){const e=layerPeriodTotal(p.layers[0],p);return[{icon:"mdi:battery-arrow-down",value:energy(p.layers[1]?layerPeriodTotal(p.layers[1],p):0)},{icon:"mdi:battery-arrow-up",value:energy(e)}]}const g=function periodTotal(e){return e.layers.reduce((t,i)=>t+layerPeriodTotal(i,e),0)}(p);return[{icon:"mdi:sigma",value:energy(g)},{icon:"mdi:calendar-today",value:energy(g/u)}]}function tick(e){const t=/* @__PURE__ */new Date,i=e._now;if(i&&t.getMinutes()===i.getMinutes()&&t.getHours()===i.getHours()&&t.getDate()===i.getDate()&&t.getMonth()===i.getMonth()&&t.getFullYear()===i.getFullYear())return;const n=!i||t.getDate()!==i.getDate()||t.getMonth()!==i.getMonth()||t.getFullYear()!==i.getFullYear();if(e._now=t,n&&e._engine){const t=e._engine.getTimelineRange();t&&(e._timeRange=t),e._chartSeries=e._engine.getTimelineSeries()??e._chartSeries}refreshHud(e)}function applyTimelinePointer(e,t){if(!e._timeRange)return;const i=t.currentTarget.getBoundingClientRect(),n=Math.max(0,Math.min(1,(t.clientX-i.left)/i.width)),r=e._timeRange.end.getTime()-e._timeRange.start.getTime(),s=e._timeRange.start.getTime()+n*r,l=Date.now(),d=e._timeRange.start.getTime(),c=e._timeRange.end.getTime();if(l>=d&&l<=c){const n=(l-d)/r,s=i.left+n*i.width,c=t.clientX;if(Math.abs(c-s)<=8)return void(e._isLiveMode&&null===e._selectedTime||(e._selectedTime=null,e._isLiveMode=!0,e._chartHoverPct=null,e._engine?.setSelectedTime(null)))}const u=new Date(s);e._selectedTime&&e._selectedTime.getTime()===u.getTime()||(e._selectedTime=u,e._isLiveMode=!1,e._chartHoverPct=100*n,e._engine?.setSelectedTime(u))}function nextGuardState(e,t){const i=function evaluateGuardHours(e){const t=new Array(e.length).fill(!1);let i=0;for(let l=0;l<e.length;l++){const{exportKwh:n,minW:r}=e[l];null!==n&&null!==r&&(n<=.1||n>=25||(i++,r>-Math.max(50,.2*n*1e3)&&(t[l]=!0)))}const n=[...t];for(let l=0;l<e.length;l++){if(!n[l])continue;const exculpates=t=>{if(t<0||t>=e.length||n[t])return!1;const i=e[t].minW;return null!==i&&i<-50};(exculpates(l-1)||exculpates(l+1))&&(t[l]=!1)}let r=0,s=-2;for(let l=0;l<e.length;l++)t[l]&&l>s+1&&(r++,s=l);return{contradictions:r,realExportHours:i}}(t);if("flagged"!==e.status)return i.contradictions>=3?{...e,status:"flagged",cleanEvals:0}:{...e,status:"healthy"};if(i.contradictions>0)return{...e,cleanEvals:0};if(0===i.realExportHours)return e;const n=e.cleanEvals+1;return n>=3?{...e,status:"healthy",cleanEvals:0}:{...e,cleanEvals:n}}function refreshGridGuard(e){const t=e._energyDefaults,i=t?.gridStatRates??[],n=t?.gridStatEnergyTos??[],r=`${[...i].sort().join(",")}|${[...n].sort().join(",")}`,s=e._gridGuard;if(r!==s.entityKey)return void(e._gridGuard={status:"unknown",cleanEvals:0,fetchKey:"",fetching:!1,entityKey:"",entityKey:r});if(1!==i.length||0===n.length||!e.hass?.callWS)return;if(s.fetching)return;const l=`${r}|${Math.floor(Date.now()/He)*He}`;if(l===s.fetchKey)return;s.fetchKey=l,s.fetching=!0;const d=Date.now(),c=d-864e5,u=i[0],p=t?.invertedRateEntities.includes(u)??!1;Promise.all([callWS(e.hass,{type:"recorder/statistics_during_period",start_time:new Date(c).toISOString(),end_time:new Date(d).toISOString(),statistic_ids:n,period:"hour",types:["change"],units:{energy:"kWh"}}),callWS(e.hass,{type:"recorder/statistics_during_period",start_time:new Date(c).toISOString(),end_time:new Date(d).toISOString(),statistic_ids:[u],period:"hour",types:["min","max"],units:{power:"W"}})]).then(([t,i])=>{const s=function buildGuardHours(e,t,i,n,r){const s=/* @__PURE__ */new Map,slot=e=>{let t=s.get(e);return t||(t={exportKwh:null,minW:null},s.set(e,t)),t};for(const l of i)for(const t of e?.[l]??[]){const e=parseStatBoundary(t?.start),i="number"==typeof t?.change&&Number.isFinite(t.change)?t.change:null;if(null===e||null===i)continue;const n=slot(e);n.exportKwh=(n.exportKwh??0)+i}for(const l of t?.[n]??[]){const e=parseStatBoundary(l?.start);if(null===e)continue;const t=r?"number"==typeof l?.max&&Number.isFinite(l.max)?-l.max:null:"number"==typeof l?.min&&Number.isFinite(l.min)?l.min:null;null!==t&&(slot(e).minW=t)}return[...s.entries()].sort((e,t)=>e[0]-t[0]).map(([,e])=>e)}(t,i,n,u,p),d=nextGuardState(e._gridGuard,s);if(d!==e._gridGuard){const t=d.status!==e._gridGuard.status;d.fetchKey=l,d.entityKey=r,e._gridGuard=d,t&&e.requestUpdate()}}).catch(()=>{}).finally(()=>{e._gridGuard.fetching=!1})}function applyGridSlot(e,t,i,n){const r=null===i?null:Math.max(0,i);"import"===t?(e._gridImportValue!==r&&(e._gridImportValue=r),e._gridImportUnit!==n&&(e._gridImportUnit=n)):(e._gridExportValue!==r&&(e._gridExportValue=r),e._gridExportUnit!==n&&(e._gridExportUnit=n))}function applyGridSplit(e,t){t>=0?(applyGridSlot(e,"import",t,"W"),applyGridSlot(e,"export",null,"")):(applyGridSlot(e,"import",null,""),applyGridSlot(e,"export",-t,"W"))}function refreshGrid(e){if(!e.hass)return null!==e._gridImportValue&&(e._gridImportValue=null),""!==e._gridImportUnit&&(e._gridImportUnit=""),null!==e._gridExportValue&&(e._gridExportValue=null),void(""!==e._gridExportUnit&&(e._gridExportUnit=""));fetchGridChangeSeries(e,"import"),fetchGridChangeSeries(e,"export"),refreshGridGuard(e);const t=e._energyDefaults?.gridStatRates??[];t.length>0&&"flagged"!==e._gridGuard.status?function readStatRates(e,t){const{watts:i,any:n}=sumLiveWatts(e.hass,t,e._energyDefaults?.invertedRateEntities);if(!n)return;applyGridSplit(e,i)}(e,t):(applyGridSlot(e,"import",null,""),applyGridSlot(e,"export",null,""))}function fetchGridChangeSeries(e,t){const i=e._energyDefaults;if(!i)return;const n="import"===t?i.gridStatEnergyFroms:i.gridStatEnergyTos;if(0===n.length)return;const r=localMidnightMinusDays(e._periodPastDays),s=changeRefreshAnchorMs(),l=[...unionChangeMeters(i)].sort(),d=`${l.join(",")}|${r}|${s}`;("import"===t?e._gridImportFetch:e._gridExportFetch).run(d,()=>fetchChangeById(e.hass,l,r,s,e._storeFetchPeriod).then(i=>{if(null===i)return;const r=mergeChangeSeries(i,n);null!==r&&("import"===t?e._gridImportChangeSeries=r:e._gridExportChangeSeries=r),e.requestUpdate()}))}function formatGridValue(e,t,i,n,r="kW"){return null===t?"":formatEntityValue(e,t,i,n,r)}var Ht=/* @__PURE__ */new Set;{const e="__heliosBannerPrinted",t=window;t[e]||(t[e]=!0,console.info("%c☀ HELIOS%c v2026.8.0-b2","background:#f59e0b;color:#1f2937;padding:2px 8px;border-radius:4px 0 0 4px;font-weight:bold;","background:#1f2937;color:#f59e0b;padding:2px 8px;border-radius:0 4px 4px 0;font-weight:bold;"))}{const e=window;e.setHeliosLocation||(e.setHeliosLocation=(t,i)=>{if(!("number"!=typeof t||"number"!=typeof i||!isFinite(t)||!isFinite(i)||t<-90||t>90||i<-180||i>180)){e.__heliosLocationOverride={lat:t,lon:i};for(const e of Ht)e.invalidateLocation()}}),e.clearHeliosLocation||(e.clearHeliosLocation=()=>{if(e.__heliosLocationOverride){e.__heliosLocationOverride=void 0;for(const e of Ht)e.invalidateLocation()}})}window.addEventListener("helios-data-cache-reset",()=>{for(const e of Ht)e.resetDataCache()});var jt,zt=1.5,Mt=class{constructor(e){this.host=e,this._gridLeaderColor="var(--energy-grid-consumption-color, #488fc2)",this._batteryLeaderColor="var(--energy-battery-out-color, #4db6ac)"}_nudgeToHomePill(e,t,i,n){return function nudgeToHomePill(e,t,i,n,r,s){const l=e-i,d=t-n,c=Math.max(0,r-s);if(Math.abs(l)<=c)return{x:e,y:n+(d>=0?1:-1)*s};const u=i+(l>=0?1:-1)*c,p=e-u,g=t-n,m=Math.sqrt(p*p+g*g)||1;return{x:u+s*p/m,y:n+s*g/m}}(e,t,i,n,38,14)}_buildLPathToHome(e,t,i,n){if(!e)return"";const r=e.home.x,s=e.home.y,l=r>t?1:-1,d=s>i?1:-1,c=t+l*n,u=i,p=r-13*l,g=s-14*d,m=Math.min(12,Math.abs(p-c)/2,Math.abs(g-u)/2),f=p-l*m,v=u+d*m;return`M ${c.toFixed(1)},${u.toFixed(1)} L ${f.toFixed(1)},${u.toFixed(1)} Q ${p.toFixed(1)},${u.toFixed(1)} ${p.toFixed(1)},${v.toFixed(1)} L ${p.toFixed(1)},${g.toFixed(1)}`}_buildLPath(e,t,i,n,r){const s=i>e?1:-1,l=n>t?1:-1,d=Math.min(12,Math.abs(i-e)/2,Math.abs(n-t)/2);if(r){const r=n-l*d,c=e+s*d;return`M ${e.toFixed(1)},${t.toFixed(1)} L ${e.toFixed(1)},${r.toFixed(1)} Q ${e.toFixed(1)},${n.toFixed(1)} ${c.toFixed(1)},${n.toFixed(1)} L ${i.toFixed(1)},${n.toFixed(1)}`}const c=i-s*d,u=t+l*d;return`M ${e.toFixed(1)},${t.toFixed(1)} L ${c.toFixed(1)},${t.toFixed(1)} Q ${i.toFixed(1)},${t.toFixed(1)} ${i.toFixed(1)},${u.toFixed(1)} L ${i.toFixed(1)},${n.toFixed(1)}`}_renderSunCrossing(e,t,i,n){if(!e)return K;const r=e.x-t.x,s=e.y-t.y,l=Math.hypot(r,s)||1,d=e.x+r/l*22,c=e.y+s/l*22,u=function formatHaTime(e,t){const i=e?.locale,n={hour:"numeric",minute:"2-digit",hour12:haUseAmPm(i)};try{return new Intl.DateTimeFormat(i?.language,n).format(t)}catch(L){return new Intl.DateTimeFormat(void 0,n).format(t)}}(this.host.hass,e.time);return B`
        ...
        `}render(){const e=null!==getHomeCoords(this.host.config,this.host.hass),t=!1!==this.host._interactive,i=this.host._labelLayout,n=resolvePvLiveEntity(this.host._energyDefaults),r=ENERGY_COLOR_pv(this.host),s=!this.host._isLiveMode&&null!==this.host._selectedTime,l=s&&this.host._selectedTime.getTime()>Date.now()+6e4,d=""!==n&&null!==i?s?function pvRateAtTime(e,t){const i=wattsAtFromChangeSeries(e._pvChangeSeries,t.getTime());return null===i?null:{value:Math.max(0,i),unit:"W"}}(this.host,this.host._selectedTime):null!==this.host._pvCurrent?currentPvRate(this.host):null:null;let c=null;if(l&&""!==n&&null!==i&&this.host._unifiedStore){const e=valueAt(this.host._unifiedStore.forecast,this.host._unifiedStore,this.host._selectedTime.getTime());null!==e&&e>0&&(c={value:e,unit:"W"})}const u=l&&null!==c,p=u?c:d,g=e&&null!==i&&""!==n&&null!==p&&(!l||u)&&(!s||p.value>0),m=valueDecimals(this.host.config),f=powerUnit(this.host.config),v=irradianceUnit(this.host.config),b=g?(u?"~ ":"")+formatPvValue(this.host.hass,p.value,p.unit,m,f):"",y=null!==d?pvNormalizeToWatts(d.value,d.unit):0,_=flowDuration(y,5e3,.5),w=!(y>0),H=resolveBatteryEntities(this.host._energyDefaults),j=null!==H.socEntity,z=null!==H.powerEntity,M=!this.host._isLiveMode&&null!==this.host._selectedTime,$=M&&this.host._selectedTime.getTime()>Date.now()+6e4,C=M&&!$?this.host._selectedTime.getTime():null,D=null!==C?wattsAtFromChangeSeries(this.host._gridImportChangeSeries,C):this.host._gridImportValue,A=null!==C?wattsAtFromChangeSeries(this.host._gridExportChangeSeries,C):this.host._gridExportValue,R=null===D?null:Math.max(0,D),E=null===A?null:Math.max(0,A),T=null!==C?"W":this.host._gridImportUnit,O=null!==C?"W":this.host._gridExportUnit,L=M?function batterySampleAtTime(e,t){if(!e||0===e.times.length)return null;const i=t.getTime(),n=e.times[0].getTime(),r=e.times[e.times.length-1].getTime();if(i<n||i>r+6e4)return null;let s=e.times.length-1;for(let l=0;l<e.times.length;l++)if(e.times[l].getTime()>i){s=l-1;break}return s<0&&(s=0),e.values[s]}(this.host._batterySocHistory,this.host._selectedTime):this.host._batterySoc;let P;if(M){const e=this.host._selectedTime.getTime(),t=wattsAtFromChangeSeries(this.host._batteryChargeChangeSeries,e),i=wattsAtFromChangeSeries(this.host._batteryDischargeChangeSeries,e);P=null===t&&null===i?null:Math.max(0,t??0)-Math.max(0,i??0)}else P=this.host._batteryPower;const F=M?"W":this.host._batteryPowerUnit,I=e&&null!==i&&!$&&j&&null!==L,U=e&&null!==i&&!$&&z&&null!==P,W=I||U,G=I?`${Math.round(L)} %`:"",Y=U?function formatBatteryPower(e,t,i,n,r="kW",s="default"){const l=pvNormalizeToWatts(t,i);return"hidden"===s?formatPowerKw(e,Math.abs(l),n,!1,r):formatPowerKw(e,"inverted"===s?-l:l,n,!0,r)}(this.host.hass,-P,F,m,f,function batterySign(e){const t=e?.["battery-sign"];return"inverted"===t||"hidden"===t?t:"default"}(this.host.config)):"",Z=l||null===p?null:pvNormalizeToWatts(p.value,p.unit),J=null!==R||null!==E?(R??0)-(E??0):null,X=U?P:null,Q=this.host._energyDefaults,ee=M||(0===Q.solarStatEnergyFroms.length||null!==Z)&&(0===Q.gridStatEnergyFroms.length&&0===Q.gridStatEnergyTos.length||null!==J)&&(0===Q.batteryStatEnergyFroms.length&&0===Q.batteryStatEnergyTos.length||null!==X)?null===Z&&null===J&&null===X?null:Math.max(0,(Z??0)+(J??0)-(X??0)):null,te=e&&null!==i&&!$&&null!==ee,ie=te?formatGridValue(this.host.hass,ee,"W",m,f):"",ae=U&&P>0,ne=U&&P<0,oe=ae?"var(--energy-battery-in-color, #f06292)":"var(--energy-battery-out-color, #4db6ac)";this._batteryLeaderColor=oe;const re=U?Math.abs(pvNormalizeToWatts(P,F)):0,se=U&&re<5,le=flowDuration(re,5e3),de=i?.batteryLabel.x??0,ce=i?.batteryLabel.y??0,ue=function batteryLevelIcon(e,t){if(null===e||!isFinite(e))return"mdi:battery";const i=Math.max(0,Math.min(100,10*Math.round(e/10)));return t?i>=100?"mdi:battery-charging-100":i<=0?"mdi:battery-charging-outline":`mdi:battery-charging-${i}`:i>=100?"mdi:battery":i<=0?"mdi:battery-outline":`mdi:battery-${i}`}(I?L:null,ae),he=U?Y:G,pe=ce-4.2,ge=!!(i&&ae&&g)?ce+4.2:ce,me=i&&W&&ne?this._buildLPathToHome(i,de,ge,22):"",fe=i&&W&&!ne?this._buildLPathToHome(i,de,ge,22):"",ve=i&&ae&&g?this._buildLPath(i.pvLabel.x+14,i.pvLabel.y+11,de-30,pe,!0):"",be=this._buildLPathToHome(i,i?.gridLabel.x??0,i?.gridLabel.y??0,22),ye=[.32,.44,.6799999999999999,.56],ke=this.host._isLiveMode||null===this.host._selectedTime?null:this.host._selectedTime.getTime(),_e=i?function activeGroups(e,t){const i=[];for(let n=1;n<=4;n++)groupDevices(e,t,n).length>0&&i.push(n);return i}(this.host.config,this.host._energyDefaults).map(e=>{const t=i.groupLabels[e-1],n=e<=2?t.x+48:t.x-48,r=i.home.x+96*(ye[e-1]-.5),s=i.home.y+14,l=this._buildLPath(r,s,n,t.y,!0),d=null!==ke?function groupPowerWAt(e,t,i){let n=0,r=!1;for(const s of groupDevices(e.config,e._energyDefaults,t)){const t=wattsAtFromChangeSeries(e._deviceChangeSeries.get(s.statConsumption)??null,i);null!==t&&(n+=t,r=!0)}return r?n:null}(this.host,e,ke):function groupLivePowerW(e,t){const i=groupDevices(e.config,e._energyDefaults,t).map(e=>e.statRate).filter(e=>""!==e);if(0===i.length)return null;const{watts:n,any:r}=sumLiveWatts(e.hass,i);return r?n:null}(this.host,e),c=monitoringGroupColor(this.host.config,e),u=monitoringGroupIcon(this.host.config,e),p=null===d?0:Math.abs(d);return{g:e,anchor:t,leadPath:l,watts:d,color:c,icon:u,beadDur:p<5?null:flowDuration(p,5e3)}}):[],we=null!==R?Math.abs(pvNormalizeToWatts(R,T)):0,Se=null!==E?Math.abs(pvNormalizeToWatts(E,O)):0,proportionalBeadDur=(e,t)=>{const i=Math.max(e,1);return Math.min(8,Math.max(1.2,1.2*t/i))},xe=we<5?null:proportionalBeadDur(we,5e3),He=Se<5?null:proportionalBeadDur(Se,1e3),je=(R??0)>=(E??0),ze=je?"var(--energy-grid-consumption-color, #488fc2)":"var(--energy-grid-return-color, #8353d1)";this._gridLeaderColor=ze;const Me=je?xe:He,$e=this.host._sunScene,Ce=e&&null!==$e&&$e.arc.length>=2,De=ENERGY_COLOR_sun(this.host),Ae=function darkenHex(e,t){const i=1-Math.max(0,Math.min(1,t)),n=Math.round(parseInt(e.slice(1,3),16)*i),r=Math.round(parseInt(e.slice(3,5),16)*i),s=Math.round(parseInt(e.slice(5,7),16)*i),h=e=>e.toString(16).padStart(2,"0");return`#${h(n)}${h(r)}${h(s)}`}(De,.2),Re=Ce?function buildArcSegments(e,t){const i=[];for(let n=0;n<e.length-1;n++){const r=e[n],s=e[n+1];i.push({x1:r.x,y1:r.y,x2:s.x,y2:s.y,color:arcColor(.5*(r.altitude+s.altitude),t),nearness:.5*(r.nearness+s.nearness),belowHorizon:r.belowHorizon||s.belowHorizon})}return i}($e.arc,De):[],Ee=this.host._arcBackBuf,Te=this.host._arcFrontBuf,Oe=this.host._arcFrontNearBuf;Ee.length=0,Te.length=0,Oe.length=0;for(const B of Re)B.belowHorizon?Ee.push(B):B.nearness>=.5?Oe.push(B):Te.push(B);const Le=Ce&&$e.sun.altitude>0,Pe=$e?.sun.irradiance??0,Fe=formatIrradiance(this.host.hass,Pe,m,v),Ie=Math.sqrt(Math.max(0,Math.min(1,Pe/1e3))),Ue=Ce&&$e.sun.altitude>0&&this.host._weatherAvailable,We=flowDuration(Pe,1e3,.8);let Ne=$e?.home.x??0,Be=$e?.home.y??0;if(i&&$e&&g){const e=i.pvLabel.x,t=i.pvLabel.y,n=28,r=11,s=$e.sun.x-e,l=$e.sun.y-t,d=Math.max(0,n-r);if(Math.abs(s)<=d)Ne=$e.sun.x,Be=t+(l>=0?1:-1)*r;else{const i=e+(s>=0?1:-1)*d,n=t,l=$e.sun.x-i,c=$e.sun.y-n,u=Math.sqrt(l*l+c*c)||1;Ne=i+r*l/u,Be=n+r*c/u}}return B`
                ...
                ${Ce&&Ee.length>0?B`
                    <svg
                        ...
                        style="--solar-daylight:${$e.daylight}"
                    >
                        ${Ee.map(e=>q`
                            <line
                                ...
                                x1="${e.x1}" y1="${e.y1}"
                                x2="${e.x2}" y2="${e.y2}"
                                stroke-width="${.5*(zt+3.5*e.nearness)}"
                            ></line>
                        `)}
                        ${Ee.map(e=>q`
                            <line
                ...
                ${g?(()=>{const e=i.pvLabel.x,t=i.pvLabel.y+11,n=this._nudgeToHomePill(e,t,i.home.x,i.home.y);return B`
                ...
                ${g?B`
                    <div
                        class="pv-pct-label ${u?"is-predicted":""} ${t&&"production"===this.host._chartTarget?"is-chart-active":""}"
                        style="left:${i.pvLabel.x}px; top:${i.pvLabel.y}px; --pv-leader-color:${r}"
                        role=${t?"button":K}
                        tabindex=${t?"0":K}
                ...
                ${W?B`
                    ...
                    <div
                        ...
                        style="left:${de}px; top:${ce}px; --battery-leader-color:${oe}"
                        role=${t?"button":K}
                        tabindex=${t?"0":K}
                        data-target="battery"
                        @click=${t?this.host.onChartTargetClick:void 0}
                ...
                ${!e||null===i||null===R&&null===E||$?K:B`
                    <svg class="grid-leader-svg">
                        <path class="grid-leader-line" style="stroke:${ze}" d=${be} />
                        ...
                        ${null!==Me?je?q`
                            <circle class="grid-leader-bead" r="3" style="fill:${ze}">
                                <animateMotion dur="${Me.toFixed(2)}s" repeatCount="indefinite"
                                               path="${be}" />
                            </circle>
                        `:q`
                            <circle class="grid-leader-bead" r="3" style="fill:${ze}">
                    ...
                    <div
                        ...
                        style="left:${i.gridLabel.x}px; top:${i.gridLabel.y}px; --grid-leader-color:${ze}"
                        role=${t?"button":K}
                        tabindex=${t?"0":K}
                        data-target="grid"
                        @click=${t?this.host.onChartTargetClick:void 0}
                    >
                        <ha-icon icon=${je?"mdi:transmission-tower-export":"mdi:transmission-tower-import"}></ha-icon>
                        <span>${formatGridValue(this.host.hass,je?R??0:E??0,je?T:O,m,f)}</span>
                    </div>
                `}
                ...
                ${e&&null!==i?_e.map(e=>B`
                    ...
                    >
                        ...
                        <span>${null===e.watts?"":formatPvValue(this.host.hass,e.watts,"W",m,f)}</span>
                    </div>
                `):K}
                    ...
                    >
                        ${Te.map(e=>q`
                            <line
                                ...
                                x1="${e.x1}" y1="${e.y1}"
                                x2="${e.x2}" y2="${e.y2}"
                                stroke-width="${zt+3.5*e.nearness}"
                            ></line>
                        `)}
                        ${Te.map(e=>q`
                            <line
                ...
                ${Ce&&Le?B`
                    <svg class="solar-svg solar-ray-svg"
                        ...
                        <line
                            class="solar-ray"
                            style="--sun-flow-duration:${We}s"
                            x1=${$e.sun.x}  y1=${$e.sun.y}
                            x2=${Ne}    y2=${Be}
                            stroke=${De}
                        ></line>
                        ...
                        >
                            <animateMotion
                                dur="${We}s"
                                repeatCount="indefinite"
                                path="M ${$e.sun.x},${$e.sun.y} L ${Ne},${Be}"
                            ></animateMotion>
                        </circle>
                    </svg>
                `:K}
                ...
                ${Ce?B`
                    <svg
                        class="solar-svg solar-svg-sun ${$e.sun.nearness>=.5?"solar-svg-sun-near":"solar-svg-sun-far"}"
                        style="--solar-daylight:${$e.daylight}"
                    >
                        ${(()=>{const e=this.host._engine?.getSunArcScale()??1,t=Math.min((10+10*$e.sun.nearness)*e,22),i=t*Ie,n=3*t,r=.55*Ie;return q`
                                <defs>
                                    <radialGradient id="solar-halo-grad-${this.host._instanceId}">
                                        <stop offset="0%"   stop-color="${De}" stop-opacity="${r}"></stop>
                                        <stop offset="100%" stop-color="${De}" stop-opacity="0"></stop>
                                    </radialGradient>
                                </defs>
                                <circle
                                    class="solar-sun-halo"
                                    cx="${$e.sun.x}" cy="${$e.sun.y}"
                                    r="${n}"
                ...
                ${Ue?B`
                    <div
                        ...
                        style="left:${$e.sun.x}px; top:${$e.sun.y-22}px"
                        role=${t?"button":K}
                        tabindex=${t?"0":K}
                ...
                ${Ce&&$e?B`
                    ${this._renderSunCrossing($e.sunrise,$e.home,"mdi:weather-sunset-up",De)}
                    ${this._renderSunCrossing($e.sunset,$e.home,"mdi:weather-sunset-down",De)}
                `:K}
                ...
                ${e&&null!==i?B`
                    <div
                        class="home-pill ${te?"has-usage":""} ${this.host._homeHover?"is-hovered":""} ${t&&"consumption"===this.host._chartTarget?"is-chart-active":""}"
                    ...
                    >
                        ...
                        ${te?B`<span class="home-pill-usage">${ie}</span>`:K}
                    </div>
                `:K}
        `}};function __decorateMetadata(e,t){if("object"==typeof Reflect&&"function"==typeof Reflect.metadata)return Reflect.metadata(e,t)}function __decorate(e,t,i,n){var r,s=arguments.length,l=s<3?t:null===n?n=Object.getOwnPropertyDescriptor(t,i):n;if("object"==typeof Reflect&&"function"==typeof Reflect.decorate)l=Reflect.decorate(e,t,i,n);else for(var d=e.length-1;d>=0;d--)(r=e[d])&&(l=(s<3?r(l):s>3?r(t,i,l):r(t,i))||l);return s>3&&l&&Object.defineProperty(t,i,l),l}var $t=((jt=class HeliosMiniCard extends le{constructor(...e){super(...e),this.preview=!1,this._interactive=!1,this._hud=new Mt(this),this._now=/* @__PURE__ */new Date,this._cloudCover=-1,this._labelLayout=null,this._sunScene=null,this._pvCurrent=null,this._pvUnit="",this._batterySoc=null,this._batteryPower=null,this._batteryPowerUnit="",this._gridImportValue=null,this._gridImportUnit="",this._gridExportValue=null,this._gridExportUnit="",this._energyDefaults=Xe,this._energyDefaultsLoaded=!1,this._homeHover=!1,this._selectedTime=null,this._isLiveMode=!0,this._timeRange=null,this._chartSeries=null,this._periodPastDays=2,this._periodFutureDays=2,this._unifiedStore=null,this._pvChangeSeries=null,this._gridImportChangeSeries=null,this._gridExportChangeSeries=null,this._batteryChargeChangeSeries=null,this._batteryDischargeChangeSeries=null,this._batterySocHistory=null,this._deviceChangeSeries=/* @__PURE__ */new Map,this._chartTarget="production",this._weatherAvailable=!0,this._instanceId=`h${Math.floor(1e9*Math.random()).toString(36)}`,this._arcBackBuf=[],this._arcFrontBuf=[],this._arcFrontNearBuf=[],this._lastHomeKey="",this._lastConfigSig="",this._initInflight=!1,this._homeColorToken="",this._lastRefreshHassRef=void 0,this._lastRefreshConfigSig=void 0,this._lastRefreshEnergyDefaultsRef=void 0,this.onHomeEnter=()=>{this._homeHover=!0},this.onHomeLeave=()=>{this._homeHover=!1}}setConfig(e){if(!e)throw new Error("Invalid HELIOS configuration");this.config={...e}}static getConfigElement(){return document.createElement("helios-card-editor")}static getStubConfig(e,t){if(e&&Array.isArray(t)&&t.length>0)for(const i of t){if("string"!=typeof i||!i.startsWith("zone."))continue;const t=e.states?.[i],n=t?.attributes?.latitude,r=t?.attributes?.longitude;if("number"==typeof n&&Number.isFinite(n)&&"number"==typeof r&&Number.isFinite(r))return{"home-latitude":n,"home-longitude":r}}return{}}getCardSize(){return 10}getGridOptions(){return{rows:6,columns:12,min_rows:4,max_rows:24,min_columns:12,max_columns:12}}invalidateLocation(){this._lastHomeKey="",this.requestUpdate()}resetDataCache(){this._engine?.resetDataCache(),this._engine?.forceBuildingsRefetch(),this.requestUpdate()}effectiveCacheId(){return cacheId(this.config)}themeIsDark(){const e=this.hass?.themes;return e&&"boolean"==typeof e.darkMode?e.darkMode:isDarkFromCss(this)}connectedCallback(){super.connectedCallback(),Ht.add(this),void 0!==this._engineTeardownTimer&&(window.clearTimeout(this._engineTeardownTimer),this._engineTeardownTimer=void 0),this._tick(),this._timer=window.setInterval(()=>{this._tick()},3e4),initVisibilityObserver(this),subscribeEnergyPrefs(this)}disconnectedCallback(){super.disconnectedCallback(),Ht.delete(this),window.clearInterval(this._timer),this._visibilityObserver?.disconnect(),this._visibilityObserver=void 0,this._onVisibilityChange&&(document.removeEventListener("visibilitychange",this._onVisibilityChange),this._onVisibilityChange=void 0),unsubscribeEnergyPrefs(this),this._engine&&(this._engine.cacheKey=this.effectiveCacheId()),this._engine?.persistCameraPose(),void 0!==this._engine&&void 0===this._engineTeardownTimer&&(this._engineTeardownTimer=window.setTimeout(()=>{this._engineTeardownTimer=void 0,this._engine?.cleanup(),this._engine=void 0},400)),this._initInflight=!1}willUpdate(e){super.willUpdate(e),e.has("hass")&&setServerTimeZone(this.hass?.config?.time_zone)}updated(e){if(publishConsumptionColor(this),this.hass&&!this._energyPrefsUnsub&&subscribeEnergyPrefs(this),!this.hass?.config||!this.config)return;const t=getHomeCoords(this.config,this.hass);if(!t)return;const{lat:i,lon:n}=t,r=`${i.toFixed(5)},${n.toFixed(5)}`,s=r!==this._lastHomeKey;if(!this._engine){if(!this.isConnected||this._initInflight)return;return this._lastHomeKey=r,this._lastConfigSig=computeConfigSig(this.config),void initEngine(this)}s&&(this._lastHomeKey=r,this._engine.setHome(i,n));const l=computeConfigSig(this.config);l!==this._lastConfigSig&&(this._lastConfigSig=l,this._engine.updateConfig(this.config)),this.hass===this._lastRefreshHassRef&&l===this._lastRefreshConfigSig&&this._energyDefaults===this._lastRefreshEnergyDefaultsRef||(this._lastRefreshHassRef=this.hass,this._lastRefreshConfigSig=l,this._lastRefreshEnergyDefaultsRef=this._energyDefaults,function refreshPvLive(e){if(!e.hass)return;const t=currentPvRate(e);t?(e._pvCurrent!==t.value&&(e._pvCurrent=t.value),e._pvUnit!==t.unit&&(e._pvUnit=t.unit)):null!==e._pvCurrent&&(e._pvCurrent=null,e._pvUnit="")}(this),function refreshBatteryLive(e){if(!e.hass)return;const{powerEntity:t,socEntity:i}=resolveBatteryEntities(e._energyDefaults);if(!t&&!i)return null!==e._batterySoc&&(e._batterySoc=null),null!==e._batteryPower&&(e._batteryPower=null),void(""!==e._batteryPowerUnit&&(e._batteryPowerUnit=""));const{soc:n,power:r,unit:s}=computeBatteryLive(e.hass,e._energyDefaults);n!==e._batterySoc&&(e._batterySoc=n),r!==e._batteryPower&&(e._batteryPower=r),s!==e._batteryPowerUnit&&(e._batteryPowerUnit=s)}(this),function refreshGridLive(e){const t=e.hass?e._energyDefaults?.gridStatRates??[]:[];if(t.length>0){const{watts:i,any:n}=sumLiveWatts(e.hass,t,e._energyDefaults?.invertedRateEntities);if(n)return void applyGridSplit(e,i)}applyGridSlot(e,"import",null,""),applyGridSlot(e,"export",null,"")}(this))}_tick(){const e=/* @__PURE__ */new Date,t=this._now;if(t&&e.getMinutes()===t.getMinutes()&&e.getHours()===t.getHours()&&e.getDate()===t.getDate()&&e.getMonth()===t.getMonth()&&e.getFullYear()===t.getFullYear())return;const i=!t||e.getDate()!==t.getDate()||e.getMonth()!==t.getMonth()||e.getFullYear()!==t.getFullYear();if(this._now=e,i&&this._engine){const e=this._engine.getTimelineRange();e&&(this._timeRange=e),this._chartSeries=this._engine.getTimelineSeries()??this._chartSeries}refreshHud(this)}render(){const e=this._hud.render();return B`
            <ha-card class=${this.themeIsDark()?"theme-dark":"theme-light"}>
                <div id="map-container"></div>
                ${e}
            </ha-card>
        `}}).styles=[Ye],jt);__decorate([n$1({attribute:!1}),__decorateMetadata("design:type",Object)],$t.prototype,"hass",void 0),__decorate([n$1({attribute:!1}),__decorateMetadata("design:type",Object)],$t.prototype,"config",void 0),__decorate([n$1({attribute:!1}),__decorateMetadata("design:type",Object)],$t.prototype,"preview",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],$t.prototype,"_engine",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],$t.prototype,"_cloudCover",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],$t.prototype,"_labelLayout",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],$t.prototype,"_sunScene",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],$t.prototype,"_pvCurrent",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],$t.prototype,"_pvUnit",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],$t.prototype,"_batterySoc",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],$t.prototype,"_batteryPower",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],$t.prototype,"_batteryPowerUnit",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],$t.prototype,"_gridImportValue",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],$t.prototype,"_gridImportUnit",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],$t.prototype,"_gridExportValue",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],$t.prototype,"_gridExportUnit",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],$t.prototype,"_energyDefaults",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],$t.prototype,"_homeHover",void 0),$t=__decorate([t$2("helios-mini-card")],$t);var srgbToLinear=e=>{const t=e/255;return t<=.04045?t/12.92:((t+.055)/1.055)**2.4},linearToSrgb=e=>{const t=e<=.0031308?12.92*e:1.055*e**(1/2.4)-.055;return Math.max(0,Math.min(255,Math.round(255*t)))};function toOklab(e){const t=function parseColor(e){const t=e.trim();if(""===t||"transparent"===t)return null;const i=/^rgba?\(([^)]+)\)$/i.exec(t);if(i){const e=i[1].split(/[,\s/]+/).filter(Boolean);if(e.length<3)return null;const t=parseFloat(e[0]),n=parseFloat(e[1]),r=parseFloat(e[2]);return[t,n,r].every(Number.isFinite)?{r:t,g:n,b:r}:null}const n=/^#([0-9a-f]{3}|[0-9a-f]{6})$/i.exec(t);if(n){let e=n[1];return 3===e.length&&(e=e[0]+e[0]+e[1]+e[1]+e[2]+e[2]),{r:parseInt(e.slice(0,2),16),g:parseInt(e.slice(2,4),16),b:parseInt(e.slice(4,6),16)}}return null}(e);if(!t)return null;const{L:i,C:n,h:r}=function rgbToOklch({r:e,g:t,b:i}){const n=srgbToLinear(e),r=srgbToLinear(t),s=srgbToLinear(i),l=.4122214708*n+.5363325363*r+.0514459929*s,d=.2119034982*n+.6806995451*r+.1073969566*s,c=.0883024619*n+.2817188376*r+.6299787005*s,u=Math.cbrt(l),p=Math.cbrt(d),g=Math.cbrt(c),m=1.9779984951*u-2.428592205*p+.4505937099*g,f=.0259040371*u+.7827717662*p-.808675766*g;return{L:.2104542553*u+.793617785*p-.0040720468*g,C:Math.hypot(m,f),h:Math.atan2(f,m)}}(t);return{L:i,a:n*Math.cos(r),b:n*Math.sin(r)}}function mixOklab(e){let t=0,i=0,n=0,r=0;for(const{c:l,w:d}of e)d<=0||(t+=l.L*d,i+=l.a*d,n+=l.b*d,r+=d);if(r<=0)return null;t/=r,i/=r,n/=r;const s=function oklchToRgb({L:e,C:t,h:i}){const n=t*Math.cos(i),r=t*Math.sin(i),s=(e+.3963377774*n+.2158037573*r)**3,l=(e-.1055613458*n-.0638541728*r)**3,d=(e-.0894841775*n-1.291485548*r)**3;return{r:linearToSrgb(4.0767416621*s-3.3077115913*l+.2309699292*d),g:linearToSrgb(-1.2684380046*s+2.6097574011*l-.3413193965*d),b:linearToSrgb(-.0041960863*s-.7034186147*l+1.707614701*d)}}({L:t,C:Math.hypot(i,n),h:Math.atan2(n,i)});return`rgb(${s.r}, ${s.g}, ${s.b})`}var midPoint=(e,t,i,n)=>[(e-i)/2+i,(t-n)/2+n];function getPath(e){const t=e.filter(Boolean);if(!t.length)return"";let i=t[0],n=i,r=`M ${i[0]},${i[1]}`;for(const s of t){n=s;const[e,t]=midPoint(i[0],i[1],n[0],n[1]);r+=` ${e},${t} Q${n[0]},${n[1]}`,i=n}return r+=` ${n[0]},${n[1]}`,r}var mkGrid=()=>Array.from({length:7},()=>new Array(96).fill(0));function weekBoundsFor(e){const t=localMidnightMinusDays((i=localMidnightMinusDays(0),(new Date(i).getDay()+6)%7+7*Math.max(0,e)));var i;const n=new Date(t);return n.setDate(n.getDate()+6),{startMs:t,endMs:n.getTime()+fe}}function percentile(e,t){return 0===e.length?0:e[Math.min(e.length-1,Math.max(0,Math.round(t/100*(e.length-1))))]}function bin(e,t,i,n){if(e)for(const r of e){if(!Number.isFinite(r.kwh))continue;const e=t(r.startMs);e&&(i[e.r][e.c]+=Math.max(0,r.kwh),n[e.r][e.c]+=1)}}async function buildFingerprint(e,t,i,n,r){if(!e?.callWS)return null;const{startMs:s,endMs:l}=weekBoundsFor(i),d=Array.from({length:7},(e,t)=>{const i=new Date(s);return i.setDate(i.getDate()+t),i.getTime()}),c=Math.min(Date.now(),l),u=t.batteryStatEnergyFroms.length>0||t.batteryStatEnergyTos.length>0,p=null!==r.priceNumber||null!==r.priceValue,g=p||r.compensationStats.length>0,m=!p&&r.compensationStats.length>0,[f,v,b,y,_,w]=await Promise.all([fetchChangeSeries(e,t.solarStatEnergyFroms,s,c,"5minute"),fetchChangeSeries(e,t.gridStatEnergyFroms,s,c,"5minute"),fetchChangeSeries(e,t.gridStatEnergyTos,s,c,"5minute"),u?fetchChangeSeries(e,t.batteryStatEnergyTos,s,c,"5minute"):Promise.resolve(null),u?fetchChangeSeries(e,t.batteryStatEnergyFroms,s,c,"5minute"):Promise.resolve(null),m?fetchChangeSeries(e,r.compensationStats,s,c,"5minute"):Promise.resolve(null)]),map=e=>{const t=(e=>{let t=-1;for(let i=0;i<7&&e>=d[i];i++)t=i;return t})(e);return t<0?null:{r:t,c:Math.min(95,Math.max(0,Math.floor(4*serverHourFrac(e))))}},H=mkGrid(),j=mkGrid(),z=mkGrid(),M=mkGrid(),$=mkGrid(),C=mkGrid(),D=mkGrid();bin(f,map,H,D),bin(v,map,j,D),bin(b,map,z,D),bin(y,map,M,D),bin(_,map,$,D),w&&bin(w,map,C,mkGrid());const A=r.priceNumber??r.priceValue??0,R=[],E=[];let T=!1;for(let L=0;L<7;L++){const e=[];for(let t=0;t<96;t++){const i=D[L][t]>0,n=H[L][t],r=j[L][t],s=z[L][t],l=M[L][t],c=$[L][t],u=Math.max(0,n-s-l),p=Math.max(0,r+c+u),g=p>0?Math.min(1,u/p):null,f=m?Math.max(0,C[L][t]):s*A;e.push({consumption:p,solar:n,gridImport:r,gridExport:s,batteryCharge:l,batteryDischarge:c,coverage:g,revenue:f,hasData:i,ms:d[L]+15*t*6e4}),i&&(T=!0,E.push(p))}R.push(e)}const O=new Intl.DateTimeFormat(n||void 0,{weekday:"short",day:"numeric"});return{rows:7,cols:96,startMs:s,endMs:l,cells:R,rowLabels:d.map(e=>O.format(new Date(e))),p95:Math.max(1e-6,percentile([...E].sort((e,t)=>e-t),95)),hasAnyData:T,hasBattery:u,hasSale:g}}var Ct=i$6`
...
`,Dt={type:"helios-fingerprint-card",name:"Helios Energy Fingerprint",description:"A heatmap of your home's energy habits: intensity is consumption, colour is solar coverage.",preview:!0};window.customCards=window.customCards||[];var At=window.customCards.findIndex(e=>"helios-fingerprint-card"===e.type);At>=0?window.customCards[At]=Dt:window.customCards.push(Dt);var Rt,Et=[{name:"title",selector:{text:{}}}],Tt={title:"Title"},Ot=class HeliosFingerprintCardEditor extends le{constructor(...e){super(...e),this._config={type:""},this._computeLabel=e=>Tt[e.name]??e.name,this._valueChanged=e=>{e.stopPropagation(),this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:{...this._config,...e.detail.value}},bubbles:!0,composed:!0}))}}setConfig(e){this._config={...e}}render(){return this.hass?B`
            <ha-form
                .hass=${this.hass}
                .data=${this._config}
                .schema=${Et}
            ...
            ></ha-form>`:K}};__decorate([n$1({attribute:!1}),__decorateMetadata("design:type",Object)],Ot.prototype,"hass",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Ot.prototype,"_config",void 0),Ot=__decorate([t$2("helios-fingerprint-card-editor")],Ot);var Lt=0,Pt=50,Ft=40,It=(Rt=class HeliosFingerprintCard extends le{constructor(...e){super(...e),this._energyDefaults=Xe,this._energyDefaultsLoaded=!1,this._model=null,this._weekOffset=0,this._hover=null,this._detailRow=null,this._cursor=null,this._hidden=/* @__PURE__ */new Set,this._navLoading=!1,this._wallComputing=!1,this._pendingCursorQ=null,this._uid="fp"+ ++Lt,this._srcColors=null,this._modelKey="",this._wallOffset=1/0,this._lastThemesRef=void 0,this._paintedModel=null,this._paintedSize="",this._paintedDetail=null,this._disp=null,this._animKey="",this._animRow=null,this._prevWeek=()=>{this._weekOffset+1<this._wallOffset&&(this._weekOffset+=1)},this._nextWeek=()=>{this._weekOffset>0&&(this._weekOffset-=1)},this._closeDetail=()=>{this._detailRow=null},this._onMove=e=>{if("touch"===e.pointerType)return;const t=this._geom(),i=this._model;if(!t||!i)return;const n=Math.floor((e.clientX-t.box.left-Pt)/t.cw),r=Math.floor((e.clientY-t.box.top)/t.ch);if(r<0||r>=i.rows||n<0||n>=i.cols||!i.cells[r][n].hasData)return this._hover&&(this._hover=null),void(this._cursor&&(this._cursor=null));if(this._hover=this._makeHover(r,n,e.clientX,e.clientY,t),r===this._detailRow&&this._chart&&this._dayView){const e=this._chart.getBoundingClientRect();e.width>0&&(this._cursor=this._cursorAt(n,e,this._dayView))}else this._cursor&&(this._cursor=null)},this._onLeave=e=>{"touch"!==e.pointerType&&this._hover&&(this._hover=null)},this._onClick=e=>{const t=this._geom(),i=this._model;if(!t||!i)return;const n=Math.floor((e.clientY-t.box.top)/t.ch);if(n<0||n>=i.rows)return;if(e.clientX-t.box.left<Pt)return void(this._detailRow=this._detailRow===n?null:n);const r=Math.floor((e.clientX-t.box.left-Pt)/t.cw);r<0||r>=i.cols||!i.cells[n][r].hasData||(this._detailRow=n,this._pendingCursorQ=r,this._disp=this._dayTarget(i.cells[n]),this._animRow=n,this._animKey=`${this._weekOffset}|${n}`)},this._onChartMove=e=>{const t=this._chart,i=this._dayView;if(!t||!i)return;const n=t.getBoundingClientRect();if(n.width<=0)return;const r=Math.min(1,Math.max(0,(e.clientX-n.left)/n.width));this._cursor=this._cursorAt(Math.floor(r*i.cols),n,i)},this._onChartLeave=()=>{this._cursor&&(this._cursor=null)},this._onLegend=e=>{const t=e.currentTarget?.dataset?.k;if(!t)return;const i=new Set(this._hidden);i.has(t)?i.delete(t):i.add(t),this._hidden=i}}setConfig(e){this._config={...e}}getCardSize(){return 4}getGridOptions(){return{rows:8,columns:12,min_rows:6,max_rows:16,min_columns:12,max_columns:12}}static getConfigElement(){return document.createElement("helios-fingerprint-card-editor")}static getStubConfig(){return{type:"custom:helios-fingerprint-card"}}get _prefsHost(){return this}get _t(){return pickTranslations(this.hass?.language).fingerprint??pickTranslations("en").fingerprint}connectedCallback(){super.connectedCallback(),this.hass&&subscribeEnergyPrefs(this._prefsHost)}disconnectedCallback(){super.disconnectedCallback(),unsubscribeEnergyPrefs(this._prefsHost),this._ro?.disconnect(),this._ro=void 0,void 0!==this._animFrame&&(cancelAnimationFrame(this._animFrame),this._animFrame=void 0)}updated(e){e.has("hass")&&this.hass&&(setServerTimeZone(this.hass.config?.time_zone),this._energyPrefsUnsub||subscribeEnergyPrefs(this._prefsHost),this.hass.themes!==this._lastThemesRef&&(this._lastThemesRef=this.hass.themes,this._resolveColors())),this._srcColors||this._resolveColors(),this._computeWall(),this._maybeRefresh(),this._applyPendingCursor(),this._svg&&!this._ro&&"undefined"!=typeof ResizeObserver&&(this._ro=new ResizeObserver(()=>this._paintGrid()),this._ro.observe(this._svg)),this._paintGrid(),this._syncDayAnim()}_syncDayAnim(){const e=this._model,t=this._detailRow,i=`${this._weekOffset}|${t??-1}`;if(i===this._animKey)return;const n=t!==this._animRow;if(this._animKey=i,this._animRow=t,void 0!==this._animFrame&&(cancelAnimationFrame(this._animFrame),this._animFrame=void 0),!e||null===t||t>=e.rows)return void(this._disp=null);const r=e.cells[t],s=this._dayTarget(r),l=r.map(()=>0),d=this._disp??{cons:l,imp:l,sol:l,bat:l},c="undefined"!=typeof matchMedia&&matchMedia("(prefers-reduced-motion: reduce)").matches,u=function parseCssDuration(e){const t=e.trim();if(!t)return 0;const i=t.endsWith("ms"),n=parseFloat(i?t.slice(0,-2):t.endsWith("s")?t.slice(0,-1):t);return!Number.isFinite(n)||n<0?0:i||!t.endsWith("s")?n:1e3*n}(getComputedStyle(this).getPropertyValue("--ha-animation-duration-slow"));if(!n||c||u<=1)return this._disp=s,void this.requestUpdate();const p=performance.now(),step=e=>{const t=Math.min((e-p)/u,1),i=1-(1-t)**3,lerp=(e,t)=>t.map((t,n)=>(e[n]??0)+(t-(e[n]??0))*i);this._disp={cons:lerp(d.cons,s.cons),imp:lerp(d.imp,s.imp),sol:lerp(d.sol,s.sol),bat:lerp(d.bat,s.bat)},this.requestUpdate(),this._animFrame=t<1?requestAnimationFrame(step):void 0};this._animFrame=requestAnimationFrame(step)}_resolveColors(){if(!this.shadowRoot)return;const e=this._probe??(this._probe=(()=>{const e=document.createElement("span");return e.style.display="none",this.shadowRoot.appendChild(e),e})()),read=t=>(e.style.color=t,getComputedStyle(e).color),t=toOklab(read("var(--energy-grid-consumption-color, var(--primary-color, #488fc2))")),i=toOklab(read("var(--energy-solar-color, var(--warning-color, #ff9800))"));this._srcColors=t&&i?{grid:t,solar:i}:null,this._paintedModel=null}async _computeWall(){if(this._wallOffset===1/0&&!this._wallComputing&&this._energyDefaultsLoaded&&this.hass?.callWS){this._wallComputing=!0;try{const{startMs:e}=weekBoundsFor(8),t=await fetchChangeSeries(this.hass,this._energyDefaults.gridStatEnergyFroms,e,Date.now(),"5minute");let i=1;if(t&&t.length){let e=1/0;for(const i of t)i.startMs<e&&(e=i.startMs);i=8;for(let t=0;t<=8;t++)if(weekBoundsFor(t).endMs<=e){i=t;break}}this._wallOffset=Math.max(1,Math.min(i,8)),this.requestUpdate()}finally{this._wallComputing=!1}}}async _maybeRefresh(){if(!this._energyDefaultsLoaded||!this.hass?.callWS)return;const e=this._energyDefaults,t=Math.floor(Date.now()/3e5),i={compensationStats:e.gridExportCompensationStats,priceNumber:e.gridExportPriceNumber,priceValue:this._resolvePrice(e.gridExportPriceEntity)},n=`${this._weekOffset}|${t}|${e.solarStatEnergyFroms}|${e.gridStatEnergyFroms}|${e.gridStatEnergyTos}|${i.priceNumber}|${i.priceValue}|${e.gridExportCompensationStats}`;if(n!==this._modelKey){this._modelKey=n,this._navLoading=!0;try{const t=await buildFingerprint(this.hass,e,this._weekOffset,this.hass?.language??"",i);if(this._modelKey!==n)return;this._model=t}finally{this._navLoading=!1}}}_weekLabel(){const e=this._model;return e?new Intl.DateTimeFormat(this.hass?.language||void 0,{day:"numeric",month:"short"}).formatRange(new Date(e.startMs),/* @__PURE__ */new Date(e.endMs-1)):""}_paintGrid(){const e=this._model,t=this._svg;if(!t||!e)return;const i=t.getBoundingClientRect(),n=i.width,r=i.height;if(n<60||r<40)return;const s=`${Math.round(n)}x${Math.round(r)}`;if(this._model===this._paintedModel&&s===this._paintedSize&&this._detailRow===this._paintedDetail)return;this._paintedModel=this._model,this._paintedSize=s,this._paintedDetail=this._detailRow,t.setAttribute("viewBox",`0 0 ${n} ${r}`);const l=(n-Pt)/e.cols,d=(r-16)/e.rows;let c="";for(let g=0;g<e.rows;g++){const t=g*d;this._detailRow===g&&(c+=`<rect x="${47..toFixed(2)}" y="${(t-1).toFixed(2)}" width="${(e.cols*l+3).toFixed(2)}" height="${Math.max(0,d-1+2).toFixed(2)}" rx="4" fill="var(--primary-text-color)" fill-opacity="0.07"></rect>`,c+=`<rect x="${47..toFixed(2)}" y="${(t+1).toFixed(2)}" width="2.5" height="${Math.max(0,d-1-3).toFixed(2)}" rx="1.25" fill="var(--primary-color)"></rect>`);for(let i=0;i<e.cols;i++){const n=e.cells[g][i],r=Pt+i*l;let s="var(--divider-color, rgba(127,127,127,0.2))",u=.1;if(n.hasData){const t=n.consumption;if(u=t>0?.22+.75*Math.min(1,t/e.p95)**.72:.1,t>0&&this._srcColors){const e=n.coverage??0;s=mixOklab([{c:this._srcColors.grid,w:1-e},{c:this._srcColors.solar,w:e}])??"var(--primary-color)"}else s="var(--primary-color)"}c+=`<rect x="${r.toFixed(2)}" y="${t.toFixed(2)}" width="${Math.max(0,l-1).toFixed(2)}" height="${Math.max(0,d-1).toFixed(2)}" rx="2" fill="${s}" fill-opacity="${u.toFixed(3)}"></rect>`}c+=`<text x="43" y="${(t+d/2).toFixed(1)}" text-anchor="end" dominant-baseline="central" class="fp-ylabel ${this._detailRow===g?"is-on":""}">${e.rowLabels[g]}</text>`}const u=n-Pt,p=[1,2,3,4,6].find(e=>u/24*e>=44)??6;for(let g=p;g<24;g+=p)c+=`<text x="${(Pt+g/24*u).toFixed(1)}" y="${r-4}" text-anchor="middle" class="fp-xlabel">${String(g).padStart(2,"0")}h</text>`;t.innerHTML=c}_geom(){const e=this._svg,t=this._model;if(!e||!t)return null;const i=e.getBoundingClientRect(),n=(i.width-Pt)/t.cols,r=(i.height-16)/t.rows;return n<=0||r<=0?null:{box:i,cw:n,ch:r}}_makeHover(e,t,i,n,r){const s=i-r.box.left,l=n-r.box.top;return{r:e,q:t,cx:s,cy:Math.min(Math.max(l,46),Math.max(46,r.box.height-46)),x:Pt+t*r.cw,y:e*r.ch,w:Math.max(0,r.cw-1),h:Math.max(0,r.ch-1),flip:s>.5*r.box.width}}_dayTarget(e){return{cons:e.map(e=>e.consumption),imp:e.map(e=>e.gridImport),sol:e.map(e=>e.solar),bat:e.map(e=>e.batteryDischarge-e.batteryCharge)}}_renderHover(){const e=this._hover,t=this._model;if(!e||!t)return K;const i=t.cells[e.r][e.q],n=this._t,r=this.hass?.language||void 0,s=new Date(i.ms),l=`${new Intl.DateTimeFormat(r,{weekday:"short",day:"numeric"}).format(s)} - ${new Intl.DateTimeFormat(r,{hour:"2-digit",minute:"2-digit"}).format(s)}`,d=i.consumption,c=Math.max(0,d-i.gridImport-i.batteryDischarge),u=Math.round(4*d*1e3),srcRow=(e,t,i)=>B`
            <div class="fp-tip-src"><span><i class="fp-sdot ${e}"></i>${t}</span><span>${i.toFixed(2)} kWh · ${(e=>d>0?`${Math.round(e/d*100)}%`:"-")(i)}</span></div>`;return B`
            <div class="fp-hl" style="left:${e.x}px; top:${e.y}px; width:${e.w}px; height:${e.h}px"></div>
            <div class="fp-tip ${e.flip?"is-left":""}" style="left:${e.cx}px; top:${e.cy}px">
                <div class="fp-tip-head">${l}</div>
                <div class="fp-tip-row fp-tip-total"><span>${n.consumption}</span><span>${d.toFixed(2)} kWh · ${u} W</span></div>
                ${srcRow("fp-sdot-imp",n.grid,i.gridImport)}
                ${srcRow("fp-sdot-sol",n.solar,c)}
                ${t.hasBattery?srcRow("fp-sdot-bat",n.battery,i.batteryDischarge):K}
                ${i.gridExport>0?B`<div class="fp-tip-src fp-tip-export"><span>${t.hasSale?n.sold:n.exported}</span><span>${i.gridExport.toFixed(2)} kWh${t.hasSale?` · ${this._fmtMoney(i.revenue)}`:""}</span></div>`:K}
            </div>`}_renderDetail(){const e=this._t,t=this._model,i=this._detailRow;if(!t||null===i||i>=t.rows)return this._dayView=void 0,B`<div class="fp-detail is-empty"><span>${e.selectDay}</span></div>`;const n=t.cells[i],r=t.cols,s=r-1,l=t.hasBattery,d=this._hidden,c=!d.has("cons"),u=!d.has("imp"),p=!d.has("sol"),g=l&&!d.has("bat"),m=!d.has("loss"),f=this._disp?.cons??n.map(e=>e.consumption),v=this._disp?.imp??n.map(e=>e.gridImport),b=this._disp?.sol??n.map(e=>e.solar),y=this._disp?.bat??n.map(e=>e.batteryDischarge-e.batteryCharge);let _=1e-6,w=0;for(const P of n)if(P.hasData&&(_=Math.max(_,P.consumption,P.gridImport,P.solar),l)){const e=P.batteryDischarge-P.batteryCharge;_=Math.max(_,e),w=Math.min(w,e)}const H=_-w,yOf=e=>37-(e-w)/H*34;this._dayView={yMin:w,yMax:_,cons:f,imp:v,sol:b,bat:y,hasBattery:l,cols:r};const j=[0,1,2,3,4].map(e=>3+34*e/4),z=[.25,.5,.75].map(e=>e*r),coordsOf=e=>[...e.map((e,t)=>[t,yOf(e)]),[r,yOf(e[s])]],line=(e,t)=>q`<path class=${e} stroke-width=${2} vector-effect="non-scaling-stroke" stroke-linecap="round" stroke-linejoin="round" d=${t}></path>`,M=f.map((e,t)=>e+Math.max(0,-y[t])),$=[...b.map((e,t)=>[t,Math.min(yOf(e),yOf(M[t]))]),[r,Math.min(yOf(b[s]),yOf(M[s]))]],C=[[r,yOf(M[s])],...M.map((e,t)=>[s-t,yOf(M[s-t])])],D=`${getPath($)} ${getPath(C).replace(/^M/,"L")} Z`,A=yOf(0),R=`${this._uid}-up`,E=`${this._uid}-down`,T=this.hass?.language||void 0,O=new Intl.DateTimeFormat(T,{weekday:"long",day:"numeric",month:"long"}).format(new Date(n[0].ms)),L=this._cursor,leg=(e,t,i)=>B`
            <li class=${d.has(e)?"hidden":""}>
                <button type="button" class="legend-toggle" data-k=${e} aria-pressed=${!d.has(e)} @click=${this._onLegend}>
                    <ha-svg-icon .path=${d.has(e)?"M12,20A8,8 0 0,1 4,12A8,8 0 0,1 12,4A8,8 0 0,1 20,12A8,8 0 0,1 12,20M12,2A10,10 0 0,0 2,12A10,10 0 0,0 12,22A10,10 0 0,0 22,12A10,10 0 0,0 12,2Z":"M12 2C6.5 2 2 6.5 2 12S6.5 22 12 22 22 17.5 22 12 17.5 2 12 2M10 17L5 12L6.41 10.59L10 14.17L17.59 6.58L19 8L10 17Z"} style=${d.has(e)?"":`color:${t}`}></ha-svg-icon>
                </button>
                <button type="button" class="label" data-k=${e} @click=${this._onLegend}>${i}</button>
            </li>`;return B`
            <div class="fp-detail">
                <div class="fp-detail-head">
                    <span class="fp-detail-day">${O}</span>
                    <button type="button" class="fp-detail-close" aria-label=${e.close} @click=${this._closeDetail}>
                ...
                <div class="fp-chart" style="margin-left:${Pt}px" @pointermove=${this._onChartMove} @pointerdown=${this._onChartMove} @pointerleave=${this._onChartLeave}>
                    ${q`
                        <svg class="fp-detail-svg" viewBox="0 0 ${r} ${Ft}" preserveAspectRatio="none" xmlns="http://www.w3.org/2000/svg">
                            ${g?q`<defs>
                                <clipPath id=${R}><rect x="0" y="0" width=${r} height=${A}></rect></clipPath>
                                <clipPath id=${E}><rect x="0" y=${A} width=${r} height=${Ft-A}></rect></clipPath>
                            </defs>`:K}
                            ${j.map(e=>q`<line class="fp-grid-line" x1="0" y1=${e} x2=${r} y2=${e} vector-effect="non-scaling-stroke"></line>`)}
                            ${z.map(e=>q`<line class="fp-grid-line" x1=${e} y1=${3} x2=${e} y2=${37} vector-effect="non-scaling-stroke"></line>`)}
                            ${m?q`<path class=${t.hasSale?"fp-area-sold":"fp-area-export"} d=${D}></path>`:K}
                            ${g?q`<line class="fp-zero" x1="0" y1=${A} x2=${r} y2=${A} vector-effect="non-scaling-stroke"></line>`:K}
                            ${c?line("fp-line-cons",getPath(coordsOf(f))):K}
                            ${u?line("fp-line-imp",getPath(coordsOf(v))):K}
                            ${p?line("fp-line-sol",getPath(coordsOf(b))):K}
                            ${g?q`
                                <g clip-path="url(#${R})">${line("fp-line-bat-out",getPath(coordsOf(y)))}</g>
                                <g clip-path="url(#${E})">${line("fp-line-bat-in",getPath(coordsOf(y)))}</g>
                            `:K}
                        </svg>`}
                    ${j.map((e,t)=>B`<span class="fp-yval" style="top:${(e/Ft*100).toFixed(2)}%">${this._fmtKwh(w+(1-t/4)*H)}${0===t?" kWh":""}</span>`)}
                    ${L?B`
                        <div class="fp-cursor" style="left:${L.xPx}px; height:${L.hPx}px"></div>
                        <div class="fp-time" style="left:${L.xPx}px">${this._slotLabel(n[L.q].ms)}</div>
                        ${c?B`<div class="fp-beam fp-beam-cons" style="left:${L.xPx}px; top:${L.consY}px"></div>`:K}
                        ${u?B`<div class="fp-beam fp-beam-imp" style="left:${L.xPx}px; top:${L.impY}px"></div>`:K}
                        ${p?B`<div class="fp-beam fp-beam-sol" style="left:${L.xPx}px; top:${L.solY}px"></div>`:K}
                        ${g&&null!==L.batY?B`<div class="fp-beam ${L.batCharge?"fp-beam-bat-in":"fp-beam-bat-out"}" style="left:${L.xPx}px; top:${L.batY}px"></div>`:K}
                    `:K}
                </div>
                <div class="fp-detail-legend chart-legend">
                    <ul>
                        ${leg("cons","var(--primary-text-color, #212121)",e.consumption)}
                        ${leg("imp","var(--energy-grid-consumption-color, var(--primary-color, #488fc2))",e.grid)}
                        ${leg("sol","var(--energy-solar-color, var(--warning-color, #ff9800))",e.solar)}
                        ${l?leg("bat","var(--energy-battery-out-color, #4db6ac)",e.battery):K}
                        ${t.hasSale?leg("loss","var(--success-color, #4caf50)",e.sold):leg("loss","var(--energy-grid-return-color, #8353d1)",e.exported)}
                    </ul>
                </div>
            </div>`}_slotLabel(e){const t=new Intl.DateTimeFormat(this.hass?.language||void 0,{hour:"2-digit",minute:"2-digit"});return`${t.format(new Date(e))} - ${t.format(new Date(e+9e5))}`}_fmtKwh(e){if(Math.abs(e)<.005)return"0";const t=Math.abs(e);return e.toFixed(t>=10?0:t>=1?1:2)}_resolvePrice(e){if(!e)return null;const t=Number.parseFloat(this.hass?.states?.[e]?.state);return Number.isFinite(t)?t:null}_fmtMoney(e){const t=this.hass?.config?.currency,i=this.hass?.language||void 0;try{return new Intl.NumberFormat(i,t?{style:"currency",currency:t}:{maximumFractionDigits:2}).format(e)}catch{return e.toFixed(2)}}_cursorAt(e,t,i){const n=Math.min(i.cols-1,Math.max(0,e)),r=n/i.cols*t.width,s=i.yMax-i.yMin,yPx=e=>(37-(e-i.yMin)/s*34)/Ft*t.height;return{q:n,xPx:r,hPx:t.height,consY:yPx(i.cons[n]),impY:yPx(i.imp[n]),solY:yPx(i.sol[n]),batY:i.hasBattery?yPx(i.bat[n]):null,batCharge:i.bat[n]<0}}_applyPendingCursor(){if(null===this._pendingCursorQ)return;const e=this._chart,t=this._dayView;if(!e||!t)return;const i=e.getBoundingClientRect();if(i.width<=0)return;const n=this._pendingCursorQ;this._pendingCursorQ=null,this._cursor=this._cursorAt(n,i,t)}_renderChartLink(){const e=this._cursor,t=this._detailRow,i=this._model;if(!e||null===t||!i)return K;const n=this._geom();return n?B`<div class="fp-hl" style="left:${Pt+e.q*n.cw}px; top:${t*n.ch}px; width:${Math.max(0,n.cw-1)}px; height:${Math.max(0,n.ch-1)}px"></div>`:K}render(){if(!this._config)return K;const e=this._t,t=this._config.title??e.title,i=null!==this._model,n=this._wallOffset!==1/0&&this._weekOffset+1<this._wallOffset;return B`
            <ha-card>
                <div class="fp-header">
                    <span class="fp-title">${t}</span>
                    <div class="fp-nav">
                        <button type="button" class="fp-nav-btn" aria-label=${e.prevWeek} ?disabled=${!n||this._navLoading} @click=${this._prevWeek}>&lsaquo;</button>
                        <span class="fp-week">${this._weekLabel()}</span>
                        <button type="button" class="fp-nav-btn" aria-label=${e.nextWeek} ?disabled=${0===this._weekOffset||this._navLoading} @click=${this._nextWeek}>&rsaquo;</button>
                    </div>
                </div>
                <div class="fp-body">
                    ${i&&this._model.hasAnyData?B`
                            ...
                            ${this._renderDetail()}
                        `:B`<div class="fp-empty">${i?e.noData:e.loading}</div>`}
                </div>
            </ha-card>`}},Rt.styles=Ct,Rt);__decorate([n$1({attribute:!1}),__decorateMetadata("design:type",Object)],It.prototype,"hass",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],It.prototype,"_config",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],It.prototype,"_energyDefaults",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],It.prototype,"_energyDefaultsLoaded",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],It.prototype,"_model",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],It.prototype,"_weekOffset",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],It.prototype,"_hover",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],It.prototype,"_detailRow",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],It.prototype,"_cursor",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],It.prototype,"_hidden",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],It.prototype,"_navLoading",void 0),__decorate([e$2(".fp-svg"),__decorateMetadata("design:type","undefined"==typeof SVGSVGElement?Object:SVGSVGElement)],It.prototype,"_svg",void 0),__decorate([e$2(".fp-chart"),__decorateMetadata("design:type","undefined"==typeof HTMLElement?Object:HTMLElement)],It.prototype,"_chart",void 0),It=__decorate([t$2("helios-fingerprint-card")],It);var Ut,Wt,Nt=i$6`
    .editor
    {
        padding: 16px;
        display: flex;
...
`,Bt=(Ut=class HeliosCardEditor extends le{constructor(...e){super(...e),this._cfg={},this._pickerReady=!1,this._openSection=null,this._sliderDebounce=/* @__PURE__ */new Map,this._energyDefaults=Xe,this._energyDefaultsLoaded=!1,this._gridGuard={status:"unknown",cleanEvals:0,fetchKey:"",fetching:!1,entityKey:""},this._onSectionToggleEvt=e=>{const t=e.currentTarget.dataset.section;t&&this._onSectionToggle(t,e)},this._onNumFieldChange=e=>{const t=e.currentTarget.dataset.key;t&&this._numField(t,e)},this._onNumSliderInput=e=>{const t=e.currentTarget.dataset.key;t&&this._numSlider(t,e)},this._onEntityValueChanged=e=>{const t=e.currentTarget.dataset.key;if(!t)return;const i=e.detail.value,n=null==i||""===i?void 0:i;(this._cfg[t]??void 0)!==(n??void 0)&&this._update(t,n)},this._onBoolToggleClick=e=>{const t=e.currentTarget,i=t.dataset.key;i&&this._update(i,"true"===t.dataset.value)},this._onDeviceToggleClick=e=>{const t=e.currentTarget.dataset.stat;if(!t)return;const i=this._cfg["hidden-devices"],n=Array.isArray(i)?i.filter(e=>"string"==typeof e):[],r=n.includes(t)?n.filter(e=>e!==t):[...n,t];this._update("hidden-devices",r.length?r:void 0)},this._onDeviceGroupClick=e=>{const t=e.currentTarget.dataset.stat;if(!t)return;const i=monitoringGroups(this._cfg),n=(i.get(t)??0)+1,r=n>4?0:n,s={};for(const[l,d]of i)l!==t&&(s[l]=d);r>=1&&(s[t]=r),this._update("monitoring-groups",Object.keys(s).length?s:void 0)},this._solarIrradianceEntityFilter=e=>{if(!e||!e.attributes)return!1;if("irradiance"===e.attributes.device_class)return!0;const t=String(e.attributes.unit_of_measurement??"").trim();return"W/m²"===t||"W/m2"===t},this._onGroupNameChanged=e=>{const t=e.currentTarget;t.dataset.group&&this._updateGroupMap("monitoring-group-names",t.dataset.group,t.value.trim())},this._onGroupColorChanged=e=>{e.stopPropagation();const t=e.currentTarget.dataset.group;t&&this._updateGroupMap("monitoring-group-colors",t,"string"==typeof e.detail.value?e.detail.value:"")},this._onGroupIconChanged=e=>{e.stopPropagation();const t=e.currentTarget.dataset.group;t&&this._updateGroupMap("monitoring-group-icons",t,"string"==typeof e.detail.value?e.detail.value:"")},this._resetFeedback=null}disconnectedCallback(){super.disconnectedCallback(),unsubscribeEnergyPrefs(this);for(const e of this._sliderDebounce.values())window.clearTimeout(e);this._sliderDebounce.clear(),void 0!==this._resetFeedbackTimer&&(window.clearTimeout(this._resetFeedbackTimer),this._resetFeedbackTimer=void 0)}setConfig(e){if(this._cfg={...e},Wt.LEGACY_KEYS.some(e=>e in this._cfg)&&setTimeout(()=>{if(!this.isConnected)return;const e={...this._cfg};let t=!1;for(const i of Wt.LEGACY_KEYS)i in e&&(delete e[i],t=!0);t&&(this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:e}})),this._cfg=e)},0),!this._cfg["cache-id"]){const e=`c${Date.now().toString(36)}${Math.floor(1e9*Math.random()).toString(36)}`;setTimeout(()=>{this.isConnected&&(this._cfg["cache-id"]||this._update("cache-id",e))},0)}}connectedCallback(){super.connectedCallback(),this._ensureEntityPicker(),subscribeEnergyPrefs(this)}async _ensureEntityPicker(){if(!this._pickerReady)if("undefined"!=typeof customElements&&customElements.get("ha-entity-picker"))this._pickerReady=!0;else try{const e=window;if("function"==typeof e.loadCardHelpers){const t=await e.loadCardHelpers();if(t?.createCardElement){const e=(await t.createCardElement({type:"entities",entities:[]}))?.constructor;"function"==typeof e?.getConfigElement&&await e.getConfigElement()}}"undefined"!=typeof customElements&&await Promise.race([customElements.whenDefined("ha-entity-picker"),new Promise(e=>{setTimeout(e,Wt.PICKER_LOAD_TIMEOUT_MS)})])}catch(e){}finally{this._pickerReady=!0}}_t(){return pickTranslations(this.hass?.language)}updated(){this.hass&&refreshGridGuard(this),this._pruneStaleDeviceIds()}_pruneStaleDeviceIds(){if(!this._energyDefaultsLoaded)return;const e=this._energyDefaults.devices;if(0===e.length)return;const t=new Set(e.map(e=>e.statConsumption)),i=["hidden-devices"],n={...this._cfg};let r=!1;for(const l of i){const e=this._cfg[l];if(!Array.isArray(e))continue;const i=e.filter(e=>"string"==typeof e&&t.has(e));i.length!==e.length&&(r=!0,i.length?n[l]=i:delete n[l])}const s=this._cfg["monitoring-groups"];if(s&&"object"==typeof s&&!Array.isArray(s)){const e={};let i=!1;for(const[n,r]of Object.entries(s))t.has(n)&&"number"==typeof r?e[n]=r:i=!0;i&&(r=!0,Object.keys(e).length?n["monitoring-groups"]=e:delete n["monitoring-groups"])}r&&(this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:n}})),this._cfg=n)}_liveStatusLine(e,t,i){return B`
            ...
            ${t.map(t=>B`
                <div class="group-block">
                    <div class="group-line">
                        <span class="group-name-badge" style="--group-pill-color:${monitoringGroupColor(this._cfg,t)}">${(()=>{const e=monitoringGroupIcon(this._cfg,t);return e?B`<ha-icon icon=${e}></ha-icon>`:B`${t}`})()}</span>
                        <input
                ...
                <details class="advanced-section about-section" data-section="about" ?open=${"about"===this._openSection} @toggle=${this._onSectionToggleEvt}>
                    ...
                    <div class="about-row">
                        ...
                        <a class="about-row-link about-version-link"
                           href="https://github.com/ReikanYsora/Helios/releases/tag/v${"2026.8.0-b2"}"
                    ...
                    >
                        ...
                        >
                            ...
                            >
                                ...
                                ${"standard"===this._timelineMode||"today"===this._timelineMode||"yesterday"===this._timelineMode||"week"===this._timelineMode?renderTimelineNightZones(this):K}
                                ${function renderTimelineFutureMask(e){const t=e._timeRange;if(!t)return K;const i=t.start.getTime(),n=t.end.getTime(),r=n-i;if(r<=0)return K;const s=Date.now();return s<=i||s>=n?K:B`
        <div
    ...
    `:K}(this):K}
        ...
        `}_maybeRebuildUnifiedStore(){(function isStoreFresh(e,t){return!!t&&t.dataVersion===computeDataVersion(e)})(this,this._unifiedStore)||(this._unifiedStore=buildUnifiedStore(this))}_isCameraLocked(){return!!this._engine&&this._engine.isCameraLocked()}_registerCacheId(){const e=cacheId(this.config);if(!e)return;const t=Gt.get(e)??[];t.includes(this)||(t.push(this),Gt.set(e,t))}_unregisterCacheId(){const e=cacheId(this.config),t=e?Gt.get(e):void 0;if(!t)return;const i=t.indexOf(this);i>=0&&t.splice(i,1),0===t.length&&Gt.delete(e)}effectiveCacheId(){const e=cacheId(this.config);if(!e)return"";const t=Gt.get(e),i=t?t.indexOf(this):-1;return i>0?`${e}#${i+1}`:e}_uiStateStorageKey(){const e=this.effectiveCacheId();if(e)return`helios:ui-state:${e}`;const t=getHomeCoords(this.config,this.hass);return t?`helios:ui-state:${Math.round(1e3*t.lat)/1e3}:${Math.round(1e3*t.lon)/1e3}`:null}_restoreUiState(){if(this._uiStateRestored)return;const e=this._uiStateStorageKey();if(e){this._uiStateRestored=!0;try{const t=window.localStorage.getItem(e);if(!t)return;const i=JSON.parse(t);if(i&&"object"==typeof i){const e=["production","consumption","grid","battery","battery-soc","irradiance",..._t];"string"==typeof i.chartTarget&&e.includes(i.chartTarget)&&(this._chartTarget=i.chartTarget),"string"==typeof i.timelineMode&&i.timelineMode in Be&&(this._timelineMode=i.timelineMode,this._periodPastDays=modePastDays(this._timelineMode),this._periodFutureDays=modeFutureDays(this._timelineMode))}}catch(L){}}}persistUiState(){const e=this._uiStateStorageKey();if(e)try{window.localStorage.setItem(e,JSON.stringify({chartTarget:this._chartTarget,timelineMode:this._timelineMode}))}catch(L){}}},Vt.styles=[Ye,Ze],Vt);__decorate([n$1({attribute:!1}),__decorateMetadata("design:type",Object)],Kt.prototype,"hass",void 0),__decorate([n$1({attribute:!1}),__decorateMetadata("design:type",Object)],Kt.prototype,"config",void 0),__decorate([n$1({attribute:!1}),__decorateMetadata("design:type",Object)],Kt.prototype,"preview",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Kt.prototype,"_engine",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Kt.prototype,"_now",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Kt.prototype,"_cloudCover",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Kt.prototype,"_labelLayout",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Kt.prototype,"_pvCurrent",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Kt.prototype,"_pvUnit",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Kt.prototype,"_pvChangeSeries",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Kt.prototype,"_pvChangeSeriesPerEntity",void 0),__decorate([r$1(),__decorateMetadata("design:type",Array)],Kt.prototype,"_haSolarForecast",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Kt.prototype,"_batterySoc",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Kt.prototype,"_batteryPower",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Kt.prototype,"_batteryPowerUnit",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Kt.prototype,"_gridImportValue",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Kt.prototype,"_gridImportUnit",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Kt.prototype,"_gridExportValue",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Kt.prototype,"_gridExportUnit",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Kt.prototype,"_gridImportChangeSeries",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Kt.prototype,"_gridExportChangeSeries",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Kt.prototype,"_batterySocHistory",void 0),__decorate([r$1(),__decorateMetadata("design:type",Array)],Kt.prototype,"_batterySocPerBankHistory",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Kt.prototype,"_periodHourly",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Kt.prototype,"_batteryChargeChangeSeries",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Kt.prototype,"_batteryDischargeChangeSeries",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Kt.prototype,"_deviceChangeSeries",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Kt.prototype,"_sunScene",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Kt.prototype,"_energyDefaults",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Kt.prototype,"_haSolarTodayKwh",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Kt.prototype,"_homeHover",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Kt.prototype,"_chartHoverPct",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Kt.prototype,"_chartTarget",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Kt.prototype,"_infoPanelOpen",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Kt.prototype,"_uiHidden",void 0),__decorate([e$2("ha-card"),__decorateMetadata("design:type","undefined"==typeof HTMLElement?Object:HTMLElement)],Kt.prototype,"_haCard",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Kt.prototype,"_chartSeries",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Kt.prototype,"_timeRange",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Kt.prototype,"_selectedTime",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Kt.prototype,"_isLiveMode",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Kt.prototype,"_timelineMode",void 0),__decorate([r$1(),__decorateMetadata("design:type",Object)],Kt.prototype,"_unifiedStore",void 0),Kt=__decorate([t$2("helios-card")],Kt);export{Kt as HeliosCard};