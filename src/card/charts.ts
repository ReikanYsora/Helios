//Stable import surface for the timeline charts: the shared structural types and the friendly-name label helpers that
//many modules read off, plus re-exports of the render + sampling concerns that now live in focused sibling modules.
//Charts only read; state mutations live elsewhere.
...
//darkens off the base solar token. Shared by the PV chart + the tooltip's per-source pastilles.
export const chartIsDark = (host: ChartHost): boolean => !!host.hass?.themes?.darkMode;


//Re-exports keeping every symbol that used to live here importable from this module, now that the render +
//sampling concerns sit in focused siblings.
