...
* **Day curve** *(when a solar source is configured)*, click the PV chip a second time and the day you are scrubbing rises around the house as a line, standing on the sun's own path projected down onto the ground. It is not a ring: the track closes in on the house at noon, when the sun is overhead, and reaches out to the arc's own feet at sunrise and sunset, so where a point sits on it IS the hour. A dashed leader drops from the sun to the curve beneath it, so the scrub reads the day at a glance. Today's remaining hours carry on from your solar forecast, dashed. The day writes itself on as it is raised, and the chips that have nothing to say about production step aside while it is up. Click the chip again to put it away.
...
| `src/scene/day-curve.ts`        | Day curve geometry: the sun's ground track, the spline, the depth split and the sun's leader |
